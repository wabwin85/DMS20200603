var InventoryAdjustList = {};

InventoryAdjustList = function () {
    var that = {};

    var business = 'Inventory.InventoryAdjustList';
    ...
    var openImportInfo = function () {
        top.createTab({
            id: 'M2_InventoryAdjust_ImportIAFile',
            title: 'Excel导入',
            url: 'Revolution/Pages/Inventory/InventoryAdjustAuditImport.aspx'
        });
    }

    //TODO:20191217 其他出入库导出明细，暂时隐藏，后期放开
    var exportListDetails = function () {
        var urlExport = Common.ExportUrl;
        var data = that.GetModel();
        data.QryBeginDate = $("#QryBeginDate").val();
        data.QryEndDate = $("#QryEndDate").val();
        urlExport = Common.UpdateUrlParams(urlExport, 'Business', business);
        urlExport = Common.UpdateUrlParams(urlExport, 'DownloadCookie', 'InventoryAdjustList');
        ...
        urlExport = Common.UpdateUrlParams(urlExport, 'CreateDateStart', data.QryBeginDate);
        urlExport = Common.UpdateUrlParams(urlExport, 'CreateDateEnd', data.QryEndDate);
        urlExport = Common.UpdateUrlParams(urlExport, 'AdjustNumber', data.QryOrderNo);
        urlExport = Common.UpdateUrlParams(urlExport, 'Status', data.QryStatus.Key);
        urlExport = Common.UpdateUrlParams(urlExport, 'Cfn', data.QryProductType);
        urlExport = Common.UpdateUrlParams(urlExport, 'LotNumber', data.QryLotNumber);
        startDownload(urlExport, 'InventoryAdjustList');//下载名称
    }
