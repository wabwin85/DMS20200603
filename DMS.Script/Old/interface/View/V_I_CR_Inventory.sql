DROP VIEW [interface].[V_I_CR_Inventory]
GO
...
CREATE VIEW [interface].[V_I_CR_Inventory]
AS
 --SELECT 
 --   ID=Lot.LOT_ID,
...
SELECT 
   ...
   DealerID=whm.WHM_DMA_ID
      ...
      ,PRH_Type=''
      ,'' AS FormNbr 
      ,isnull(TransferDate,isnull(ReceiptDate,'2016-02-01 0:00:00')) AS TransferDate
      ,isnull(ReceiptDate,'2016-02-01 0:00:00') AS ReceiptDate
	  FROM Inventory inv(nolock)
	  inner join Warehouse whm(nolock) on inv.INV_WHM_ID=whm.WHM_ID
	  inner join Product pma(nolock) on pma.PMA_ID=inv.INV_PMA_ID
	  inner join CFN(nolock) on CFN.CFN_ID=pma.PMA_CFN_ID
	  ...
	  left join Interface.Stage_V_Receipt_UPNLOT_MaxReceiptDate VR 
	     ON (VR.WHM_ID = WHM.WHM_DMA_ID AND
