USE [GenesisDMS_Test]
GO
/****** Object:  StoredProcedure [Contract].[Proc_QueryAppointmentList]    Script Date: 2019/9/29 16:22:08 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

ALTER PROCEDURE [Contract].[Proc_QueryAppointmentList](
...
BEGIN
	...
	FROM   [Contract].AppointmentMain A
		   ...
		   LEFT JOIN V_DivisionProductLineRelation F
	            ON  A.DepId = F.DivisionCode
	WHERE  (
	           @ContractNo = ''
	       ...
	       AND (
	               @CompanyName = ''
	               OR B.CompanyName LIKE '%' + @CompanyName + '%'
	           )
	       ...
	       AND (
	               ...
	               OR (
	                      ...
	                      AND (
	                              EXISTS (
	                                  SELECT 1
	                                  FROM   [Contract].AppointmentCandidate CA,
	                                         DealerMaster DM
	                                  WHERE  CA.ContractId = A.ContractId
	                                         AND CA.CompanyID = DM.DMA_ID
			...
			AND ((SubCompanyId = @SubCompanyId AND BrandId = @BrandId) 
                        OR (ContractStatus='Draft' AND ISNULL(A.DepId,0)=0)
                   )
	ORDER BY
	       A.CreateDate DESC
END