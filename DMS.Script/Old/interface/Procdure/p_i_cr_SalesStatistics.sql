DROP proc [interface].[p_i_cr_SalesStatistics]
GO


CREATE proc [interface].[p_i_cr_SalesStatistics]
as

declare @StartTime datetime, @EndTime datetime
set @StartTime=DATEADD(mi, 00, DATEADD(hh, 12, DATEADD(WEEK, DATEDIFF(WEEK, 0, GETDATE()) - 1, 0)))
set @EndTime=DATEADD(mi, 00, DATEADD(hh, 12, DATEADD(WEEK, DATEDIFF(WEEK, 0, GETDATE()), 0)))

delete from interface.T_I_CR_SalesStatistics
where StartTime between @StartTime and @EndTime
insert into interface.T_I_CR_SalesStatistics
	select 
		DMA_SAP_Code,
		...
		,[dbo].[fn_GetPurchaseCount](DMA_ID) IsPurchased
