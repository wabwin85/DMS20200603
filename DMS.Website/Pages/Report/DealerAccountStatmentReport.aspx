<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="DealerAccountStatmentReport.aspx.cs" Inherits="DMS.Website.Pages.Report.DealerAccountStatmentReport" %>
    ...
    function CheckCondition()
     {
        ...
        if (Dealer == "")
        {
            alert('<%=GetLocalResourceObject("CheckCondition.Dealer.alert").ToString()%>');
            return false;
        }
        ...
        if (Year == "")
        {
            alert('<%=GetLocalResourceObject("CheckCondition.Year.alert").ToString()%>');
            return false;
        }
        
        var Month = <%=cbMCOP.ClientID%>.getValue();
        if (Month == "")
        {
            alert('<%=GetLocalResourceObject("CheckCondition.Month.alert").ToString()%>');
            return false;
        }
                                    ...
                                    <ext:Panel ID="Panel5" runat="server" Border="false" Header="false">
                                        <Body>
                                            <ext:FormLayout ID="FormLayout4" runat="server" LabelAlign="Left" LabelWidth="80">
                                                <ext:Anchor>
                                                    <ext:ComboBox ID="cbMCOP" runat="server" 
                                                    EmptyText="<%$ Resources: cbMCOP.EmptyText %>" 
                                                    Width="150" 
                                                    Editable="false"
                                                    TypeAhead="true" 
                                                    StoreID="MonthStore" 
                                                    ValueField="COP_Period" 
                                                    DisplayField="COP_Period"
                                                    FieldLabel="<%$ Resources: cbMCOP.FieldLabel %>"                                                    
                                                    >
                                                    <Triggers>
                                                        <ext:FieldTrigger Icon="Clear" Qtip="<%$ Resources: FieldTrigger.Qtip %>" />
                                                    </Triggers>
