DROP PROCEDURE [dbo].[GC_Report_DealerBuyInWeek_Period] 
GO



CREATE PROCEDURE [dbo].[GC_Report_DealerBuyInWeek_Period] 
	@FY		nvarchar(30),  
	@FW		nvarchar(30)
AS
BEGIN
...
DELETE FROM Report_DealerBuyInWeek
WHERE COP_Year = @FY
 and COP_Week = @FW
...
FROM POReceiptHeader 
	...
	inner join COP as COP_W on COP_W.COP_Type='W' and COP_W.COP_Year = @FY and COP_W.COP_Period = @FW and POReceiptHeader.PRH_ReceiptDate >= COP_W.COP_StartDate and POReceiptHeader.PRH_ReceiptDate < DateAdd(day, 1, COP_W.COP_EndDate)
