var InventoryAdjustList_New = {};

InventoryAdjustList_New = function () {
    var that = {};

    var business = 'Inventory.InventoryAdjustList_New';
    var CustomerFaceNbr = [];
    var g = 0;
    ...
    that.InitPage = function (mainData, model) {
        ...
        $("#RstDetailList").data("kendoGrid").hideColumn('ExpiredDate');
        $("#RstDetailList").data("kendoGrid").hideColumn('Delete');

        $("#RstDetailList").data("kendoGrid").columns[6].editable = true;
        $("#RstDetailList").data("kendoGrid").columns[7].editable = true;
        $("#RstDetailList").data("kendoGrid").columns[11].editable = true;
        $("#RstDetailList").data("kendoGrid").columns[13].editable = true;
        ...
        if (model.IsDealer) {
            if (mainData.Status == "Draft") {
                ...
                if ($('#IptAdjustType').FrameDropdownList('getValue').Key == "StockOut") {
                    $("#RstDetailList").data("kendoGrid").columns[13].editable = true;
                    $("#RstDetailList").data("kendoGrid").columns[5].editable = true;
                }
                else {
                    $("#RstDetailList").data("kendoGrid").columns[13].editable = false;//编辑
                    $("#RstDetailList").data("kendoGrid").columns[5].editable = false;
                }
                //如果是其他入库，则序列号、有效期可以做修改
                if (mainData.Reason == "StockIn") {
                    $("#RstDetailList").data("kendoGrid").showColumn('QRCode');//6
                    $("#RstDetailList").data("kendoGrid").showColumn('QRCodeEdit');
                    //$("#RstDetailList").data("kendoGrid").columns[5].editable = false;//编辑
                    $("#RstDetailList").data("kendoGrid").columns[7].editable = false;//编辑
                }
                if (mainData.Reason == "StockOut") {
                    $("#RstDetailList").data("kendoGrid").showColumn('QRCode');//6
                    $("#RstDetailList").data("kendoGrid").showColumn('QRCodeEdit');//13
                }
                //$("#RstDetailList").data("kendoGrid").columns[5].editable = false;//编辑
                ...
                $("#RstDetailList").data("kendoGrid").columns[13].editable = true;
                $("#RstDetailList").data("kendoGrid").showColumn('ExpiredDate');//7
    ...
    var createRstProductDetail = function (datasource) {
        $("#RstDetailList").kendoGrid({
            ...
            {
                field: "UPN", title: "条形码", width: '100px', editable: true, hidden: true,
            ...
            {
                field: "QRCodeEdit", title: "二维码", width: '100px', editable: true,
                headerAttributes: { "class": "text-center text-bold", "title": "二维码" },
                attributes: { "class": "table-td-cell" }
            },
             {
    ...
    that.UpdateItem = function (e) {
        ...
        FrameUtil.SubmitAjax({
            ...
            callback: function (model) {
                if (model.IsSuccess) {
                    if (model.ExecuteMessage.length > 0) {
                        FrameWindow.ShowAlert({
                            target: 'top',
                            alertType: 'info',
                ...
                FrameWindow.ShowConfirm({
                    ...
                    cancelCallback: function () {
                        ...
                        $.each(LstBuArr, function (index, val) {
                            if (dms.common.ToLowerCaseFn($("#hiddenProductLineId").val()) === dms.common.ToLowerCaseFn(val.Key))
                                originalBu = {
    ...
    that.AdjustTypeChange = function (Type) {
        if (Type == 'StockIn') {
            $("#RstDetailList").data("kendoGrid").showColumn('QRCodeEdit');
            $("#RstDetailList").data("kendoGrid").columns[5].editable = false;
            $("#RstDetailList").data("kendoGrid").columns[13].editable = false;
        }
        else {
            $("#RstDetailList").data("kendoGrid").showColumn('QRCode');
            $("#RstDetailList").data("kendoGrid").showColumn('QRCodeEdit');
            $("#RstDetailList").data("kendoGrid").columns[5].editable = true;//禁用
            $("#RstDetailList").data("kendoGrid").columns[13].editable = true;
        }
    ...
    that.CheckForm = function (data) {
        ...
        else if ($.trim(data.IptAdjustDate) == "") {
            message.push('请填调整日期');
        }
        ...
        //表单数量验证
        var msg1 = ""; var msg2 = ""; var msg3 = ""; var msg4 = "";
        for (var i = 0; i < data.RstDetailList.length; i++) {
            ...
            if (record.QRCodeEditlength > 1 && record.QRCodeEdit != ''&& record.QRCodeEdit.toLowerCase()!='noqr') {
    ...
    that.AddUpn = function () {
        ...
        if (data.IptProductLine.Key == "" || data.IptAdjustType.Key == "") {
            ...
            url = Common.AppVirtualPath + 'Revolution/Pages/Inventory/InventoryAdjustListMtrPicker.aspx?InstanceId=' + InstanceId + '&&hiddenDealerId=' + hiddenDealerId + '&&hiddenProductLineId=' + hiddenProductLineId + '&&hiddenAdjustType=' + hiddenAdjustType + '&&hiddenReturnType=' + hiddenReturnType + '&&hiddApplyType=' + hiddApplyType + '&&ChoiceType=0'
