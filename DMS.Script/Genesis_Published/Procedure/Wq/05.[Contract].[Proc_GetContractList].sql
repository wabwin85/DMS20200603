USE [GenesisDMS_Test]
GO
/****** Object:  StoredProcedure [Contract].[Proc_GetContractList]    Script Date: 2019/9/29 13:47:32 ******/
SET ANSI_NULLS ON
GO
...
AS
	...
	IF (@DealerType = 'T2')
		...
		D.DMA_ID,D.DMA_ChineseName,A.ContractNo,A.ContractId,A.ContractType,D.DMA_DealerType,V.DivisionName as SubProductName,V.ProductLineName as DeptName,CONVERT(NVARCHAR(10),MAX(A.CreateDate),112)AS CreateDate,A.CreateUser,B.FileName,B.FileType,
		...
		A.DeptName,A.ContractType,D.DMA_DealerType,ProductLineName,DivisionName
        ...
        UploadFilePath,CASE CHARINDEX('~', SUBSTRING(FileSrcPath,1,1)) WHEN 1 THEN SUBSTRING(FileSrcPath,2,LEN(FileSrcPath)-1) ELSE  FileSrcPath END AS FileSrcPath,IsSinge,FileSrcName ,VersionStatus ,VersionNo,StatusName, IsSinge,SubProductName,DeptName FROM #TMP_T2
		ORDER BY ROWNUMBER;
	...
	ELSE IF (@DealerType = 'LP' OR @DealerType = 'LS')
      BEGIN
		...
		SELECT A.ExportId, 
		D.DMA_ID,D.DMA_ChineseName,A.ContractNo,A.ContractId,A.ContractType,D.DMA_DealerType,SubProductName,A.DeptName,CONVERT(NVARCHAR(10),MAX(A.CreateDate),112)AS CreateDate,A.CreateUser,B.FileName,B.FileType,
		REPLACE(B.UploadFilePath,'\','/') AS UploadFilePath,
		REPLACE(SUBSTRING(B.UploadFilePath,1,LEN(B.UploadFilePath) - CHARINDEX('/',REVERSE(B.UploadFilePath))),'\','//') AS FileSrcPath,REVERSE(SUBSTRING(REVERSE(B.UploadFilePath),1,CHARINDEX('/',REVERSE(B.UploadFilePath))-1)) AS FileSrcName,
		C.VersionStatus,C.VersionNo,E.VALUE1 AS StatusName,CASE ISNULL(F.Id,'') WHEN '' THEN '0' ELSE '1' END IsSinge
		...
		A.DeptName,A.ContractType,D.DMA_DealerType
        ...
        SELECT ROWNUMBER,ExportId,DMA_ID,DMA_ChineseName,ContractNo,ContractId,ContractType,DMA_DealerType,CreateDate,CreateUser,FileName,FileType,
        UploadFilePath,CASE CHARINDEX('~', SUBSTRING(FileSrcPath,1,1)) WHEN 1 THEN SUBSTRING(FileSrcPath,2,LEN(FileSrcPath)-1) ELSE  FileSrcPath END AS FileSrcPath,IsSinge,FileSrcName ,VersionStatus ,VersionNo,StatusName, IsSinge,SubProductName,DeptName 
		FROM #TMP_LP
		 WHERE ( ROWNUMBER BETWEEN ( ( @CurrentPageIndex - 1 ) * @PageSize )
                                    + 1
                            AND     ( @CurrentPageIndex - 1 ) * @PageSize
