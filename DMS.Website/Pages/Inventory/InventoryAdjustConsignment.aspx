<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="InventoryAdjustConsignment.aspx.cs"
    Inherits="DMS.Website.Pages.Inventory.InventoryAdjustConsignment" %>
...
<head>
    ...
    <script language="javascript">
        ...
        var ChangeAdjustType = function() {
            ...
            var grid = Ext.getCmp('GridPanel2');
            var hiddenIsModified = Ext.getCmp('hiddenIsModified');
            if (cbAdjustTypeWin.getValue() == 'StockIn') {
             ...
             grid.getColumnModel().setHidden(11,false);
                ...
                grid.getColumnModel().setEditable(4, false);
            }
        ...
        var CheckSubmit = function() {
            ...
            var hiddenRetrunVal=Ext.getCmp('hiddenRetrunVal');
            var grid = Ext.getCmp('GridPanel2');
            if (cbProductLineWin.getValue() != '' && cbAdjustTypeWin.getValue() != '' && txtAdjustReasonWin.getValue() != '' && grid.store.getCount() > 0) {
                if (CheckLotQty()) {
                    Coolite.AjaxMethods.DoSubmit({
                        success: function() {
                        if(hiddenRetrunVal.getValue()=="true")
                        {
                            Ext.getCmp('hiddenIsModified').setValue('');
                            Ext.getCmp('GridPanel1').store.reload();
                            Ext.getCmp('DetailWindow').hide();
        ...
        var CheckLotQty = function() {
            ...
            for (var i = 0; i < store.getCount(); i++) {
                ...
                if((record.data.QRCodeEdit==''|| record.data.QRCodeEdit == null)&& hiddenAdjustType.getValue()=='StockIn')
                {
                 Ext.Msg.alert('<%=GetLocalResourceObject("CheckLotQty.Alert.Title1").ToString()%>', '其他入库操作必须填写二维码');
                    return false;
                }
                 if((record.data.QRCodeEdit==''|| record.data.QRCodeEdit == null)&& record.data.QRCode=="NoQR" && hiddenAdjustType.getValue()=='StockOut')
                {
                     Ext.Msg.alert('<%=GetLocalResourceObject("CheckLotQty.Alert.Title1").ToString()%>', '其他出库操作二维码不能为NoQR');
                    return false;
                }
...
//                   {
//                   alert(record.data.QRCodeEdit);
//                     Ext.Msg.alert('<%=GetLocalResourceObject("CheckQty.Alert.Title").ToString()%>','二维码'+record.data.QRCodeEdit+"出现多次");
//                    return false;
//                   }
//                   
//                   if(store.query("QRCode",record.data.QRCodeEdit).length>0 &&record.data.QRCode=="NoQR"&&record.data.QRCodeEdit!='' &&  record.data.QRCode!=record.data.QRCodeEdit)
//                   {
...
<body>
                            ...
                            <ext:Button ID="btnInsert" runat="server" Text="<%$ Resources: Panel1.btnInsert.Text %>"
                                ...
                                <AjaxEvents>
                                    <Click Before="#{GridPanel1}.getSelectionModel().clearSelections();" OnEvent="ShowDetails"
                                        ...
                                        Success="#{cbDealerWin}.clearValue();#{cbProductLineWin}.setValue(#{cbProductLineWin}.store.getTotalCount()>0?#{cbProductLineWin}.store.getAt(0).get('Id'):'');#{hiddenProductLineId}.setValue(#{cbProductLineWin}.getValue());#{cbAdjustTypeWin}.clearValue();ClearItems();#{GridPanel2}.clear();#{hiddenIsModified}.setValue('new');">
                                ...
                                <ext:GridPanel ID="GridPanel1" runat="server" Title="<%$ Resources: GridPanel1.Title %>"
                                    ...
                                    <AjaxEvents>
                                        <Command OnEvent="ShowDetails" Failure="Ext.MessageBox.alert(MsgList.ShowDetails.FailureTitle, MsgList.ShowDetails.FailureMsg);"
                                            Success="#{cbDealerWin}.clearValue(); #{cbDealerWin}.setValue(#{hiddenDealerId}.getValue());#{cbProductLineWin}.clearValue(); #{cbProductLineWin}.setValue(#{hiddenProductLineId}.getValue());#{cbAdjustTypeWin}.setValue(#{hiddenAdjustTypeId}.getValue());ClearItems();#{GridPanel2}.reload();#{hiddenIsModified}.setValue('old');#{gpLog}.store.reload();">
                                            <EventMask ShowMask="true" Target="CustomTarget" CustomTarget="={#{GridPanel1}.body}"
    ...
    <ext:Hidden ID="hiddenRetrunVal" runat="server" />
     <ext:Hidden ID="hiddenReturnMessing" runat="server" />
                                                    ...
                                                    <ext:FormLayout ID="FormLayout4" runat="server" LabelAlign="Left" LabelWidth="80">
                                                        <ext:Anchor>
                                                            <ext:ComboBox ID="cbDealerWin" runat="server" Width="200" Editable="true" TypeAhead="true"
                                                                ...
                                                                <Listeners>
                                                                    <Select Handler="ChangeDealer();if (#{hiddenDealerId}.getValue() != #{cbDealerWin}.getValue()){Coolite.AjaxMethods.DeleteDetail( {
                                                                    success: function() {
                                                    ...
                                                    <ext:FormLayout ID="FormLayout5" runat="server" LabelAlign="Left" LabelWidth="80">
                                                        ...
                                                        <ext:Anchor>
                                                            <ext:ComboBox ID="cbAdjustTypeWin" runat="server" Width="150" Editable="false" TypeAhead="true"
                                                                StoreID="AdjustTypeStore" ValueField="Key" DisplayField="Value" FieldLabel="<%$ Resources: DetailWindow.FormLayout5.cbAdjustTypeWin.FieldLabel %>"
                                                    ...
                                                    <ext:FormLayout ID="FormLayout8" runat="server" LabelAlign="Left" LabelWidth="80">
                                                        <ext:Anchor>
                                                            <ext:TextArea ID="txtAdjustReasonWin" runat="server" FieldLabel="<%$ Resources: DetailWindow.FormLayout8.txtAdjustReasonWin.FieldLabel %>"
                                                                Width="300" SelectOnFocus="true" Height="50" />
                                                        </ext:Anchor>
                                                    </ext:FormLayout>
                                                </Body>
                                            </ext:Panel>
                                ...
                                <Body>
                                    <ext:FitLayout ID="FitLayout2" runat="server">
                                        <ext:GridPanel ID="GridPanel2" runat="server" Title="<%$ Resources: GridPanel2.Title %>"
                                            ...
                                            Header="false" AutoExpandColumn="WarehouseName">
                                            <TopBar>
...
--%>
                                            <Listeners>
                                                ...
                                                <BeforeEdit Handler="#{hiddenCurrentEdit}.setValue(this.getSelectionModel().getSelected().id);
                                                            ...
                                                            #{txtQrCode}.setValue(this.getSelectionModel().getSelected().data.QRCodeEdit);
                                                              ...
                                                              if(this.getSelectionModel().getSelected().data.QRCode!='NoQR'&& #{hiddenAdjustTypeId}.getValue()!='StockIn')
                                                            {
                                        ...
                                        " />
                                                <AfterEdit Handler="Coolite.AjaxMethods.SaveItem(#{txtCFNEdit}.getValue(),#{txtLotNumber}.getValue(),#{dtExpiredDate}.getValue(),#{txtAdjustQty}.getValue(), #{txtQrCode}.getValue(),{success:function(){#{hiddenIsEditting}.setValue('');},failure: function(err) {Ext.Msg.alert(MsgList.Error, err);}});" />
                                            </Listeners>
                                        </ext:GridPanel>
                                    </ext:FitLayout>
                                </Body>
                            </ext:Tab>
                            <ext:Tab ID="TabLog" runat="server" Title="<%$ Resources: TabLog.Title %>">
                                <Body>
                                    <ext:FitLayout ID="FitLayout3" runat="server">
                                        <ext:GridPanel ID="gpLog" runat="server" Title="<%$ Resources: gpLog.Title %>" StoreID="OrderLogStore"
                                            Border="false" Icon="Lorry" StripeRows="true" EnableHdMenu="false" Header="false"
                                            AutoExpandColumn="OperNote">
