DROP PROCEDURE [Promotion].[Proc_Pro_Cal_PrdSalesQuantity]
GO
...
/**********************************************
	...
	1.创建 2015-08-31
**********************************************/
CREATE PROCEDURE [Promotion].[Proc_Pro_Cal_PrdSalesQuantity]
...
BEGIN
	...
	BEGIN
		...
		--取得指定产品的医院植入量 
		IF @CalType = 'ByDealer' --如果当前政策是ByDealer，那么本因素应该会有关联的条件因素“产品”,也许有“医院”
		BEGIN
			SET @SQL = N'INSERT INTO #TMP_SALES(Dealer,SalesAmount) 
				SELECT DealerId,SUM(A.QTY) FROM interface.V_I_QV_InHospitalSales_WithQR A
				WHERE Transaction_Date >='''+@BeginDate+''' AND Transaction_Date < '''+@EndDate+''' AND Transaction_Date >='''+@FactBeginDate+''' AND Transaction_Date < '''+@FactEndDate+'''  '
				...
				+' AND A.UPN IN (SELECT UPN FROM #TMP_UPN) GROUP BY A.DealerId'
				PRINT @SQL
				EXEC(@SQL)
		END
		...
		BEGIN 
			SET @SQL = N'INSERT INTO #TMP_SALES(Dealer,HospitalId,SalesAmount) 
				SELECT DealerId,DMScode,SUM(A.QTY) QTY FROM interface.V_I_QV_InHospitalSales_WithQR A 
				WHERE Transaction_Date >='''+@BeginDate+''' AND Transaction_Date < '''+@EndDate+''' 
				AND Transaction_Date >='''+@FactBeginDate+''' AND Transaction_Date < '''+@FactEndDate+''' 
				AND A.DMScode IN (SELECT HospitalId FROM '+@TableName+') 
				AND A.UPN IN (SELECT UPN FROM #TMP_UPN) 
				GROUP BY A.DealerId,A.DMScode'
				PRINT @SQL
				EXEC(@SQL)
		END
		...
		BEGIN
			...
			BEGIN
				SET @SQL = N'INSERT INTO #TMP_SALES_TMP(Dealer,SalesAmount) 
					SELECT DealerId,FLOOR(CONVERT(DECIMAL(18,4),SUM(A.QTY))/CONVERT(DECIMAL(18,4),'+CONVERT(NVARCHAR,@Qty)+'))  
					FROM interface.V_I_QV_InHospitalSales_WithQR A 
					WHERE Transaction_Date >='''+@BeginDate+''' AND Transaction_Date < '''+@EndDate+'''  AND Transaction_Date >='''+@FactBeginDate+''' AND Transaction_Date < '''+@FactEndDate+''' '
					+CASE WHEN @iCOUNT_INCLUDE_HOSPITAL > 0 THEN ' AND A.DMScode IN (SELECT HospitalId FROM #TMP_HOSPITAL WHERE OperTag = ''INCLUDE'')' ELSE '' END
					+CASE WHEN @iCOUNT_EXCLUDE_HOSPITAL > 0 THEN ' AND A.DMScode NOT IN (SELECT HospitalId FROM #TMP_HOSPITAL WHERE OperTag = ''EXCLUDE'')' ELSE '' END
					+' AND A.UPN IN (SELECT UPN FROM #TMP_UPN) GROUP BY A.DealerId'
			...
			BEGIN 
				SET @SQL = N'INSERT INTO #TMP_SALES_TMP(Dealer,HospitalId,SalesAmount) 
					SELECT DealerId,DMScode,FLOOR(CONVERT(DECIMAL(18,4),SUM(A.QTY))/CONVERT(DECIMAL(18,4),'+CONVERT(NVARCHAR,@Qty)+')) 
					FROM interface.V_I_QV_InHospitalSales_WithQR A
					WHERE Transaction_Date >='''+@BeginDate+''' AND Transaction_Date < '''+@EndDate+''' 
					AND Transaction_Date >='''+@FactBeginDate+''' AND Transaction_Date < '''+@FactEndDate+''' 
					AND A.DMScode IN (SELECT HospitalId FROM '+@TableName+')
			...
			BEGIN
				...
				BEGIN
					...
					DELETE A
					FROM #TMP_SALES A WHERE NOT EXISTS (SELECT 1 FROM #TMP_SALES_TMP B WHERE Dealer = A.Dealer AND A.HospitalId = B.HospitalId AND SalesAmount >0)
				END
			END
