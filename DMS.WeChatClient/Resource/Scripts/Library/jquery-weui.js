...
+ function ($) {
    ...
    $.prompt = function (text, title, onOK, onCancel, input) {
        ...
        var modal = $.modal({
            text: '<p class="weui-prompt-text">' + (config.text || '') + '</p><textarea rows="3" class="weui_input weui-prompt-input" id="weui-prompt-input" value="' + (config.input || '') + '" />',
            title: config.title,
            autoClose: false,
            buttons: [
            {
            ...
            },
            {
                ...
                onClick: function () {
                    ...
                    if (!config.empty && (input === "" || input === null)) {
                        ...
                        $.toptip("请输入" + config.title, 'warning');
                        return false;
                    }
                    $.closeModal();
            ...
            this.find('.weui-prompt-input').trigger("click");
            this.find('.weui-prompt-input').trigger("focus");
        });
...
+ function ($) {
    ...
    var Picker = function (params) {
        ...
        var defaults = {
          ...
          <a href="javascript:;" class="picker-button clean-picker" style="left:0!important;text-align:left!important;">{{cleanText}}</a>\
        ...
        p.setValue = function (arrValues, transition) {
            ...
            for (var i = 0; i < p.cols.length; i++) {
                if (p.cols[i] && !p.cols[i].divider) {
                    //2016.09.09 By XuXing :修改无法加载自定义明细的bug
                    var intValue = parseInt(arrValues[valueIndex]);
                    if (p.cols[i].values.indexOf(arrValues[valueIndex]) > 0 || (!isNaN(parseInt(intValue)) && p.cols[i].values.indexOf(intValue) > 0)) {
                        p.cols[i].setValue(arrValues[valueIndex], transition);
                    } else {
                        p.cols[i].setValue(p.cols[i].values[0], transition);
                    }
                    valueIndex++;
                }
            }
        ...
        p.layout = function () {
            ...
            pickerHTML =
                '<div class="' + (pickerClass) + '">' +
                    (p.params.toolbar ? p.params.toolbarTemplate.replace(/{{closeText}}/g, p.params.toolbarCloseText).replace(/{{title}}/g, p.params.title).replace(/{{cleanText}}/g, p.params.toolbarCleanText) : '') +
        ...
        function openOnInput(e) {
            ...
            p.open();
            p.updateValue();
            //2016.09.22 XuXing 添加清空选项
            if (p.params.toolbarCleanText) {
                $(".clean-picker").unbind("click").bind("click", function () {
                    p.cleanValue();
                    p.close();
                });
            }
...
+ function ($) {
    ...
    Select.prototype.parseInitValue = function () {
        ...
        var dataValues = this.$input.attr('data-values');

        //如果input为空，只有在第一次初始化的时候才保留默认选择。因为后来就是用户自己取消了全部选择，不能再为他选中默认值。
        if (!this._init && (value === undefined || value == null || value === "")) return;

        var titles = this.config.multi ? value.split(this.config.split) : [value];
        var vals = this.config.multi ? dataValues.split(this.config.split) : [dataValues];

        // 建议以值来判断哪些要处于选中状态，以文本来判断有太多不确定因素，如值多了/少个空格
        if (dataValues&&(vals || vals.length > 0)) {
            for (var i = 0; i < items.length; i++) {
                items[i].checked = false;
                for (var j = 0; j < vals.length; j++) {
                    if (items[i].value.toLowerCase() === vals[j].toLowerCase()) {
...
+ function ($) {
    ...
    Datetime.prototype = {
        ...
        getConfig: function () {
            var today = common.ConvertToDateTime(this.params.defaultValue ? this.params.defaultValue : new Date()),
                params = this.params,
                self = this,
                lastValidValues;
            ...
            var config = {
                ...
                onClean: function (picker) {
                    if (self.params.onClean) {
                        self.params.onClean.apply(this, arguments);
                    }
                },
