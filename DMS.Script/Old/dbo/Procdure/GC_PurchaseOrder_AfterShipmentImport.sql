DROP PROCEDURE [dbo].[GC_PurchaseOrder_AfterShipmentImport]
GO


CREATE PROCEDURE [dbo].[GC_PurchaseOrder_AfterShipmentImport]
	@ShipmentNbr nvarchar(max),
...
create table #tmp_PurchaseOrderDetail (
   ...
   primary key (POD_ID)
	...
	--将单据号转成列
	select val as ShipmentNbr into #tmp_no from dbo.GC_Fn_SplitStringToTable(@ShipmentNbr,'#')

	SELECT @VENDORID = DMA_Parent_DMA_ID from DealerMaster,ShipmentHeader
	where DMA_ID = SPH_Dealer_DMA_ID
	and SPH_ShipmentNbr = (select top 1 ShipmentNbr from #tmp_no)
	...
	select NEWID(),SPH_ProductLine_BUM_ID,SPH_Dealer_DMA_ID,@VENDORID,@ShipmentType,'Manual',SPH_Shipment_User,
		GETDATE(),'Submitted',0,0,NULL,GETDATE(),SPH_Shipment_User,ShipmentNbr
	from (
	select distinct SPH_ProductLine_BUM_ID,SPH_Dealer_DMA_ID,
	SPH_Shipment_User,ShipmentNbr=Replace(STUFF(( SELECT ';' + ta.ShipmentNbr
          FROM (select SPH_ProductLine_BUM_ID,ShipmentNbr 
					from ShipmentHeader,#tmp_no where sph_ShipmentNbr = ShipmentNbr ) ta 
					where SPH.SPH_ProductLine_BUM_ID = TA.SPH_ProductLine_BUM_ID					
            FOR XML PATH('')), 1, 1, ''),';',';')
	from ShipmentHeader SPH,ShipmentLine
	where SPH_ID = SPL_SPH_ID
	and sph_ShipmentNbr IN (select ShipmentNbr from #tmp_no)
	) tab

	update t1
	set t1.POH_WHM_ID = t2.SLT_WHM_ID
	--select * 
	from #tmp_PurchaseOrderHeader T1,
	(SELECT distinct SPH_ProductLine_BUM_ID,SLT_WHM_ID FROM ShipmentHeader,ShipmentLine,ShipmentLot
		where SPH_ID = SPL_SPH_ID
		and SPL_ID = SLT_SPL_ID
		and sph_ShipmentNbr IN (select ShipmentNbr from #tmp_no)) t2
	where t1.POH_ProductLine_BUM_ID = t2.SPH_ProductLine_BUM_ID		
    ...
    inner join ShipmentHeader ON (POH_DMA_ID = SPH_Dealer_DMA_ID and POH_ProductLine_BUM_ID = SPH_ProductLine_BUM_ID)
    ...
    LEFT join MD.V_INF_UPN_REG AS REG ON (cfn.CFN_CustomerFaceNbr = REG.CurUPN)
	  where SPH_ShipmentNbr in (select ShipmentNbr from #tmp_no)
	  ...
	  and t3.CFNP_PriceType = 'DealerConsignment'
		...
		--insert into PurchaseOrderDetail select * from #tmp_PurchaseOrderDetail
     ...
     CASE WHEN charindex('@@',POD_LotNumber) > 0
       ...
       from #tmp_PurchaseOrderDetail
		...
		select NEWID(),'',POH_ProductLine_BUM_ID,POH_DMA_ID,POH_VendorID,
		   POH_TerritoryCode,POH_RDD,POH_ContactPerson,POH_Contact,
		   POH_ContactMobile,POH_Consignee,POH_ShipToAddress,
		   POH_ConsigneePhone,POH_Remark ,POH_InvoiceComment,
		   POH_CreateType,POH_CreateUser,POH_CreateDate,
		   POH_UpdateUser,POH_UpdateDate,null,null,
		   POH_LastBrowseUser,POH_LastBrowseDate,'Draft',
		   POH_LatestAuditDate,POH_IsLocked,POH_SAP_OrderNo,
		   POH_SAP_ConfirmDate,POH_LastVersion,'Consignment',
		   POH_VirtualDC,POH_SpecialPriceID,POH_WHM_ID ,POH_POH_ID ,null
		from #tmp_PurchaseOrderHeader
		...
		insert into PurchaseOrderHeader( POH_ID,
		   ...
		   POH_POH_ID 
		from #tmp_PurchaseOrderHeader
		where POH_OrderType='Consignment'
		
		insert into #tmp_PurchaseOrderDetail 
		(POD_ID,POD_POH_ID,POD_CFN_ID,POD_CFN_Price,POD_RequiredQty,POD_Amount,
				POD_Tax,POD_ReceiptQty,POD_UOM,POD_LotNumber,
        POD_CurRegNo,POD_CurValidDateFrom,POD_CurValidDataTo,POD_CurManuName,POD_LastRegNo,POD_LastValidDateFrom,POD_LastValidDataTo,
        POD_LastManuName,POD_CurGMKind,POD_CurGMCatalog)
	  select newid(),POH_ID,CFN_ID,0,sum(cast(round(SLT_LotShippedQty,2) as numeric(10))),0,0,0 ,CFN_Property3,NULL,
           REG.CurRegNo,REG.CurValidDateFrom,REG.CurValidDataTo,REG.CurManuName,REG.LastRegNo,REG.LastValidDateFrom,
           REG.LastValidDataTo,REG.LastManuName,REG.CurGMKind,REG.CurGMCatalog
	  from #tmp_PurchaseOrderHeader 
         inner join ShipmentHeader ON (POH_DMA_ID = SPH_Dealer_DMA_ID and POH_ProductLine_BUM_ID = SPH_ProductLine_BUM_ID)
         ...
         LEFT join MD.V_INF_UPN_REG AS REG ON (CFN.CFN_CustomerFaceNbr = REG.CurUPN)
	  where SPH_ShipmentNbr in (select ShipmentNbr from #tmp_no)
	    and POH_OrderType='Consignment'
	  group by POH_ID,CFN_ID,CFN_Property3,
             REG.CurRegNo,REG.CurValidDateFrom,REG.CurValidDataTo,REG.CurManuName,REG.LastRegNo,REG.LastValidDateFrom,
             REG.LastValidDataTo,REG.LastManuName,REG.CurGMKind,REG.CurGMCatalog
	  
	  delete from #tmp_PurchaseOrderDetail where POD_RequiredQty = 0
	  
	  delete from #tmp_PurchaseOrderHeader where POH_ID not in (select POD_POH_ID from #tmp_PurchaseOrderDetail ) and POH_OrderType='Consignment'
     ...
     from #tmp_PurchaseOrderDetail
	  where POD_POH_ID in (select POH_ID from #tmp_PurchaseOrderHeader where POH_OrderType='Consignment')
	  ...
	  --UPDATE BY KAICHUN.HUA 20170207
		...
		BEGIN
			...
			where POD_POH_ID in (select POH_ID from #tmp_PurchaseOrderHeader where POH_OrderType='ClearBorrowManual')
	  ...
	  --插入订单操作日志
		INSERT INTO PurchaseOrderLog (POL_ID,POL_POH_ID,POL_OperUser,POL_OperDate,POL_OperType,POL_OperNote)
		SELECT NEWID(),POH_ID,POH_CreateUser,GETDATE(),'Generate',NULL FROM #tmp_PurchaseOrderHeader where POH_OrderType='Consignment'
