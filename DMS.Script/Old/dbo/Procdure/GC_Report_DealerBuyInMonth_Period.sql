DROP PROCEDURE [dbo].[GC_Report_DealerBuyInMonth_Period] 
GO


/*
	@StartDate 是财月的开始日期，格式：yyyy-mm-dd
	@EndDate 是财月的结束日期，格式：yyyy-mm-dd
*/


CREATE PROCEDURE [dbo].[GC_Report_DealerBuyInMonth_Period] 
	@FY		nvarchar(30),  
	@FM		nvarchar(30)
AS
BEGIN
...
DELETE FROM Report_DealerBuyInMonth
WHERE COP_Year = @FY
 and COP_Month = @FM
...
FROM POReceiptHeader 
	...
	inner join COP on COP.COP_Type='M' and COP.COP_Year = @FY and COP.COP_Period = @FM and POReceiptHeader.PRH_ReceiptDate >= COP.COP_StartDate and POReceiptHeader.PRH_ReceiptDate < DateAdd(day, 1, COP.COP_EndDate)
