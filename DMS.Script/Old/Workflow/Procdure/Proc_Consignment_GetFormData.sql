...
DROP PROCEDURE [Workflow].[Proc_Consignment_GetFormData]
GO

CREATE PROCEDURE [Workflow].[Proc_Consignment_GetFormData]
...
IF @ApplyType = 'ConsignmentApply'
	BEGIN
		...
		SELECT CAH_OrderNo AS RequestNo,ISNULL(@TotalRmb,0) AS TotalRMB,CAST((ISNULL(@TotalRmb,0) / ISNULL(@UsdRate,1)) AS decimal(18,2)) AS TotalUSD,'1' AS Type FROM dbo.ConsignmentApplyHeader 
...
ELSE IF @ApplyType = 'DelayApply'
	BEGIN
		...
		SELECT CAH_OrderNo AS RequestNo,ISNULL(@TotalRmb,0) AS TotalRMB,CAST((ISNULL(@TotalRmb,0) / ISNULL(@UsdRate,1)) AS decimal(18,2)) AS TotalUSD,'2' AS Type FROM dbo.ConsignmentApplyHeader 
		WHERE LOWER(CAST(CAH_CAH_ID AS NVARCHAR(100))) = LOWER(@InstanceId)
