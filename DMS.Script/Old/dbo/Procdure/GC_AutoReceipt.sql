DROP Procedure [dbo].[GC_AutoReceipt]
GO
...
T1/T2自动收货
*/
CREATE Procedure [dbo].[GC_AutoReceipt]
...
BEGIN TRAN
	...
	--将超过五天未收货的收货单添加到临时表待处理
	SELECT PH.PRH_ID INTO #TMP_DATA FROM POReceiptHeader PH
	INNER JOIN DealerMaster D ON D.DMA_ID = PH.PRH_Dealer_DMA_ID
	WHERE DATEDIFF(DAY,PRH_SAPShipmentDate,GETDATE()) >= 5
	AND PRH_Type = 'PurchaseOrder' AND PRH_Status = 'Waiting'
	AND D.DMA_DealerType IN ('T1','T2')

	--更新收货单的状态Complete
	UPDATE POReceiptHeader SET PRH_Status = 'Complete'
	FROM #TMP_DATA T WHERE POReceiptHeader.PRH_ID = T.PRH_ID

	--记录单据操作日志
	INSERT INTO PurchaseOrderLog (POL_ID,POL_POH_ID,POL_OperUser,POL_OperDate,POL_OperType,POL_OperNote)
	SELECT NEWID(),PRH_ID,@SysUserId,GETDATE(),'Confirm',NULL
	FROM #TMP_DATA
	...
	SELECT PL.POR_ReceiptQty,NEWID(),PL.POR_ID,'采购入库',PH.PRH_WHM_ID,PL.POR_SAP_PMA_ID,ISNULL(PL.POR_UnitPrice,0),
	'接收'+PH.PRH_PONumber+'的第'+CONVERT(NVARCHAR(50),PL.POR_LineNbr)+'行'
  	FROM POReceipt PL
	INNER JOIN POReceiptHeader PH ON PH.PRH_ID = PL.POR_PRH_ID
	INNER JOIN #TMP_DATA T ON T.PRH_ID = PH.PRH_ID
