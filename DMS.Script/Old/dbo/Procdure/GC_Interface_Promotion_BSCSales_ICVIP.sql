DROP proc [dbo].[GC_Interface_Promotion_BSCSales_ICVIP]
GO
...
--exec [dbo].[GC_Interface_Promotion_BSCSales_ICVIP] 'IC-PRO-2015-0002','2014-01-01','2015-05-01'
CREATE proc [dbo].[GC_Interface_Promotion_BSCSales_ICVIP](@PRCode nvarchar(200),
...
-------Calculate Comercial Sales-------------
	 ...
	 from (
		select
			 ...
			 ,NBR
			 ,CreateTime=GETDATE()
			 ,AchIndicator= dbo.GC_Fn_GetIndicator(IsAchIndicator,bs.SAPID,ComputeCycle,t.Division,@FinishDate,MarketType) 
			 ,IsTiersRule
			from [interface].[V_I_EW_BSCSales] bs	  
			join interface.T_I_EW_PromotionRules t on  t.Division=bs.Division  and t.PRCode=@PRCode
			join interface.T_I_QV_VIPDealers d on d.SAPID=bs.SAPID and d.EndDate>=GETDATE()
	  ...
	  from (
			select
			 ...
			 ,NBR
			 ,CreateTime=GETDATE()
			,AchIndicator= dbo.GC_Fn_GetIndicator(IsAchIndicator,bs.SAPID,ComputeCycle,t.Division,@FinishDate,t.MarketType) 
			 ,IsTiersRule
			...
			from interface.T_I_QV_LPSales(nolock) bs	 
			    ...
			    join interface.T_I_QV_VIPDealers d on d.SAPID=bs.SAPID and d.EndDate>=GETDATE()
				join interface.T_I_EW_PromotionProduct p on p.PRCode=@PRCode and p.UPN=bs.UPN
				join DealerMaster dm on dm.DMA_SAP_Code=d.SAPID
