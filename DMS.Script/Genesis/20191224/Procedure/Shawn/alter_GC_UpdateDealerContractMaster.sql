...
维护合同主数据
*/
ALTER Procedure [dbo].[GC_UpdateDealerContractMaster]
...
BEGIN TRAN
	...
	BEGIN
			...
			BEGIN
				...
				SELECT @CC_ID,NEWID(),CAP_DMA_ID,(SELECT DivisionCode FROM dbo.T_DivisionProductLineRelation WHERE DivisionName = ContractAppointment.CAP_Division ),
	...
	BEGIN
		...
		IF @Check_DCM >0
			BEGIN
				...
				SET @AmendmentSQLUpdate =@AmendmentSQLUpdate+ ' AND  DCM_Division IN (SELECT DivisionCode FROM dbo.T_DivisionProductLineRelation WHERE DivisionName = '''+@Division+''' )'
