...
{
    public class DealerTransferInService : ABaseQueryService,IQueryExport
    {
        #region Ajax Method
        ReportDao rptDao = new ReportDao();
        IRoleModelContext context = RoleModelContext.Current;
        public DealerTransferInVO Init(DealerTransferInVO model)
        {
            try
            {
        ...
        public DealerTransferInVO ChangeBrand(DealerTransferInVO model)
        {
            try
            {
        ...
        public string Query(DealerTransferInVO model)
        {
            try
            {
                ...
                DataSet ds = rptDao.DealerTransferInReport(param, start, model.PageSize, out totalCount); 
        ...
        {
            if (Parameters["ExportType"].ToSafeString() == "ExportTransferIn")
            {
                Hashtable param = new Hashtable();
                ...
                param.Add("SubCompanyId", BaseService.CurrentSubCompany?.Key);
                int totalCount = 0;
                DataSet ds = rptDao.DealerTransferInReport(param, 0, int.MaxValue, out totalCount);
                //删除多余列               
                ...
                ds.Tables[0].Columns["ToDMA_ChineseName"].ColumnName = "经销商名称";
                ds.Tables[0].Columns["ToDMA_EnglishName"].ColumnName = "经销商英文名称";
                ds.Tables[0].Columns["ToDMA_SAP_Code"].ColumnName = "ERPCode";
                ds.Tables[0].Columns["ProductLineName"].ColumnName = "产品线";
                ds.Tables[0].Columns["PRH_PONumber"].ColumnName = "借货入库单号";
                ds.Tables[0].Columns["PRH_SAPShipmentID"].ColumnName = "对应的借货出库单号";
                ds.Tables[0].Columns["PRH_ReceiptDate"].ColumnName = "接收日期";
                ds.Tables[0].Columns["FromDMA_ChineseName"].ColumnName = "借出经销商名称";
                ds.Tables[0].Columns["FromDMA_EnglishName"].ColumnName = "借出经销商英文名称";
                ds.Tables[0].Columns["FromDMA_SAP_Code"].ColumnName = "借出经销商ERPCode";
                ds.Tables[0].Columns["WHM_Name"].ColumnName = "接收库";
                ...
                ds.Tables[0].Columns["PRL_ReceiptQty"].ColumnName = "借入数量";
