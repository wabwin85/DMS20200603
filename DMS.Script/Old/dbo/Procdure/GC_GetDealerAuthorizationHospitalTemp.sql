DROP Procedure [dbo].[GC_GetDealerAuthorizationHospitalTemp]
GO


CREATE Procedure [dbo].[GC_GetDealerAuthorizationHospitalTemp]
...
BEGIN TRAN
	...
	SELECT  DISTINCT B.HOS_ID FROM DealerAuthorizationTableTemp A(nolock) INNER JOIN ContractTerritory B(nolock) ON A.DAT_ID=B.Contract_ID
	...
	BEGIN
		...
		FROM V_DealerAuthorizationBR WHERE CA_ID IN ( SELECT distinct CA_ID FROM interface.ClassificationAuthorization(nolock)) and CC_Code =@PartsContractCode;
	  ...
	  FROM    (SELECT DISTINCT dat.DAT_DMA_ID,
				 ...
				 FROM Hospital hos(nolock)
					  INNER JOIN dbo.ContractTerritory cont(nolock)
						 ON hos.HOS_ID = cont.HOS_ID
					  INNER JOIN dbo.DealerAuthorizationTableTemp dat(nolock)
						 ON dat.DAT_ID = cont.Contract_ID
					  INNER JOIN (select distinct CA_ID,CA_Code from interface.ClassificationAuthorization(nolock)) ca on ca.CA_ID=dat.DAT_PMA_ID
					  LEFT JOIN Lafite_DICT dst(nolock)
					  ...
					  [DealerName] =
						 STUFF (
							(SELECT ',' + tt2.[DMA_ChineseName]
							   FROM #TerritoryTemp tt1, DealerMaster tt2(nolock)
	...
	BEGIN
		...
		FROM V_DealerAuthorizationNoBR WHERE CA_ID IN (SELECT distinct CA_ID FROM interface.ClassificationAuthorization(nolock)) and CC_Code =@PartsContractCode;
...
SELECT NEWID(),HOS_ID,
HOS_HospitalShortName,HOS_HospitalName,HOS_Grade,HOS_Key_Account,HOS_Province,HOS_City,HOS_District,isnull(HOS_Depart,''),isnull(HOS_DepartType,''),isnull(VALUE1,''),isnull(HOS_DepartRemark,''),Territory_Type, HospitalCount, TCount, RepeatDealer
,row_number () OVER (ORDER BY HOS_ID ASC)
FROM (SELECT DISTINCT 
C.HOS_ID,C.HOS_HospitalShortName,HOS_HospitalName,HOS_Grade,HOS_Key_Account,HOS_Province,HOS_City,HOS_District,HOS_Depart,HOS_DepartType,dst.VALUE1,HOS_DepartRemark,B.Territory_Type,@HospitalCount as HospitalCount,0 AS TCount,'' AS RepeatDealer 
...
INNER JOIN (select distinct CA_ID,CA_Code from interface.ClassificationAuthorization) ca on ca.CA_ID=A.DAT_PMA_ID
LEFT JOIN Lafite_DICT dst 
ON dst.DICT_TYPE = 'HospitalDepart' AND CONVERT (NVARCHAR (50),A.DAT_ProductLine_BUM_ID) = dst.PID AND B.HOS_DepartType = dst.DICT_KEY
WHERE A.DAT_DCL_ID=@ContractId
) TAB 
