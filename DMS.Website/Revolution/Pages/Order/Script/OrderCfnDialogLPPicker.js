...
var OrderCfnDialogLPPicker = {};

OrderCfnDialogLPPicker = function () {
    var that = {};

    var business = 'Order.OrderCfnDialogLPPicker';
    var pickedList = [];
    ...
    $("#hidSpecialPriceId").val(Common.GetUrlParam('spid'));
    $("#hidOrderTypeId").val(Common.GetUrlParam('otype'));
    ...
    that.CreateResultList = function () {
        ...
        $("#RstResultList").kendoGrid({
            ...
            columns: [
                ...
                {
                    field: "EnglishName", title: "产品英文名称", width: '80px', editable: true,
                    headerAttributes: { "class": "text-center text-bold", "title": "产品英文名" },
                    attributes: { "class": "table-td-cell" }
                },
                {
                    field: "ChineseName", title: "产品中文名称", width: '80px', editable: true,
                    headerAttributes: { "class": "text-center text-bold", "title": "产品中文名" },
                    attributes: { "class": "table-td-cell" }
                },
                {
                    field: "Price", title: "产品单价", width: '80px', editable: true, template: function (gridrow) { if (gridrow.Price == null || "" == gridrow.Price || undefined == gridrow.Price) gridrow.Price = 0; return gridrow.Price },
                    headerAttributes: { "class": "text-center text-bold", "title": "产品单价" },
                    attributes: { "class": "table-td-cell" }
                },
                {
                    field: "UOM", title: "包装单位", width: '50px', editable: true,
                    headerAttributes: { "class": "text-center text-bold", "title": "包装单位" },
                    attributes: { "class": "table-td-cell" }
                },
                {
                    field: "Sheet", title: "单位数量", width: '50px', editable: true,
                    headerAttributes: { "class": "text-center text-bold", "title": "单位数量" },
                    attributes: { "class": "table-td-cell" }
                },
                {
                    field: "Avaiable", title: "可订数量", width: '80px', editable: true,
                ...
                {
                    field: "ProductMsg", title: "产品状态详细信息", width: '120px', editable: true,
                    headerAttributes: { "class": "text-center text-bold", "title": "产品状态详细信息" },
                    attributes: { "class": "table-td-cell" }
                },
                {
                    field: "ShipmentQtyDialog", title: "订购数量(个)", width: '120px', editable: false, format: "{0:N0}", template: function (gridrow) { if (gridrow.ShipmentQtyDialog == null || "" == gridrow.ShipmentQtyDialog || undefined == gridrow.ShipmentQtyDialog) gridrow.ShipmentQtyDialog = gridrow.PackageFactor; return gridrow.ShipmentQtyDialog },
                    headerAttributes: { "class": "text-center text-bold", "title": "订购数量(个)" },
                    attributes: { "class": "table-td-cell" }
                },
                {
                ...
                $("#RstResultList").find(".Check-Item").on("click", function () {
                    ...
                    if (checked) {
                        if (dataItem.ShipmentQtyDialog % dataItem.PackageFactor != 0) {
                            ...
                            //$(row).find("input[name=ShipmentQtyDialog]").val(data.PackageFactor);
                            //dataItem.ShipmentQtyDialog = data.PackageFactor;
    ...
    that.BtnCfnInfo = function () {
        var model = FrameUtil.GetModel();
        top.createTab({
            id: 'M2_OrderApply_OACfnLPPicker',
            title: '产品不可订购信息查询',
