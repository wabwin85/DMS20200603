DROP PROCEDURE [dbo].[Proc_ReturnBusinessSample_EKP]
GO



CREATE PROCEDURE [dbo].[Proc_ReturnBusinessSample_EKP](
     @UserId         NVARCHAR(500),
...
BEGIN
   ...
   SELECT @SampleType = SampleType FROM SampleApplyHead WHERE ApplyNo=@ApplyNo
		...
		VALUES
		  (@ReturnHeadId, @SampleType, @ReturnNo, @ApplyNo, @DealerId, @ReturnDate, @ReturnUserId, @ReturnUser, @ReturnUserId, @ReturnUser, @ReturnHosp, @DealerName, @ReturnMemo, 'New', @UserId, GETDATE())
    ...
    SET @SyncContent += '<SAMPLETYPE><![CDATA[1]]></SAMPLETYPE>';
		...
		VALUES
		  (@ReturnHeadId, '退货单', @SampleType, '单据创建', @ReturnNo, @SyncContent, 'Wait', 0, '', GETDATE())
