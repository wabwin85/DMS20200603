USE [GenesisDMS_PRD]
GO
/****** Object:  StoredProcedure [dbo].[GC_DealerPriceImportInit]    Script Date: 2019/12/19 14:19:55 ******/
SET ANSI_NULLS ON
GO
...
/**********************************************
	...
	1.创建 2019-10-18
**********************************************/
ALTER PROCEDURE [dbo].[GC_DealerPriceImportInit] 
	@UserId NVARCHAR(36),
...
BEGIN
	...
	BEGIN
		...
		,CFNP_DeletedFlag,CFNP_SubCompanyId,CFNP_BrandId,CFNP_Province,CFNP_City,CFNP_PriceType,CFNP_DealerType)
		...
		@UserId,GETDATE(),0,@SubCompanyId,@BrandId,CFNPI_Province_ID,CFNPI_City_ID,CFNPI_LevelKey,CFNPI_DealerType
		FROM CFNPriceImportInit
		WHERE CreateUser=@UserId
		AND NOT EXISTS(
			SELECT 1 FROM CFNPrice WHERE CFNP_CFN_ID=CFNPI_CFN_ID AND CFNP_Group_ID=CFNPI_Group_ID 
			AND CFNP_PriceType = CFNPI_LevelKey
		)

		UPDATE dbo.CFNPrice SET CFNP_Price = CFNPI_Price,CFNP_Market_Price=CFNPI_Price
		FROM CFNPriceImportInit
		WHERE CreateUser=@UserId
		AND CFNP_CFN_ID=CFNPI_CFN_ID 
		AND CFNP_Group_ID=CFNPI_Group_ID 
		AND CFNP_PriceType = CFNPI_LevelKey
