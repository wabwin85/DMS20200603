DROP view [interface].[V_I_QV_InHospitalSalesBeforeSix_WithQR_Optimize]
GO




Create view [interface].[V_I_QV_InHospitalSalesBeforeSix_WithQR_Optimize]
as
		select	
      ...
      ,CASE WHEN charindex('@@',LTM_LotNumber) > 0
		   ...
		   when DMA_DealerType='LP'
		        ...
		        then 2 --平台的平台正常采购货
		  when DMA_DealerType='T1' and i.WHM_Type in ('Normal','DefaultWH')
		        then 3   --一级的一级采购库
          when DMA_DealerType='T1' 
          --and i.WHM_Type not in ('Normal','DefaultWH')
          and i.WHM_Type='Borrow'
		        then 4   --一级的波科借货库
		   when  DMA_DealerType='T2' and i.WHM_Type in ('Normal','DefaultWH')
		        then 5   --二级的二级采购库
