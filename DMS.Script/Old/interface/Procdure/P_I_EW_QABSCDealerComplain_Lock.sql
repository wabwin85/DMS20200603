DROP PROCEDURE [interface].[P_I_EW_QABSCDealerComplain_Lock]
GO


CREATE PROCEDURE [interface].[P_I_EW_QABSCDealerComplain_Lock]
@DMSBSCCode nvarchar(20), @Status nvarchar(20), @CarrierNumber nvarchar(50), @UserId uniqueidentifier, @InstanceId uniqueidentifier, @ComplainNo nvarchar(200), @DNNumber nvarchar(50)
WITH EXEC AS CALLER
AS
...
SELECT NEWID(),NEWID(),NEWID(),@DMSBSCCode+';'+@Status+';'+isnull(@DNNumber,'')+CONVERT(nvarchar(50),@InstanceId),GETDATE()

	--根据退货单号查询单据是否存在，且单据状态必须是待审批
    IF (@DMSBSCCode IS NULL AND @InstanceId IS NULL)
      RAISERROR ('单据ID不存在',16,1);
    
    SELECT @OrderId = DC_ID FROM dbo.DealerComplain WHERE ((DC_ComplainNbr = @DMSBSCCode OR @DMSBSCCode IS NULL) OR (EFInstanceCode = @DMSBSCCode) ) 
    ...
    BEGIN
		...
		begin
			select @DMSBSCCode =  DC_ComplainNbr from dbo.DealerComplain where DC_ID = @InstanceId
		end
		
      SELECT @OrderId = DC_ID,@CorpId = DC_CorpId,@RETURNNUM = ReturnNum,@UPN = UPN,@LOT = LOT ,@WHMID= dc.WHM_ID ,@WHMTYPE = WHM_Type,@DMATYPE = DMA_DealerType,@ReturnType=CASE WHEN CONFIRMRETURNTYPE IS NULL THEN RETURNTYPE ELSE CONFIRMRETURNTYPE END
		FROM dbo.DealerComplain dc
		LEFT JOIN Warehouse wh ON dc.WHM_ID = wh.WHM_ID
      ...
      IF @Status = 'Accept'
         BEGIN
           --修改状态
           UPDATE dbo.DealerComplain set DC_Status='Accept' where DC_ID = @OrderId  --投诉已收到
             ...
             from DealerComplain t1, dealermaster t2, MailMessageTemplate t3,
                  (
                    select isnull(t1.EMAIL1,t1.EMAIL2) AS MDA_MailAddress
                     from Lafite_IDENTITY t1,(select DC_ComplainNbr,EID,DC_CorpId from DealerComplain where DC_ComplainNbr=@DMSBSCCode) t2
      ...
      IF @Status = 'REJECT'
         BEGIN
           --修改状态
           UPDATE dbo.DealerComplain set DC_Status='Reject' where DC_ID = @OrderId  --审批拒绝
      ...
      IF @Status = 'Revoke'
         BEGIN
           --修改状态
           UPDATE dbo.DealerComplain set DC_Status='Revoked' WHERE DC_ID=@OrderId  --经销商撤销
           
           --记录单据日志
        	 INSERT INTO PurchaseOrderLog (POL_ID,POL_POH_ID,POL_OperUser,POL_OperDate,POL_OperType,POL_OperNote)
        	 VALUES (NEWID(),@OrderId,@SysUserId,GETDATE(),'Submit','经销商撤销申请，状态修改为：已撤销')
      ...
      IF @Status = 'Confirmed'
         BEGIN
            --修改状态
            UPDATE dbo.DealerComplain set DC_Status='Confirmed', COMPLAINTID = @ComplainNo where DC_ID = @OrderId  --投诉已确认，请经销商返回投诉产品  
            
            --记录单据日志
          	INSERT INTO PurchaseOrderLog (POL_ID,POL_POH_ID,POL_OperUser,POL_OperDate,POL_OperType,POL_OperNote)
          	VALUES (NEWID(),@OrderId,@SysUserId,GETDATE(),'Submit','eWorkflow修改状态为:投诉已确认，请返回投诉产品,投诉号：'+isnull(@ComplainNo,''))
           
            --发送邮件(发给二级经销商)
           INSERT INTO MailMessageQueue
           SELECT newid(),'email','',t4.MDA_MailAddress,replace(MMT_Subject,'{#UploadNo}',@ComplainNo),
                   replace(replace(replace(MMT_Body,'{#UploadNo}',DC_ComplainNbr),'{#DealerName}',t2.DMA_ChineseName),'{#SubmitDate}',convert(VARCHAR(30),getdate(),20)) AS MMT_Body,
                   'Waiting',getdate(),null
             from DealerComplain t1, dealermaster t2, MailMessageTemplate t3,
                  (
      ...
      IF @Status = 'SampleReceived'
         BEGIN
           --修改状态
           UPDATE dbo.DealerComplain set DC_Status='SampleReceived' where DC_ID = @OrderId  --投诉产品已收到
           ...
           BEGIN
            --库存操作
            select @CorpId= DC_CorpId,@UPN= UPN,@LOT= LOT,@RETURNNUM= ReturnNum,@WHMID= WHM_ID from dbo.DealerComplain where DC_ID = @OrderId
      ...
      IF @Status = 'CSConfirmed'
         BEGIN
            --修改状态
            UPDATE dbo.DealerComplain set CONFIRMRETURNTYPE=@DNNumber where DC_ID = @OrderId    
      ...
      IF @Status = 'Delivered'
         BEGIN
           --修改状态                              
           UPDATE dbo.DealerComplain set DC_Status='Delivered', DN=@DNNumber where DC_ID = @OrderId  --波科货物已发送
    ...
    --    				--if(@DMATYPE = 'T2' and (@WHMTYPE = 'LP_Consignment' or @WHMTYPE='LP_Borrow') and @ReturnType <> '13' )
    --    				--begin
    ...
    --    				--	   SELECT newid(),CFN_ProductLine_BUM_ID,@ParentDMAID,'Normal','Manual',@SysUserId,getdate(),'Submitted',0,0,WHM_ID,'投诉'
    --    				--	   from DealerComplain,Product,CFN 
    --    				--	   where DC_ID = @OrderId
    --    				--	   and UPN = PMA_UPN
    ...
    --    				--	  from #tmp_PurchaseOrderHeader AS Header,DealerMaster as dm,DealerComplain,Product as p,CFN as cfn,CFNPrice as cfnp
    --    				--	  where dm.DMA_ID= DC_CorpId
    --    				--	  and Header.POH_DMA_ID = dm.DMA_Parent_DMA_ID
    --    				--	  and Header.POH_WHM_ID = WHM_ID
    --    				--	  and UPN = PMA_UPN
    ...
    --    			 BEGIN
				...
				--select @Bum = DivisionName from Product,CFN,V_DivisionProductLineRelation where PMA_CFN_ID = CFN_ID 
				--and CFN_ProductLine_BUM_ID = ProductLineID 
				--and PMA_UPN = @UPN
				
				--EXEC [GC_GetNextAutoNumber] @CorpId,'Next_ShipmentNbr',@Bum,@PONumber OUTPUT
				...
				----写入销售数据表
				--insert into ShipmentHeader(SPH_ID,SPH_Hospital_HOS_ID,SPH_ShipmentNbr,SPH_Dealer_DMA_ID,SPH_ShipmentDate,SPH_Status,SPH_Type,SPH_ProductLine_BUM_ID,SPH_Shipment_User,SPH_SubmitDate,AdjReason)
				--select distinct SPH_ID,HOS_ID,NewShipmentNbr,DMA_ID,SPH_ShipmentDate,SPH_Status,'Hospital',T3.CFN_ProductLine_BUM_ID,@SysUserID,getdate(),ADJ_Reason
    ...
    --           and ((DC.SUBSOLDTONAME is not null and DC.SUBSOLDTONAME<>'' and exists (select 1 from warehouse WH where WH.WHM_ID = DC.WHM_ID and WH.WHM_Type IN ('Consignment','LP_Consignment')))
    ...
    --           )
      ...
      IF @Status = 'OutOfStock'
         BEGIN
           ...
           --修改状态
           UPDATE dbo.DealerComplain set DC_Status='OutOfStock' where DC_ID = @OrderId  --缺货
      ...
      IF @Status = 'DealerCompleted'
         BEGIN
           --修改状态 
           UPDATE DealerComplain SET DC_Status='DealerCompleted' WHERE DC_ID=@InstanceId  --已完成,经销商确认收货
      ...
      IF @Status = 'Completed'
         BEGIN
           --修改状态
           UPDATE dbo.DealerComplain set DC_Status='Completed' where DC_ID = @OrderId and DC_Status not in ('Delivered') --已完成
        	 ...
        	 IF (@WHMID <> '00000000-0000-0000-0000-000000000000' and (@DNNumber ='OnlyComplain' or @DNNumber is null))
        	  BEGIN
			   ...
			   SELECT newid(),'email','',t4.MDA_MailAddress,replace(MMT_Subject,'{#UploadNo}',DC_ComplainNbr),
				...
				--仅投诉事件处理
				IF(@DMATYPE = 'T2' and @ReturnType = '13')
				BEGIN
					...
					ELSE --LP/BSC物权，不扣减库存，生成寄售退货单和订单-仅投诉订单
						BEGIN
							...
							SELECT ReturnNum,NEWID(),PMA_ID,IAH_ID,1,LOT_ID,LOT,WHM_ID,IAH_ProductLine_BUM_ID,LTM_ExpiredDate,
							...
							FROM #tmp_InventoryAdjustHeader ,DealerComplain,LotMaster,Lot,Product,Inventory
							...
							insert into #tmp_PurchaseOrderHeader(POH_ID,POH_ProductLine_BUM_ID,POH_DMA_ID,POH_OrderType,POH_CreateType,
								POH_CreateUser,POH_CreateDate,POH_OrderStatus,POH_IsLocked,POH_LastVersion,POH_WHM_ID,POH_Remark)	
							   SELECT newid(),CFN_ProductLine_BUM_ID,DC_CorpId,'Consignment','Manual',@SysUserId,getdate(),'Submitted',0,0,WHM_ID,'投诉'
							   from DealerComplain,Product,CFN 
							   where DC_ID = @OrderId
							   and UPN = PMA_UPN
							   and PMA_CFN_ID = CFN_ID
      ...
      IF @Status = 'UploadCarrier'
         BEGIN              
            UPDATE DealerComplain SET CarrierNumber=@CarrierNumber,DC_LastModifiedBy=@UserId,DC_LastModifiedDate=GETDATE() WHERE DC_ID=@InstanceId
      ...
      IF @Status = 'QAConfirmed'
         BEGIN
            --修改状态
            UPDATE dbo.DealerComplain set DC_Status='QAConfirmed', COMPLAINTID = @ComplainNo where DC_ID = @OrderId  --投诉已确认，请经销商返回投诉产品  
            
            --记录单据日志
          	INSERT INTO PurchaseOrderLog (POL_ID,POL_POH_ID,POL_OperUser,POL_OperDate,POL_OperType,POL_OperNote)
          	VALUES (NEWID(),@OrderId,@SysUserId,GETDATE(),'Submit','eWorkflow修改状态为:QA初审,投诉号：'+isnull(@ComplainNo,''))
           ...
           SELECT newid(),'email','',t4.MDA_MailAddress,replace(MMT_Subject,'{#UploadNo}',@ComplainNo),
            ...
            where DC_ComplainNbr=@DMSBSCCode and t1.DC_CorpId=t2.DMA_ID
              and t3.MMT_Code='EMAIL_QACOMPLAIN_QACONFIRMED'
           
          END
    END
    
    EXEC [dbo].[GC_DealerComplain_SendMail] @OrderId,'BSC'
	--记录日志
...
BEGIN CATCH 
	...
	--记录日志
	INSERT INTO InterfaceLog (IL_ID,IL_Name,IL_StartTime,IL_EndTime,IL_Status,IL_Message,IL_ClientID,IL_BatchNbr) 
	VALUES (NEWID(),'P_I_EW_QABSCDealerComplain_Lock',GETDATE(),GETDATE(),'Failure','接口调用失败：DMSBSCCode = '+@DMSBSCCode+' 错误信息：' + ERROR_MESSAGE() + CONVERT(NVARCHAR(10),ERROR_LINE()),'SYS',@BatchNbr)
