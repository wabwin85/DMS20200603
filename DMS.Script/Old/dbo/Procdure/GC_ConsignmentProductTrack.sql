DROP PROCEDURE [dbo].[GC_ConsignmentProductTrack]	
GO

/*
* 短期寄售产品追踪汇总
*/
CREATE PROCEDURE [dbo].[GC_ConsignmentProductTrack]	
...
BEGIN TRAN
	...
	SELECT @DealerId = CAH_DMA_ID FROM ConsignmentApplyHeader WHERE CAH_ID = @OrderId
	...
	FROM InventoryAdjustHeader
			...
			WHERE IAH_Reason IN ('Return','Exchange')
				...
				AND EXISTS(SELECT 1 FROM ConsignmentApplyHeader 
									INNER JOIN POReceiptHeader ON PRH_PurchaseOrderNbr COLLATE Chinese_PRC_CI_AS LIKE '%' + CAH_POH_OrderNo COLLATE Chinese_PRC_CI_AS + '%'  
								WHERE CAH_ID = @OrderId
	...
	SELECT *,(POReceiptQty-ISNULL(ClearBorrowQty,0)-ISNULL(ReturnQty,0)-ISNULL(ComplainQty,0)) AS Qty,
	CASE WHEN CHARINDEX('@@',LotNumber) > 0 THEN substring(LotNumber,1,CHARINDEX('@@',LotNumber)-1) ELSE LotNumber END AS PORLotNumber,
    CASE WHEN CHARINDEX('@@',LotNumber) > 0 THEN substring(LotNumber,CHARINDEX('@@',LotNumber)+2,LEN(LotNumber)-CHARINDEX('@@',LotNumber)) ELSE 'NoQR' END AS PORQrCode 
    FROM #TEMP_1 
