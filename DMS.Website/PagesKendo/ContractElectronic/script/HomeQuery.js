...
Page = function () {
    ...
    that.InitPage = function () {
        ...
        $('#QryDealerType').FrameDropdownList({
            dataSource: [{ Key: 'AllDealer', Value: '全部' }, { Key: 'LP', Value: 'LP' }, { Key: 'T1', Value: 'T1' }],
            dataKey: 'Key',
            dataValue: 'Value',
        ...
        var data = this.GetModel('InitPageLPT1');
        createPolicyList();
        createEffectState();
    ...
    that.Query = function () {
        var data = this.GetModel('QueryLPORT1');
    ...
    var openPage = function (data,eSignContractType) {
        ...
        if (data.DealerType == 'LP') {
            if (eSignContractType == 'Appointment') {

                openPdfPage(data.ContractNo, 'LPApply.aspx' + url);
            }
            else if (eSignContractType == 'Renewal') {
                openPdfPage(data.ContractNo, 'LPApply.aspx' + url);
            }
            else if (eSignContractType == 'Amendment') {
                openPdfPage(data.ContractNo, 'LPModify.aspx' + url);
            }
        }
        else if (data.DealerType == 'T1') {
            if (eSignContractType == 'Appointment') {
                openPdfPage(data.ContractNo, 'T1Apply.aspx' + url);
            }
            else if (eSignContractType == 'Renewal') {
                openPdfPage(data.ContractNo, 'T1Apply.aspx' + url);
            }
            else if (eSignContractType == 'Termination') {
                openPdfPage(data.ContractNo, 'T1Termination.aspx' + url);
            }
            else if (eSignContractType == 'Amendment') {
                openPdfPage(data.ContractNo, 'T1Edit.aspx' + url);
            }
        }
    }
    ...
    var openPdfPage = function (title, url, data) {
        openWindow({
            target: 'top',
            title: title,
            url: Common.AppVirtualPath + 'PagesKendo/ContractElectronic/LPTOne/' + url,
            width: 1300,
            height: 600,
            maxed: false,
            resizable: true,
