<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="DealerComplainForGoodsReturnCRM.aspx.cs"
    Inherits="DMS.Website.Pages.Inventory.DealerComplainForGoodsReturnCRM" ValidateRequest="false" %>
                    ...
                    <North MarginsSummary="5 5 5 5" Collapsible="true">
                        <ext:Panel ID="Panel1" runat="server" Title="<%$ Resources: Panel1.Title %>" AutoHeight="true"
                                        ...
                                        <ext:Panel ID="Panel5" runat="server" Border="false" Header="false">
                                            <Body>
                                                <ext:FormLayout ID="FormLayout3" runat="server" LabelAlign="Left" LabelWidth="150">
                                                    ...
                                                    <ext:Anchor>
                                                        <ext:TextField ID="txtDN" runat="server" Width="150" FieldLabel="投诉号码(PI)" />
                                                    </ext:Anchor>
                                                </ext:FormLayout>
                                            </Body>
                                        </ext:Panel>
                            ...
                            <Buttons>
                                ...
                                <%--<ext:Button ID="btnInsertBSC" runat="server" Text="新增BSC投诉" Icon="Add" IDMode="Legacy">
                                <AjaxEvents>
                                ...
                                <ext:Button ID="btnInsertCRM" runat="server" Text="新增CRM投诉" Icon="Add" IDMode="Legacy">
                                    ...
                                    <%--<Listeners>
                                    <Click Handler="openDealerComplain();" />
                                </Listeners>--%>
                                    <Listeners>
                                        <%--<Click Handler="var newid = NewGuid(); window.parent.loadExample('/Pages/Inventory/DealerComplainForCRMEdit.aspx?InstanceId=' + newid,  'subMenu' + newid, '投诉申请单');"></Click>--%>
                                        <Click Handler="var newid = NewGuid(); top.createTab({id: 'subMenu'+ newid,title: '投诉申请单',url: 'Pages/Inventory/DealerComplainForCRMEdit.aspx?InstanceId='+ newid});" />
                                    ...
                                    <ext:GridPanel ID="GridPanel1" runat="server" Title="<%$ Resources: GridPanel1.Title %>"
                                        ...
                                        <ColumnModel ID="ColumnModel1" runat="server">
                                            <Columns>
                                                ...
                                                <ext:Column ColumnID="DN" DataIndex="DN" Header="投诉号码(PI)" Width="140">
                                                </ext:Column>
                                        ...
                                        <Listeners>
                                            <%--<Command Handler="Coolite.AjaxMethods.DealerComplainForGoodsReturnCRM.Show(record.data.DC_ID,record.data.ComplainType,{success:function(){if (record.data.ComplainType =='BSC') {} else {SetCRMSubmit(false);} if (record.data.DC_Status=='Completed') {SetCRMConfirm(true);}else {SetCRMConfirm(false);}  if (record.data.DC_Status=='Submit') {SetCRMCancel(true);} else {SetCRMCancel(false);}  if (record.data.DC_Status=='Confirmed') {SetCRMDelivered(true);} else {SetCRMDelivered(false);} if (record.data.DC_Status=='Delivered') {SetCRMConfirmed(true);} else {SetCRMConfirmed(false);} },failure:function(err){Ext.Msg.alert('Error', err);}});" />--%>
                                           <%--<Command Handler="if(command == 'Edit'){ 
                                            window.parent.loadExample('/Pages/Inventory/DealerComplainForCRMEdit.aspx?InstanceId=' + record.data.DC_ID,  'subMenu' + record.data.DC_ID, '投诉申请单');
                                            }
                                            else if(command=='Confirm'){  Ext.Msg.confirm('确认', '是否确认平台已换货给T2或寄送退款协议给T2?', function(e) { if (e == 'yes') { Coolite.AjaxMethods.DealerComplainForGoodsReturnCRM.UpdateDealerComplain(record.data.DC_ID,{ success: function() { #{GridPanel1}.reload();Ext.MessageBox.alert('Success', '保存成功!'); },failure: function(err) {Ext.Msg.alert('Error', err);}}); } });}              
                                            else if(command=='Copy'){Coolite.AjaxMethods.DealerComplainForGoodsReturnCRM.Show(record.data.DC_ID,'New',{success:function(){SetCRMSubmit(true);SetCRMCancel(false);SetCRMConfirm(false);SetCRMDelivered(false);},failure:function(err){Ext.MessageBox.alert(MsgList.btnInsert.FailureTitle, MsgList.btnInsert.FailureMsg);}});}
                                            else if(command=='UpdateIAN'){Coolite.AjaxMethods.DealerComplainForGoodsReturnCRM.IANEditShow(record.data.DC_ID,{success:function(){},failure:function(err){}});}
                                            else if(command=='UpdateExpress'){Coolite.AjaxMethods.DealerComplainForGoodsReturnCRM.ExpressEditShow(record.data.DC_ID,{success:function(){},failure:function(err){}});}    
                                                " />--%>

                                            <Command Handler="if(command == 'Edit'){ 
                                            top.createTab({id: 'subMenu'+ record.data.DC_ID,title: '投诉申请单',url: 'Pages/Inventory/DealerComplainForCRMEdit.aspx?InstanceId=' + record.data.DC_ID});
                                            }
                                            else if(command=='Confirm'){  Ext.Msg.confirm('确认', '是否确认平台已换货给T2或寄送退款协议给T2?', function(e) { if (e == 'yes') { Coolite.AjaxMethods.DealerComplainForGoodsReturnCRM.UpdateDealerComplain(record.data.DC_ID,{ success: function() { #{GridPanel1}.reload();Ext.MessageBox.alert('Success', '保存成功!'); },failure: function(err) {Ext.Msg.alert('Error', err);}}); } });}              
                                            else if(command=='Copy'){Coolite.AjaxMethods.DealerComplainForGoodsReturnCRM.Show(record.data.DC_ID,'New',{success:function(){SetCRMSubmit(true);SetCRMCancel(false);SetCRMConfirm(false);SetCRMDelivered(false);},failure:function(err){Ext.MessageBox.alert(MsgList.btnInsert.FailureTitle, MsgList.btnInsert.FailureMsg);}});}
                                            else if(command=='UpdateIAN'){Coolite.AjaxMethods.DealerComplainForGoodsReturnCRM.IANEditShow(record.data.DC_ID,{success:function(){},failure:function(err){}});}
        ...
        <ext:Window ID="ExpressEditWindows" runat="server" Icon="Group" Title="经销商回填快递信息"
            ...
            <Body>
                <ext:FitLayout ID="flExpressEdit" runat="server">
                    <ext:Panel ID="Panel36" runat="server" Header="false" AutoHeight="true" Border="false" BodyStyle="padding:5px;">
                        <Body>
            ...
            <Buttons>
                <ext:Button ID="btnSubmit" runat="server" Text="保存" Icon="PageSave">
                    <Listeners>
                        <Click Handler="Coolite.AjaxMethods.DealerComplainForGoodsReturnCRM.ExpressSave({success:function(){Ext.MessageBox.alert('Success', '保存成功!');#{hidDcId}.setValue('');#{tfCourierNumber}.setValue('');#{tfCourierCompany}.setValue('');#{ExpressEditWindows}.hide(null);},failure:function(err){Ext.MessageBox.alert('Failure', err);}})" />
                    </Listeners>
                </ext:Button>
        ...
        <ext:Window ID="IANEditWindow" runat="server" Icon="Group" Title="维护全球投诉号码"
                    ...
                    <ext:Panel ID="Panel6" runat="server" Header="false" AutoHeight="true" Border="false">
                        <Body>
                            <ext:FormLayout ID="FormLayout4" runat="server">
                                ...
                                <ext:Anchor>
                                    <ext:TextField ID="txtWinComplaintID" runat="server" FieldLabel="投诉号码(PI)">
                                    </ext:TextField>
                                </ext:Anchor>
                                <ext:Anchor>
                                    <ext:TextField ID="txtWinIAN" runat="server" FieldLabel="退换货码(IAN)">
                                    </ext:TextField>
                                </ext:Anchor>
                            </ext:FormLayout>
                        </Body>
            ...
            <Buttons>
                <ext:Button ID="Button1" runat="server" Text="保存" Icon="PageSave">
                    <Listeners>
                        <Click Handler="Coolite.AjaxMethods.DealerComplainForGoodsReturnCRM.IANSave({success:function(){Ext.MessageBox.alert('Success', '保存成功!');#{hidWinDcId}.setValue('');#{txtWinComplaintID}.setValue('');#{txtWinIAN}.setValue('');#{IANEditWindow}.hide(null);#{ResultStore}.reload();},failure:function(err){Ext.MessageBox.alert('Failure', err);}})" />
                    </Listeners>
                </ext:Button>
