DROP Procedure [dbo].[GC_DealerComplain_CheckUPNAndDateCRM]
GO

/*
经销商提交投诉退货前对UPN、LOT及手术日期进行校验，返回校验结果
*/
CREATE Procedure [dbo].[GC_DealerComplain_CheckUPNAndDateCRM]
...
--    END
  IF (len(@RtnMsg) = 0  and @WHMID <> '00000000-0000-0000-0000-000000000000')
    BEGIN
      --判断库存是否够扣减
      select @NUM = sum(Lot.LOT_OnHandQty) - 1 
      ...
      IF (@ComplainType = 'CRM')
        BEGIN
           ...
           if (@Validdate < @EventDate)
              SET @RtnMsg = '产品有效期：'+ Convert(nvarchar(8),@Validdate,112) + ',事件发生日期大于产品有效期，属于过期使用！'      
             
        END
      ELSE
        BEGIN
          if (convert(int,Convert(nvarchar(6),@Validdate,112)) < convert(int,Convert(nvarchar(6),@EventDate,112)))
            SET @RtnMsg = '产品有效期：'+ Convert(nvarchar(6),@Validdate,112) + ',事件发生日期大于产品有效期，属于过期使用！' 
        END
    END
