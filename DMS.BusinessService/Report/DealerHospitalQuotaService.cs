...
{
    public class DealerHospitalQuotaService : ABaseQueryService,IQueryExport, IDealerFilterFac
    {
        #region Ajax Method
        ...
        public DealerHospitalQuotaVO Init(DealerHospitalQuotaVO model)
        {
            try
            {
        ...
        public DealerHospitalQuotaVO ChangeBrand(DealerHospitalQuotaVO model)
        {
            try
            {
        ...
        public string Query(DealerHospitalQuotaVO model)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(model.QryHospitalName.ToSafeString()))
                {
                    param.Add("HospitalName", model.QryHospitalName.ToSafeString());
                }
                ...
                DataSet ds = rptDao.DealerHospitalQuotaReport(param, start, model.PageSize, out totalCount); 
        ...
        {
            if (Parameters["ExportType"].ToSafeString() == "ExportDealerHospitalQuota")
            {
                Hashtable param = new Hashtable();
                ...
                param.Add("SubCompanyId", BaseService.CurrentSubCompany?.Key);
                int totalCount = 0;
                DataSet ds = rptDao.DealerHospitalQuotaReport(param, 0, int.MaxValue, out totalCount);
                //删除多余列               
                ...
                ds.Tables[0].Columns["CQName"].ColumnName = "指标分类";
                ds.Tables[0].Columns["HospitalName"].ColumnName = "医院名称";
                ds.Tables[0].Columns["HospitalNbr"].ColumnName = "医院编号";
