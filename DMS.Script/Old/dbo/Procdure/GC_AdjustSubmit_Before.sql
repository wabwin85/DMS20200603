DROP PROCEDURE [dbo].[GC_AdjustSubmit_Before]
GO



CREATE PROCEDURE [dbo].[GC_AdjustSubmit_Before]
...
BEGIN TRAN
	...
	FOR (SELECT IAC_LTM_ID AS LtmId,IAC_ShippedQty AS ShippedQty,LTM_Product_PMA_ID AS PmaId,IAC_PRH_ID AS PrhId FROM InventoryAdjustConsignment,LotMaster WHERE IAC_LTM_ID = LTM_ID AND IAC_IAH_ID = @AdjustHeadId) --查出需要的集合放到游标中
	OPEN AdjustConsignment_Cursor; --外层游标打开游标
	FETCH NEXT FROM AdjustConsignment_Cursor INTO @LtmId,@ShippedQty,@PmaId,@PurchaseOrderId; --外层游标读取第一行数据
	WHILE @@FETCH_STATUS = 0
		BEGIN
			...
			FOR (SELECT T_LotId,T_Qty,T_ProductId,T_WarehouseId,T_LotNumber,T_ExpiredDate FROM #InventoryTemp WHERE T_WarehouseType = 'LP_Consignment' AND T_LtmId = @LtmId)
				...
				BEGIN
					...
					ELSE IF @Now_Qty > @Qty  
						BEGIN
							...
							INSERT INTO InventoryAdjustLot (IAL_IAD_ID,IAL_ID,IAL_LotQty,IAL_LOT_ID,IAL_WHM_ID,IAL_LotNumber,IAL_ExpiredDate,IAL_PRH_ID)
							VALUES (@LineId,NEWID(),@Qty,@LotId,@WarehouseId,@LotNumber,@ExpiredDate,@PurchaseOrderId)
					...
					ELSE 
						BEGIN
							...
							INSERT INTO InventoryAdjustLot (IAL_IAD_ID,IAL_ID,IAL_LotQty,IAL_LOT_ID,IAL_WHM_ID,IAL_LotNumber,IAL_ExpiredDate,IAL_PRH_ID)
							VALUES (@LineId,NEWID(),@Now_Qty,@LotId,@WarehouseId,@LotNumber,@ExpiredDate,@PurchaseOrderId)
			...
			IF @Now_Qty > 0 
				BEGIN
					...
					FOR (SELECT T_LotId,T_Qty,T_ProductId,T_WarehouseId,T_LotNumber,T_ExpiredDate FROM #InventoryTemp WHERE T_WarehouseType = 'Consignment' AND T_LtmId = @LtmId)
			...
			FETCH NEXT FROM AdjustConsignment_Cursor INTO @LtmId,@ShippedQty,@PmaId,@PurchaseOrderId; --外层游标读取下一行数据
		END
