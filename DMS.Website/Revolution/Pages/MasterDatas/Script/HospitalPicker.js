...
HospitalPicker = function () {
    ...
    var business = 'MasterDatas.HospitalPicker';
    var pickedList = [];

    that.GetModel = function () {
        var model = FrameUtil.GetModel();
        model.IsFilterAuth = (Common.GetUrlParam('IsFilterAuth') && Common.GetUrlParam('IsFilterAuth') == 'true');
        model.IsMuliteSelect = (Common.GetUrlParam('IsMuliteSelect') && Common.GetUrlParam('IsMuliteSelect') == 'true');
        model.IsFilterProductLine = (Common.GetUrlParam('IsFilterProductLine') && Common.GetUrlParam('IsFilterProductLine') == 'true');
        model.ProductLineID = Common.GetUrlParam('ProductLine');
        return model;
    }
            ...
            callback: function (model) {
                ...
                $('#BtnOk').FrameButton({
                    ...
                    onClick: function () {
                        ...
                        else if (pickedList.length > 1 && !that.GetModel().IsMuliteSelect) {
                            FrameWindow.ShowAlert({
                                target: 'top',
                                alertType: 'warning',
                                message: '医院选择必须唯一'
