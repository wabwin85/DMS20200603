...
/****** Object:  StoredProcedure [interface].[P_I_EW_MaintContract_New]    Script Date: 2019/11/27 13:16:05 ******/
SET ANSI_NULLS ON
GO
...
AS BEGIN
	...
	IF (@ContractType='Appointment')
		BEGIN
			...
			BEGIN
				SELECT @CheckId=Count(*) FROM DealerMaster dm (nolock) WHERE (dm.DMA_ID=@DMA_ID OR dm.DMA_ChineseName=@DealerName) and dm.DMA_DealerType=@DealerLevel and dm.DMA_ActiveFlag=1 and (@Parent_DMA_ID is null OR DMA_Parent_DMA_ID= @Parent_DMA_ID) and @DealerMarketType=ISNULL(DMA_MarketType,0) AND @SubCompanyId=DMA_SubCompanyId
				IF (@CheckId=0)
				BEGIN
			...
			BEGIN
				...
				DECLARE @ERPCode NVARCHAR(200)
				IF @DealerLevel='T2'
				BEGIN
					IF(@Parent_DMA_ID IS NOT NULL)
					BEGIN
						--目前仅三家平台
						IF (@Parent_DMA_ID IN ('33029AF0-CFCF-495E-B057-550D16C41E4A','A54ADD15-CB13-4850-9848-6DA4576207CB','F8867C4B-2120-416A-ACDE-7BBA45C96ACB'))
						BEGIN
						...
						BEGIN
							SELECT @SAPCode=MAX(DealerMaster.DMA_SAP_Code)+1 FROM DealerMaster (nolock) 
							WHERE DealerMaster.DMA_DealerType='T2' AND DMA_Parent_DMA_ID NOT IN
							('33029AF0-CFCF-495E-B057-550D16C41E4A','A54ADD15-CB13-4850-9848-6DA4576207CB','F8867C4B-2120-416A-ACDE-7BBA45C96ACB');
						END
						...
						END
						--国科由4000开头
						ELSE IF @Parent_DMA_ID='A54ADD15-CB13-4850-9848-6DA4576207CB'
						BEGIN
							IF ISNULL(@SAPCode,'')='' 
							BEGIN
								SET @SAPCode='4000'
						...
						END
						--聚赢7000
						ELSE IF @Parent_DMA_ID='F8867C4B-2120-416A-ACDE-7BBA45C96ACB'
						BEGIN
				...
				BEGIN
					...
					SELECT @ERPCode=SAPCode FROM contract.AppointmentCandidate where contractid=@InstanceID
				END
				...
				BEGIN
					...
					SELECT NewID(), @ERPCode,@DealerName, @DealerEnName, null,1, @DealerLevel
				...
				END
				--更新关联字段
				UPDATE DealerMaster SET DMA_DMM_ID=dmm.DMM_ID
				FROM DealerMasterMain dmm
				WHERE dmm.DMM_ChineseName=@DealerName
				AND DMA_ID IN(SELECT DMA_ID FROM interface.T_I_EW_Contract (nolock) WHERE InstanceID=@InstanceID AND UserID=@UserID)
				----1.1.2更新上级经销商
				--一级
				...
				SET @CompanyAbbr=ISNULL((SELECT TOP 1 REV1 FROM dbo.Lafite_ATTRIBUTE WHERE ATTRIBUTE_TYPE='SUBCOMPANY' AND Id=@SubCompanyId),'LW');
			...
			ELSE IF (@drParameters='OLD')
			 BEGIN
					IF(@DMA_ID_Midd IS NULL)
					BEGIN
						SELECT @DMA_ID= DMA_ID FROM DealerMaster dm (nolock) WHERE dm.DMA_ChineseName=@DealerName and dm.DMA_DealerType=@DealerLevel and dm.DMA_ActiveFlag='1' and (isnull(@Parent_DMA_ID,'00000000-0000-0000-0000-000000000000')='00000000-0000-0000-0000-000000000000' or dm.DMA_Parent_DMA_ID=@Parent_DMA_ID) AND @SubCompanyId=DMA_SubCompanyId
					END
					ELSE
					BEGIN
					...
					END
					--更新合同经销商ID数据
					Update contract.AppointmentCandidate SET CompanyID=@DMA_ID WHERE ContractID=@InstanceID
					--修改经销商中英文名称
