USE [GenesisDMS_Test]
GO
/****** Object:  UserDefinedFunction [dbo].[GC_Fn_GetAuthorCodeAndDivName]    Script Date: 2019/11/12 22:36:52 ******/
SET ANSI_NULLS ON
GO
...
ALTER FUNCTION [dbo].[GC_Fn_GetAuthorCodeAndDivName]
(
...
BEGIN
	...
	--授权分类名称
     ...
     --外科产品线需要增加注册证名称 CFN_ChineseName待替换
	 IF(@Division='Frankman' or @Division='Ritchey')
	 BEGIN
		SELECT @AuthorNameString =
                 STUFF ( (SELECT ',' + tt1.PCT_Name
                     FROM (SELECT DISTINCT D.REGName PCT_Name
                             FROM    DealerAuthorizationTableTemp A
                             LEFT JOIN (select distinct CA_NameCN,CA_Code,CA_ID from V_ProductClassificationStructure f where  @BeginDate between f.StartDate and f.EndDate and f.CC_Code=@SubBU) B
                             ON A.DAT_PMA_ID = B.CA_ID
							 INNER JOIN cfnClassification C ON B.CA_ID=C.ClassificationId
							 INNER JOIN CFN D ON D.CFN_CustomerFaceNbr = C.cfnCustomerFaceNbr
                            WHERE A.DAT_DCL_ID = @ContractId
							AND ISNULL(D.REGName,'')<>'') tt1
                   FOR XML PATH ( '' )),
                 1,
                 1,
                 '')
	 ...
	 BEGIN
		SELECT @AuthorNameString =
                 ...
                 '')
	 END
	 SET @AuthorNameString=REPLACE(@AuthorNameString,'&amp;','&')
	--产品线
