USE [GenesisDMS_Test]
GO
/****** Object:  StoredProcedure [interface].[P_I_EW_DCMS_GetDealerContract]    Script Date: 2019/11/20 15:56:20 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
ALTER Procedure [interface].[P_I_EW_DCMS_GetDealerContract]
...
AS
	...
	DECLARE @AOPDealerTotal decimal(18,2)   ;
...
BEGIN  
	SET @RtnVal='';
	SET @AOPDealerTotal=0;
	--获取SubBU
	SELECT @SubDepId=CC_ID FROM interface.ClassificationContract(nolock) where CC_Code=CONVERT(nvarchar(100), @SubDepCode)
	--获取BU
	SELECT @DivisionName=DivisionName,@ProductLine=ProductLineID FROM V_DivisionProductLineRelation WHERE  DivisionCode=CONVERT(NVARCHAR(10),@Division) and IsEmerging='0'
	--获取经销商		   
	SELECT @DMA_ID=DMA_ID FROM DealerMaster(nolock) WHERE DealerMaster.DMA_SAP_Code=@DMA_SAP_Code;
	
	--计算截止到当前季度指标
	DECLARE @PRODUCT_CURQ cursor;
		SET @PRODUCT_CURQ=cursor for 
			SELECT aopd.AOPD_Year AS Year,
			  ...
			  FROM  V_AOPDealer aopd(nolock)
			WHERE  aopd.AOPD_Dealer_DMA_ID=@DMA_ID AND AOPD_ProductLine_BUM_ID=@ProductLine
			OPEN @PRODUCT_CURQ
			FETCH NEXT FROM @PRODUCT_CURQ INTO @Year,@Q1,@Q2,@Q3,@Q4
			WHILE @@FETCH_STATUS = 0        
				BEGIN
					IF(@Year<Year(getdate()))
					BEGIN
						SET @AOPDealerTotal = @AOPDealerTotal + @Q1 + @Q2 + @Q3 + @Q4
					END
					ELSE IF(@Year=Year(getdate()))
					BEGIN
						SET @AOPDealerTotal = @AOPDealerTotal + Case (Month(getdate())-1)/3+1 WHEN 1 THEN
						@Q1 WHEN 2 THEN @Q1+@Q2 WHEN 3 THEN @Q1+@Q2+@Q3 ELSE @Q1+@Q2+@Q3+@Q4 END
					END
				FETCH NEXT FROM @PRODUCT_CURQ INTO @Year,@Q1,@Q2,@Q3,@Q4
				END
			CLOSE @PRODUCT_CURQ
			DEALLOCATE @PRODUCT_CURQ ;
	--获取上个合同指标
	...
	Pricing_Rebate,Pricing_Rebate_Remark,PaymentTerm,CreditLimit,CreditTerm,SecurityDeposit,Attachment,AOPDealerQuarter,AOPDealerSum,HoapitalString,Inform,InformOther,AreaString,PayTerm,IsDeposit,AOPDealerTotal)
	...
	DCM_Pricing_Rebate,DCM_Pricing_Rebate_Remark,DCM_Payment_Term,DCM_Credit_Limit,DCM_Credit_Term,DCM_Security_Deposit,DCM_Attachment,@RtnVal,@AOPtotal,@HoapitalString,DCM_Guarantee,DCM_GuaranteeRemark,@AreaString,DCM_PayTerm,DCM_IsDeposit,@AOPDealerTotal
	...
	BEGIN
		...
		ContractType,BSCEntity,Exclusiveness,ProductLine,ProductLineRemark,Pricing_Discount,Pricing_Discount_Remark,
		Pricing_Rebate,Pricing_Rebate_Remark,PaymentTerm,CreditLimit,CreditTerm,SecurityDeposit,Attachment,AOPDealerQuarter,AOPDealerSum,HoapitalString,Inform,InformOther ,AreaString,@MarketType as MarketType,@SubDepCode as CCode,PayTerm,IsDeposit,AOPDealerTotal
		,0 AS IsNewProduct
		from #tbReturn; 
	END
	ELSE
	BEGIN
		...
		NULL AS AOPDealerSum,NULL AS HoapitalString,NULL AS Inform,NULL AS InformOther ,NULL AS AreaString,@MarketType as MarketType,@SubDepCode as CCode,NULL AS PayTerm,NULL AS IsDeposit,@AOPDealerTotal AS AOPDealerTotal
