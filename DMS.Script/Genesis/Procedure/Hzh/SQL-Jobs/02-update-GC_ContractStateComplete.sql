		...
		WHILE @@FETCH_STATUS = 0        
			...
			BEGIN
				...
				IF EXISTS (SELECT 1 FROM  interface.T_I_MDM_DeaelrTraining a 
								...
								AND A.TestStatus='已通过' AND TestName='第三方合规知识培训-测试试题')
				AND (EXISTS (SELECT 1 FROM  interface.T_I_MDM_DeaelrTraining a WHERE  a.DealerId=@DealerId 
							AND CONVERT(NVARCHAR(10),TestDate,120)>=DATEADD(DD,-90,GETDATE()) 
							--AND (YEAR(TestDate)=YEAR(@BeginDate) OR (YEAR(TestDate)+1=YEAR(@BeginDate) and MONTH(TestDate)=12))
							AND A.TestStatus='已通过' AND TestName='代理商质量培训考试') OR @DealerType='T2')
				...
				AND (EXISTS (SELECT 1 FROM interface.T_I_MDM_vDealer_QSN a INNER JOIN DealerMaster B ON A.DMSCode=B.DMA_SAP_Code WHERE B.DMA_ID=@DealerId)
					OR EXISTS (SELECT 1 FROM DealerMaster A WHERE A.DMA_ID=@DealerId AND A.DMA_DealerType='LP')
				)
				BEGIN
					SET @Sysflg=1;
				END
...
BEGIN CATCH 
 ...
 SET NOCOUNT OFF
	...
	VALUES(NEWID(),'email','','aaron.ma@gmedtech.com','经销商'+@ContractType+'合同同步错误GC_ContractStateComplete，合同编号：'+CONVERT(nvarchar(50),@ContractId),@RtnMsg,'Waiting',GETDATE(),null)
