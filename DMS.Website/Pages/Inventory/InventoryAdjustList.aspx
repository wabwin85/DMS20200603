<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="InventoryAdjustList.aspx.cs"
    Inherits="DMS.Website.Pages.Inventory.InventoryAdjustList" %>
    ...
    <script language="javascript">
        ...
        var ChangeAdjustType = function() {
            ...
            if (cbAdjustTypeWin.getValue() == 'StockIn') {
                ...
                grid.getColumnModel().setEditable(6, true);
            }
            else {
                  grid.getColumnModel().setHidden(6,false);
                  grid.getColumnModel().setHidden(13,false);
                grid.getColumnModel().setEditable(5, false);
                grid.getColumnModel().setEditable(6, false);
            }
        ...
        var CheckSubmit = function() {
            ...
            var hiddenIsRtnValue= Ext.getCmp('hiddenIsRtnValue');
            var grid = Ext.getCmp('GridPanel2');
            ...
            if (cbProductLineWin.getValue() != '' && cbAdjustTypeWin.getValue() != '' && txtAdjustReasonWin.getValue() != '' && grid.store.getCount() > 0) {
                if (CheckLotQty()) {
                    Coolite.AjaxMethods.DoSubmit({
                        success: function() {
                        if(hiddenIsRtnValue.getValue()=='true')
                        {
                            Ext.getCmp('hiddenIsModified').setValue('');
                            Ext.getCmp('GridPanel1').store.reload();
                            Ext.getCmp('DetailWindow').hide();
...
//               if((record.data.QRCodeEdit==''|| record.data.QRCodeEdit == null)&& hiddenAdjustType.getValue()=='StockOut' && record.data.QRCode=='NoQR')
//                {
//                 Ext.Msg.alert('<%=GetLocalResourceObject("CheckLotQty.Alert.Title1").ToString()%>', '二维码为NoQR的其他出库必须填写二维码');
//                    return false;
//                }
...
//                }
        ...
        var CheckQty = function() {
            ...
            record.data.TotalQty,txtAdjustQty.getValue();
...
<body>
                    ...
                    <ext:Panel ID="Panel1" runat="server" Title="<%$ Resources: Panel1.Title %>" AutoHeight="true"
                        ...
                        <Buttons>
                            ...
                            <ext:Button ID="btnImport" runat="server" Text="<%$ Resources: btnImport.Text %>"
                                ...
                                <Listeners>
                                    <%--<Click Handler="window.parent.loadExample('/Pages/Inventory/InventoryAdjustImport.aspx','subMenu300',OrderApplyMsgList.msg1);" />--%>
                                    <Click Handler="top.createTab({id: 'subMenu300',title: '导入',url: 'Pages/Inventory/InventoryAdjustImport.aspx'});" />
                                </Listeners>
                            </ext:Button>
                        </Buttons>
                    </ext:Panel>
    ...
    <ext:Hidden ID="hiddenIsRtnValue" runat="server" />
                                                    ...
                                                    <ext:FormLayout ID="FormLayout8" runat="server" LabelAlign="Left" LabelWidth="80">
                                                        <ext:Anchor>
                                                            <ext:TextArea ID="txtAdjustReasonWin" runat="server" FieldLabel="<%$ Resources: DetailWindow.FormLayout8.txtAdjustReasonWin.FieldLabel %>"
                                                                Width="380" SelectOnFocus="true" Height="50" />
                                                        </ext:Anchor>
                                                    </ext:FormLayout>
                                                </Body>
                                            </ext:Panel>
