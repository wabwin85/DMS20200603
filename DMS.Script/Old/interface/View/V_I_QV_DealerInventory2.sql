DROP view [interface].[V_I_QV_DealerInventory2]
GO
...
CREATE view [interface].[V_I_QV_DealerInventory2]
as
--是否需要过滤停用经销商的库存
select * from (
		select	
			...
			,LOT
			,EXPDate
			,QTY=case when DMA_SAP_Code='162878' 
  ...
  FROM [interface].[V_I_CR_Inventory2] i
