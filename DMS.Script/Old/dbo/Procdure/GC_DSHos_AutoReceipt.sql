DROP  Procedure [dbo].[GC_DSHos_AutoReceipt]
GO


/*
直销医院自动收货
系统自动判断直销医院有哪些收货单状态还是"Waiting",然后自动进行收货操作
每天晚上自动执行
*/
CREATE Procedure [dbo].[GC_DSHos_AutoReceipt]
...
BEGIN TRAN
	...
	AND D.DMA_DealerType = 'T1'
    AND D.DMA_Taxpayer = N'直销医院'    --用此字段代表是直销医院
  ...
  WHERE POReceiptHeader.PRH_ID = T.PRH_ID
	...
	SELECT NEWID(),PRH_ID,@SysUserId,GETDATE(),'Confirm','系统自动完成直销医院的收货确认'
	FROM #TMP_DATA
