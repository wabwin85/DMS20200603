...
BEGIN
	...
	--Line 2
	SET @Rtn += '<tr><td class="title" width="13%">申请单号(Application No.)</td><td colspan="5">' + @ContractNo + '</td></tr>'
	--Line 3
	SET @Rtn += '<tr><td class="title" width="13%">经销商类型(Dealer Type)</td><td width="20%">' + @DealerTypeShow + '</td><td class="title" width="13%">产品线(Product Line)
</td><td width="20%">' + @DepIdShow + '</td><td class="title" width="13%">合同分类(Sub-BU)</td><td width="21%">' + @SUBDEPIDShow + '</td></tr>'
	--Line 4
	...
	END
	--Line 6
	SET @Rtn += '<tr><td class="title">是否为设备经销商（Equipment Dealer）</td><td colspan="5">' + @IsEquipmentShow + '</td></tr>'
	--Line 7
	...
	--Line 1
	SET @Rtn += '<tr><td class="title" width="25%">合作类型 (Cooperation type)</td><td width="25%">' + @CurrentContractType + '</td><td class="title" width="25%">合同实体 Contractual entity</td><td width="25%">' + @CurrentBSC + '</td></tr>'
	SET @Rtn += '<tr><td class="title">独家合同 Exclusive contract</td><td colspan="3">' + @CurrentExclusiveness + '</td></tr>'
	--Line 2
	SET @Rtn += '<tr><td class="title">协议期限 协议生效日（Effective Date）</td><td>' + @CurrentAgreementBegin + '</td><td class="title">协议期限 协议到期日（Expiration Date）</td><td>' + @CurrentAgreementEnd + '</td></tr>'
	--Line 3
	SET @Rtn += '<tr><td class="title">产品线(Product Line)</td><td>' + @CurrentProduct + '</td><td class="title">(如果是部分产品线，请列出)(if partial, please list)</td><td>' + @CurrentPriceRemark + '</td></tr>'
	--Line 4
	SET @Rtn += '<tr><td class="title">价格(Price)<br />标准价格的_%的折扣  (Pricing Discount  _% off standard price list)</td><td>' + @CurrentPrice + '</td><td class="title">备注 （Remark）</td><td>' + @CurrentPriceRemark + '</td></tr>'
	--Line 5
	SET @Rtn += '<tr><td class="title">销售返利政策<br />季度采购总额的_% (Sales Rebate  _% of the quarter purchase AMT)</td><td>' + @CurrentSpecialSales + '</td><td class="title">备注（Remark）</td><td>' + @CurrentSpecialSalesRemark + '</td></tr>'
	--Line 6
	...
	SET @Rtn += '<tr><td class="title">区域(医院)/Territory(Hospital)</td><td>' + @CurrentHospital + '&nbsp;<a href="' + @CurrentHospitalUrl + '" target="_blank">查看</a></td><td class="title">采购指标(CNY)/Purchase Quota(CNY)<br />(BSC SFX Rate:USD 1 = CNY 6.69)</td><td>' + @CurrentQuota + '&nbsp;<a href="' + @CurrentQuotaUrl + '" target="_blank">查看</a></td></tr>'
	--Line 7
	SET @Rtn += '<td class="title">指标总额(CNY)不含税 Quota Total (CNY Without VAT)</td><td>' + @CurrentQuotaTotal + '</td><td class="title">付款方式(Payment)</td><td>' + @CurrentPaymentShow + '</td></tr>'
	--Line 8
	IF @CurrentPayment = 'Credit'
	BEGIN
	    SET @Rtn += '<tr><td class="title">信用期限(天数)(credit terms（Day)</td><td>' + @CurrentCreditTermShow + '</td><td class="title">信用额度(CNY, 含增值税)(Credit Limit)</td><td>' + @CurrentCreditLimit + '</td></tr>'
	END
	--Line 9
	IF @CurrentPayment = 'Credit'
	BEGIN
	    SET @Rtn += '<tr><td class="title">保证金(CNY) Deposit</td><td>' + @CurrentDeposit + '</td><td class="title">保证金形式 Deposit Type</td><td>' + @CurrentInformShow + '</td></tr>'
	END
	--Line 10
	IF @CurrentPayment = 'Credit'
	   AND EXISTS (
	...
	BEGIN
	    SET @Rtn += '<tr><td class="title">保证金形式备注  Deposit Type Remark</td><td colspan="3">' + @CurrentInformOther + '</td></tr></tr>'
	END
	--Line 11
	...
	--Head
	SET @Rtn += '<h4>续约后协议条款 New Agreement Term</h4><table class="gridtable">'
	--Line 1
	...
	SET @Rtn += '<tr><td class="title">区域(医院) Territory (Hospital)</td><td>' + @Hospital + '&nbsp;<a href="' + @HospitalUrl + '" target="_blank">查看</a><br/><a href="' + @AuthUrl + '" target="_blank"><h4>授权明细列表</h4></a></td><td class="title">采购指标(CNY)<br/>(BSC SFX Rate:USD 1 = CNY 6.69) Purchase Quota(CNY)
(BSC SFX Rate:USD 1 = CNY 6.69)</td><td>' + @Quota + '&nbsp;<a href="' + @QuotaUrl + '" target="_blank">查看</a></td></tr>'
	--Line 2
	IF @IsFirstContract = '1'
	BEGIN
	...
	--PreContract
	--SET @Rtn += '<tr><td>所拟定的年度合同价值是多少？(What is the proposed annual contract value)</td><td colspan="3">' + @Pre6_HTJZShow + '</td></tr>'
	--PreContract
...
SET @Rtn += '<tr><td class="title">经销商总计采购指标小于总计医院指标的原因 Reason for total dealer commercial Purchase target less than total hospital target</td><td >' + @DealerLessHosReason 
	...
	--Line 2
	SET @Rtn += '<tr><td class="title">产品组最小指标备注 Product group minimal target remark</td><td>' + @ProductGroupMemo + '</td><td class="title">附件 (Attachement)</td><td>' + dbo.Func_GetAttachmentHtml(@Attachment) + '</td></tr>'
	IF @IsFirstContract = '1'
	BEGIN
	...
	END
	--Line 21
	--SET @Rtn += '<tr><td class="title">附件 (Attachement)</td><td colspan="3">' + dbo.Func_GetAttachmentHtml(@Attachment) + '</td></tr>'
