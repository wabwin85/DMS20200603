DROP PROCEDURE [Promotion].[Proc_Pro_ImportResult4EWorkFlow] 
GO
...
/**********************************************
	...
	1.创建 2016-03-07
**********************************************/
CREATE PROCEDURE [Promotion].[Proc_Pro_ImportResult4EWorkFlow] 
...
BEGIN TRY 	
   ...
   WITH(   
    ...
    ALTER TABLE #ImportResult ADD LargessDesc NVARCHAR(2000);
    ALTER TABLE #ImportResult ADD DealerId NVARCHAR(100);
    ALTER TABLE #ImportResult ADD GiftPrice decimal(14, 2);
	...
	*/
	...
	BEGIN
		SET @SQL = N'INSERT INTO #TMP_NOT_ALIGN_COUNT(PolicyNo) 
			SELECT '''+@PolicyNo+''' WHERE EXISTS (SELECT * FROM '+@TempTableName+' A
			WHERE  EXISTS (SELECT * FROM #ImportResult A
			WHERE PolicyNo = '''+@PolicyNo+''' AND NOT EXISTS (SELECT 1 FROM '+@TempTableName+' 
		...
		--		AND Largess'+@CalPeriod+' <> convert(DECIMAL(18,4),b.OraNum))'
		--PRINT @SQL
		--BEGIN TRY
		--	EXEC(@SQL)	
		--END TRY
	...
	--IF ISNULL(@iStr,'') <> ''
	--BEGIN
	--	SET @Return = '如下政策原数量与系统中已结算的不符:'+@iStr
	--	RETURN
	--END
	...
	,GiftPrice=Promotion.func_Pro_Policy_Factor_GetProductMaxPrice(B.GiftPolicyFactorId,a.DealerId)
	...
	BEGIN TRAN
		...
		SELECT TOP 1 @Description,B.BU,B.Period,A.AccountMonth,'草稿',@UserId,GETDATE(),'','赠品',@MarketType,@Reason,@BeginDate,@EndDate,'待结账',NEWID()
		FROM #ImportResult A,Promotion.Pro_Policy B
		WHERE A.PolicyNo = B.PolicyNo
		
		SELECT @FlowId = SCOPE_IDENTITY()
		
		INSERT INTO Promotion.T_Pro_Flow_Detail(FlowId,PolicyId,LargessDesc,LPId,DealerId,HospitalId,OraNum,AdjustNum,GiftPrice)
		SELECT @FlowId,B.PolicyId,A.LargessDesc,A.LPId,A.DealerId,A.HospitalId,A.OraNum,A.AdjustNum,A.GiftPrice FROM #ImportResult A,Promotion.Pro_Policy B
		WHERE A.PolicyNo = B.PolicyNo
