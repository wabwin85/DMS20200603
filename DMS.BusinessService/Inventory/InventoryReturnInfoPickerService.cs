...
{
    public class InventoryReturnInfoPickerService : ABaseQueryService
    {
        IRoleModelContext _context = RoleModelContext.Current;
        public InventoryReturnInfoPickerVO Init(InventoryReturnInfoPickerVO model)
        {
            try
            {
                model.IsCFN = false;
                ...
                if (model.ReturnTypeWin.ToString() == AdjustType.StockIn.ToString())
                {
                    model.IsCFN = true;
                    list = WarehouseByDealerAndType(new Guid(model.QryDealerId), model.QryReturnType);
                }
                if (model.ReturnTypeWin.ToString() == AdjustType.StockOut.ToString())
                {
                    list = BindWarehouseStockOut(new Guid(model.QryDealerId), model.QryReturnType);
                }
                else if (model.ReturnTypeWin.ToString() == AdjustType.Return.ToString() || model.ReturnTypeWin.ToString() == AdjustType.Exchange.ToString())
                {
                    list = this.BindWarehouseWithReturn(new Guid(model.QryDealerId), model.QryReturnType);
                }
                else if (model.ReturnTypeWin.ToString() == AdjustType.Transfer.ToString())
                {
                    list = WarehouseByDealerAndType(new Guid(model.QryDealerId), "Borrow");
                }
                else
                {
                    list = this.WarehouseByDealerAndType(new Guid(model.QryDealerId), model.QryReturnType);
                }
        ...
        public string Query(InventoryReturnInfoPickerVO model)
        {
            try
            {
                if (!model.IsCFN)
                {
                    ...
                    param.Add("Type", model.ReturnTypeWin);
                    //可以有十个模糊查找的字段
                    ...
                    DataSet ds;
                    //二级经、寄售、退货
                    if ((model.ReturnTypeWin == AdjustType.Return.ToString() || model.ReturnTypeWin == AdjustType.Exchange.ToString())
        && _context.User.CorpType == DealerType.T2.ToString()
        && model.QryReturnType == AdjustWarehouseType.Consignment.ToString())
                    {
                        //平台寄售、波科寄售
                    ...
                    //寄售转销售
                    else if (model.ReturnTypeWin == AdjustType.CTOS.ToString() && _context.User.CorpType == DealerType.T2.ToString())
                    {
                        ds = business.QueryCurrentCTOSInv(param);
                    }
                    //转移给其他经销商
                    else if (model.ReturnTypeWin == AdjustType.Transfer.ToString() && (_context.User.CorpType == DealerType.T1.ToString() || _context.User.CorpType == DealerType.LP.ToString() || _context.User.CorpType == DealerType.LS.ToString()))
                    {
        ...
        public IList<Warehouse> BindWarehouseWithReturn(Guid dealerId, string dealerWarehouseType)
        {
            ...
            if (dealerWarehouseType.Equals(AdjustWarehouseType.Normal.ToString()))
            {
                switch ((DealerType)Enum.Parse(typeof(DealerType), dealer.DealerType, true))
                {
                    case DealerType.LP:
                        //平台，显示所有库存
                        list = (from t in list where t.Type != WarehouseType.SystemHold.ToString() && t.Type != WarehouseType.Consignment.ToString() && t.Type != WarehouseType.Frozen.ToString() select t).ToList<Warehouse>();
                        break;
                    case DealerType.LS:
                        //与平台相同
                        list = (from t in list where t.Type != WarehouseType.SystemHold.ToString() && t.Type != WarehouseType.Consignment.ToString() && t.Type != WarehouseType.Frozen.ToString() select t).ToList<Warehouse>();
                    ...
                    case DealerType.T2:
                        //二级经销商，显示普通、借货
                        list = (from t in list where t.Type != WarehouseType.SystemHold.ToString() && t.Type != WarehouseType.LP_Consignment.ToString() && t.Type != WarehouseType.Consignment.ToString() && t.Type != WarehouseType.Frozen.ToString() select t).ToList<Warehouse>();
                        break;
                    default: break;
                }
