...
DROP FUNCTION [Contract].[Func_GetRenewalApproveXml]
GO


CREATE FUNCTION [Contract].[Func_GetRenewalApproveXml]
(
...
BEGIN
  ...
  FROM   [Contract].RenewalMain A,
         [Contract].RenewalProposals C,
         DealerMaster D
  WHERE  A.ContractId = C.ContractId
 AND A.CompanyID = D.DMA_ID
         AND A.ContractId = @ContractId;
  
  SET @COAPPROVE = '<a href="' + dbo.Func_GetCode('CONST_CONTRACT_ContractBaseUrl', 'BaseUrl') + 'Contract/PagesEwf/Contract/RenewalApprove.aspx?ContractId=' + @APPLICATIONID + '&UserRole=CO" target="_blank">页面更改链接</a>';
  SET @CREDITAPPROVE = '<a href="' + dbo.Func_GetCode('CONST_CONTRACT_ContractBaseUrl', 'BaseUrl') + 'Contract/PagesEwf/Contract/RenewalApprove.aspx?ContractId=' + @APPLICATIONID + '&UserRole=CC" target="_blank">页面更改链接</a>';
  SET @COIAFAPPROVE = '<a href="' + dbo.Func_GetCode('CONST_CONTRACT_ContractBaseUrl', 'BaseUrl') + 'Contract/PagesEwf/Contract/RenewalApprove.aspx?ContractId=' + @APPLICATIONID + '&UserRole=COA" target="_blank">页面更改链接</a>';
  SET @FINAPPROVE = '<a href="' + dbo.Func_GetCode('CONST_CONTRACT_ContractBaseUrl', 'BaseUrl') + 'Contract/PagesEwf/Contract/RenewalApprove.aspx?ContractId=' + @APPLICATIONID + '&UserRole=FIN" target="_blank">页面更改链接</a>';
  
  SELECT @CUSTXML = [Contract].Func_GetRenewalHtml(@ContractId);
  
  SET @Rtn = '';
  SET @Rtn += '<Data>';
  SET @Rtn += '<FlowId>' + dbo.Func_GetCode('CONST_CONTRACT_EwfFlowId', 'Contract_Renewal') + '</FlowId>';
  SET @Rtn += '<IgnoreAlarm>1</IgnoreAlarm>';
  SET @Rtn += '<Initiator>' + @EID + '</Initiator>';
  SET @Rtn += '<ApproveSelect/>';
  SET @Rtn += '<Principal/>';
  SET @Rtn += '<Tables>';
  SET @Rtn += '<Table Name="BIZ_DEALER_RENEW_NEWMAIN">';
  SET @Rtn += '<R Index="1">';
