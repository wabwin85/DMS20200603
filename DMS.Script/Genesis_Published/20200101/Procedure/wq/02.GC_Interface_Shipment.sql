USE [GenesisDMS_PRD]
GO
/****** Object:  StoredProcedure [dbo].[GC_Interface_Shipment]    Script Date: 2019/12/31 17:04:08 ******/
SET ANSI_NULLS ON
GO
	  ...
	  --更新订单发货状态
		...
		WHILE @@FETCH_STATUS = 0
			BEGIN
				...
				FROM PurchaseOrderHeader poh
				WHERE poh.POH_OrderNo=@OrderNO
				AND EXISTS(SELECT 1 FROM PurchaseOrderDetail WHERE POD_ReceiptQty<POD_RequiredQty AND POD_POH_ID=poh.POH_ID)

				UPDATE 	poh set poh.POH_OrderStatus='Completed'
				FROM PurchaseOrderHeader poh
				WHERE poh.POH_OrderNo=@OrderNO
				AND NOT EXISTS(SELECT 1 FROM PurchaseOrderDetail WHERE POD_ReceiptQty<POD_RequiredQty AND POD_POH_ID=poh.POH_ID)
