...
/****** Object:  StoredProcedure [dbo].[GC_Interface_Shipment]    Script Date: 2020/3/18 16:36:23 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
ALTER PROCEDURE [dbo].[GC_Interface_Shipment]
...
AS
   ...
   BEGIN TRY
      ...
      (
		 ...
		 [POSubCompanyId]		UNIQUEIDENTIFIER NULL,
		 [POBrandId]		UNIQUEIDENTIFIER NULL
      )
      ...
      (
		 ...
		 [LotN]             NVARCHAR (50) COLLATE Chinese_PRC_CI_AS NULL,
		 [QRCode]           NVARCHAR (50) COLLATE Chinese_PRC_CI_AS NULL,
		 [ERPLineNbr]       NVARCHAR (50) COLLATE Chinese_PRC_CI_AS NULL,
		 [ERPAmount]        DECIMAL(18,6) NULL,
		 [ERPTaxRate]       DECIMAL(6,2) NULL,
		 [ERPNbr]       NVARCHAR (50) COLLATE Chinese_PRC_CI_AS NULL,
      ...
      (
		...
		[DNL_QRCode] nvarchar(30) collate Chinese_PRC_CI_AS NULL,
		[DNL_ExpressCompany] nvarchar(200) collate Chinese_PRC_CI_AS NULL,
		[DNL_ExpressNo] nvarchar(200) collate Chinese_PRC_CI_AS NULL,
		[DNL_ERPLineNbr]       NVARCHAR (50) COLLATE Chinese_PRC_CI_AS NULL,
		[DNL_ERPAmount]        DECIMAL(18,6) NULL,
		[DNL_ERPTaxRate]       DECIMAL(6,2) NULL,
		[DNL_ERPNbr]       NVARCHAR (50) COLLATE Chinese_PRC_CI_AS NULL	
      )
      ...
      (
		...
		[ISH_ExpressCompany] nvarchar(200) collate Chinese_PRC_CI_AS NULL,
		[ISH_ExpressNo] nvarchar(200) collate Chinese_PRC_CI_AS NULL,
		[ISH_ShipType] nvarchar(200),
		[ISH_ERPLineNbr]       NVARCHAR (50) COLLATE Chinese_PRC_CI_AS NULL,
		[ISH_ERPAmount]        DECIMAL(18,6) NULL,
		[ISH_ERPTaxRate]       DECIMAL(6,2) NULL,
		[ISH_ERPNbr]       NVARCHAR (50) COLLATE Chinese_PRC_CI_AS NULL			   
      )
    ...
    INSERT INTO #IShipment (
           ISH_ID,ISH_Dealer_SapCode,ISH_OrderNo,ISH_SapDeliveryNo,ISH_SapDeliveryDate,ISH_ShippingDate,ISH_ToWhmCode,ISH_ArticleNumber,ISH_SapDeliveryLineNbr,ISH_LotNumber,ISH_ExpiredDate,ISH_DeliveryQty,ISH_LineNbr,ISH_FileName,ISH_ImportDate,ISH_ClientID,ISH_BatchNbr,ISH_ShipmentType,ISH_UnitPrice,ISH_QRCode,ISH_ExpressCompany,ISH_ExpressNo,ISH_ShipType,ISH_ERPLineNbr,ISH_ERPAmount,ISH_ERPTaxRate,ISH_ERPNbr)
    ...
    select @CNT = COUNT(*) 
      from (
		     ...
		     LEFT JOIN #DealerComplainBSC AS DCBSC on REPLACE(REPLACE(ISH_OrderNo,'CR',''),'PART','') = DCBSC.COMPLAINTID and IShip.ISH_ArticleNumber = DCBSC.UPN
            ...
            UNION
		        ...
		        SELECT IShip.ISH_ID 
              FROM #IShipment AS IShip
		     LEFT JOIN #DealerComplainCRM AS DCCRM on ( REPLACE(REPLACE(ISH_OrderNo,'CR',''),'PART','') = DCCRM.IAN OR REPLACE(REPLACE(ISH_OrderNo,'CR',''),'PART','') = DCCRM.PI ) and IShip.ISH_ArticleNumber = DCCRM.Serial
          ...
          NOT EXISTS
					...
					AND NOT EXISTS 
               (SELECT 1 FROM POReceiptHeader(nolock) --POReceiptHeader_SAPNoQR
      ...
      SELECT POH_ID FROM PurchaseOrderHeader(nolock)
       WHERE POH_OrderNo IN (SELECT DISTINCT ISH_OrderNo
									            ...
									            WHERE EXISTS (SELECT 1
              													      FROM PurchaseOrderHeader(nolock)
              													     WHERE POH_OrderType IN ('ClearBorrow','ClearBorrowManual','ZTKA','ZTKB','ConsignmentReturn')
              														     AND POH_OrderNo = ISH_OrderNo
                                           )
                            )
				 AND POH_OrderType IN ('ClearBorrow', 'ClearBorrowManual','ZTKA','ZTKB','KA')
    			...
    			SELECT NEWID (), POH.POH_ID ,@SysUserId,GETDATE (),'Confirm','SAP确认订单,SAP发货单：'+isnull(ISH_SapDeliveryNo,'')
    			 ...
    			 WHERE ISH.ISH_OrderNo=POH.POH_OrderNo
    			   ...
    			   AND POH.POH_OrderNo IN (SELECT DISTINCT ISH_OrderNo
        										          ...
        										          WHERE EXISTS (SELECT 1
        														                  FROM PurchaseOrderHeader(nolock)
        													                   WHERE POH_OrderType IN ('ClearBorrow','ClearBorrowManual','ZTKA','ZTKB','KA')
        															                 AND POH_OrderNo = ISH_OrderNo))
    				 AND POH_OrderType IN ('ClearBorrow', 'ClearBorrowManual','ZTKA','ZTKB','KA')
    		   ...
    		   WHERE Header.POH_ID = Detail.POD_POH_ID
    				 AND Header.POH_OrderNo IN (SELECT DISTINCT ISH_OrderNo
    											               ...
    											               WHERE EXISTS (SELECT 1
                          															...
                          															WHERE POH_OrderType IN ('ClearBorrow','ClearBorrowManual','ZTKA','ZTKB','KA')
                          																AND POH_OrderNo = ISH_OrderNo))
    				 AND Header.POH_OrderType IN ('ClearBorrow', 'ClearBorrowManual','ZTKA','ZTKB','KA')
      ...
      ELSE
        BEGIN
          ...
          --先不判断产品分类 Edit By SongWeiming on 2017-07-03
			  ...
			  --更新经销商ID
    		  UPDATE #tmp_DN
    			   SET DNL_DealerID_DMA_ID = dma.DMA_ID,
    				     DNL_HandleDate = getdate ()
    			  FROM DealerMaster dma(nolock)
				  INNER JOIN DealerMasterMain dmm on dma.DMA_DMM_ID=dmm.DMM_ID
    		   WHERE (dmm.DMM_ERP_Code = #tmp_DN.DNL_SAPCode)
    				 AND #tmp_DN.DNL_DealerID_DMA_ID IS NULL
    				 AND #tmp_DN.DNL_POReceiptLot_PRL_ID IS NULL
    				 AND #tmp_DN.DNL_BatchNbr = @BatchNbr
					 AND dma.DMA_SubCompanyId in(select SubCompanyId from view_productLine where id=#tmp_DN.DNL_ProductLine_BUM_ID)
    ...
    --										 AND LM.LTM_Product_PMA_ID = t2.PMA_ID)
    				...
    				INSERT INTO #tmpPORImportHeader (ID,
    				   ...
    				   SELECT NEWID (),
    					 ...
    					 FROM (SELECT DNL_PONbr,
    								  ...
    								  DNL_ShipmentDate,
    								  DNL_ProductLine_BUM_ID,
    								  max (DNL_ExpressNo) AS TrackingNo,
    								  DNL_SapDeliveryDate AS SapDeliveryDate,
    								  max (DNL_BUName) AS BUName,
    								  max (DNL_ExpressCompany) AS Carrier,
    				...
    				INSERT INTO #tmpPORImportLot (LotRecID,
    				   ...
    				   SELECT NEWID (),
    						  ...
    						  #tmp_DN.DNL_LotNumber+'@@'+#tmp_DN.DNL_QRCode,
							  ...
							  #tmp_DN.DNL_QRCode,
							  #tmp_DN.DNL_ERPLineNbr,
							  #tmp_DN.DNL_ERPAmount,
							  #tmp_DN.DNL_ERPTaxRate,
							  #tmp_DN.DNL_ERPNbr,
							  #tmp_DN.DNL_TrackingNo
    				   ...
    				   IF (@@fetch_status <> -2)
    					  BEGIN
    						 ...
    						 UPDATE #tmpPORImportHeader
    							SET PONumber = @PONumber,POSubCompanyId=@SubCompanyId,POBrandId=@BrandId
    						  WHERE ID = @ID
    					  END
                    ...
                    FROM (
                          ...
                          SELECT ISH_ID, 
                                 ...
                                 ISNULL(dc.CONFIRMRETURNTYPE,dc.RETURNTYPE) as RETURNTYPE 
          					        FROM #IShipment LEFT JOIN #DealerComplainBSC dc ON REPLACE(REPLACE(ISH_OrderNo,'CR',''),'PART','') = COMPLAINTID AND ISH_ArticleNumber = DC.UPN
                          ...
                          SELECT ISH_ID,  
                                 CASE WHEN DC.WHMID = '00000000-0000-0000-0000-000000000000' 
                                      ...
                                      END AS WHM_Type,
              					         RETURNTYPE
              					    FROM #IShipment LEFT JOIN #DealerComplainCRM dc ON ( REPLACE(REPLACE(ISH_OrderNo,'CR',''),'PART','') = dc.IAN OR REPLACE(REPLACE(ISH_OrderNo,'CR',''),'PART','') = dc.PI ) AND ISH_ArticleNumber = dc.Serial
    						...
    						BEGIN
                               ...
                               from (
                                     ...
                                     SELECT DISTINCT DM.DMA_ID,W.WHM_ID
            											     FROM #tmp_DN
    												          INNER JOIN #DealerComplainBSC AS dc(nolock) on REPLACE(REPLACE(DNL_PONbr,'CR',''),'PART','') = COMPLAINTID and DNL_CFN = dc.UPN
                                      ...
                                      --CRM
    									                SELECT DISTINCT DM.DMA_ID,W.WHM_ID
    												            FROM #tmp_DN
    												           INNER JOIN #DealerComplainCRM AS dc on ( REPLACE(REPLACE(DNL_PONbr,'CR',''),'PART','') = dc.IAN OR REPLACE(REPLACE(DNL_PONbr,'CR',''),'PART','') = dc.PI ) AND DNL_CFN = dc.Serial		
        	...
        	INSERT INTO DeliveryNote(DNL_ID, DNL_LineNbrInFile, DNL_ShipToDealerCode, DNL_SAPCode, DNL_PONbr, DNL_DeliveryNoteNbr, DNL_CFN, DNL_UPN, DNL_LotNumber, DNL_ExpiredDate, DNL_DN_UnitOfMeasure, DNL_ReceiveUnitOfMeasure, DNL_ShipQty, DNL_ReceiveQty, DNL_ShipmentDate, DNL_ImportFileName, DNL_OrderType, DNL_UnitPrice, DNL_SubTotal, DNL_ShipmentType, DNL_CreatedDate, DNL_ProblemDescription, DNL_ProductDescription, DNL_SAPSOLine, DNL_SAPSalesOrderID, DNL_POReceiptLot_PRL_ID, DNL_HandleDate, DNL_DealerID_DMA_ID, DNL_CFN_ID, DNL_BUName, DNL_Product_PMA_ID, DNL_ProductLine_BUM_ID, DNL_Authorized, DNL_CreateUser, DNL_Carrier, DNL_TrackingNo, DNL_ShipType, DNL_Note, DNL_ProductCatagory_PCT_ID, DNL_SapDeliveryDate, DNL_LTM_ID, DNL_ToWhmCode, DNL_ToWhmId, DNL_ClientID, DNL_SapDeliveryLineNbr, DNL_BatchNbr, DNL_FromWhmId)
              ...
              --已存在更新生产日期及有效期，解决导入错误重新导入问题
				UPDATE lm
				SET LTM_Type = DNL_TrackingNo,LTM_ExpiredDate = DNL_ExpiredDate
				FROM LotMaster lm
				INNER JOIN #tmp_DN tdn on lm.LTM_LotNumber = tdn.DNL_LotNumber + '@@' + tdn.DNL_QRCode AND lm.LTM_Product_PMA_ID = tdn.DNL_Product_PMA_ID
              --写入NoQR的数据
              INSERT INTO LotMaster (
						 LTM_InitialQty
						,LTM_ExpiredDate
					  ...
					  select Qty,DNL_ExpiredDate,DNL_LotNumber + '@@' + DNL_QRCode,newid(),getdate(),null,DNL_Product_PMA_ID,DNL_TrackingNo,null,DNL_QRCode,DNL_LotNumber from (
					  SELECT SUM(DN.DNL_ReceiveQty) AS QTY ,DN.DNL_ExpiredDate,DN.DNL_LotNumber ,DN.DNL_Product_PMA_ID,DN.DNL_QRCode,DNL_TrackingNo     
						FROM #tmp_DN AS DN
					   WHERE NOT EXISTS				 (SELECT 1
    													  FROM LotMaster AS LM(nolock)
    												   WHERE LM.LTM_LotNumber = DN.DNL_LotNumber + '@@' + DN.DNL_QRCode 
    												     and LM.LTM_Product_PMA_ID = DN.DNL_Product_PMA_ID)      
					   GROUP BY DN.DNL_LotNumber ,DN.DNL_ExpiredDate,DN.DNL_Product_PMA_ID,DN.DNL_QRCode,DNL_TrackingNo
					  ) tab
			...
			--写入二维码数据
              INSERT INTO Qrcodemaster (
					  ...
					  select distinct
						 ...
						 getdate()
					  FROM #tmp_DN
					  where ISNULL(DNL_QRCode,'NoQR')<>'NoQR'
					  AND NOT EXISTS(
						SELECT 1 FROM Qrcodemaster WHERE QRM_QRCode = DNL_QRCode
					  )
            --对于样品的发货，需要更新
...
--    										  AND LM.LTM_Product_PMA_ID = t2.PMA_ID)
	  ...
	  --更新订单发货状态
		...
		DECLARE @OrderQty decimal(18,6)
		DECLARE @DeliveryQty decimal(18,6)
		DECLARE cursor_Order CURSOR FOR --定义游标
			SELECT DISTINCT ISH_OrderNo FROM #IShipment
		OPEN cursor_Order --打开游标
		FETCH NEXT FROM cursor_Order INTO @OrderNO  --抓取下一行游标数据
		WHILE @@FETCH_STATUS = 0
			BEGIN
				SET @OrderQty=0;
				SET @DeliveryQty=0;
				select @OrderQty=SUM(POD_RequiredQty) from PurchaseOrderDetail
				WHERE POD_POH_ID IN(SELECT POH_ID FROM PurchaseOrderHeader WHERE POH_OrderNo=@OrderNO
				AND POH_LastVersion=0)

				select @DeliveryQty=SUM(POR_ReceiptQty) from POReceipt
				WHERE POR_PRH_ID IN(SELECT PRH_ID FROM POReceiptHeader WHERE PRH_PurchaseOrderNbr = @OrderNO)

				--UPDATE 	poh set poh.POH_OrderStatus='Delivering'
				--FROM PurchaseOrderHeader poh
				--WHERE poh.POH_OrderNo=@OrderNO
				--AND @DeliveryQty<@OrderQty
				
				--UPDATE 	poh set poh.POH_OrderStatus='Completed'
				--FROM PurchaseOrderHeader poh
				--WHERE poh.POH_OrderNo=@OrderNO
				--AND @DeliveryQty>=@OrderQty
				...
				--更新发货数量
				UPDATE t
				SET t.POD_ReceiptQty = (SELECT SUM(POR_ReceiptQty) 
										FROM POReceipt 
										WHERE POR_PRH_ID IN(SELECT PRH_ID FROM POReceiptHeader WHERE PRH_PurchaseOrderNbr=@OrderNO)
										AND POR_SAP_PMA_ID IN(SELECT PMA_ID FROM Product WHERE PMA_CFN_ID = t.POD_CFN_ID))
				FROM PurchaseOrderDetail t
				WHERE POD_POH_ID IN(SELECT POH_ID FROM PurchaseOrderHeader WHERE POH_OrderNO=@OrderNO)

				IF EXISTS(SELECT 1 FROM PurchaseOrderDetail pod
				INNER JOIN PurchaseOrderHeader poh ON pod.POD_POH_ID=poh.POH_ID
				WHERE ISNULL(pod.POD_ReceiptQty,0)<ISNULL(pod.POD_RequiredQty,0) AND poh.POH_OrderNo=@OrderNO)
				BEGIN 
					UPDATE 	poh set poh.POH_OrderStatus='Delivering'
					FROM PurchaseOrderHeader poh
					WHERE poh.POH_OrderNo=@OrderNO
					...
					--记录订单日志
					INSERT INTO PurchaseOrderLog(POL_ID,POL_POH_ID,POL_OperUser,POL_OperDate,POL_OperType,POL_OperNote)
					Values(NEWID(),ISNULL((SELECT POH_ID FROM PurchaseOrderHeader WHERE POH_OrderNo=@OrderNO AND POH_CreateType = 'Manual'),'00000000-0000-0000-0000-000000000000'),'00000000-0000-0000-0000-000000000000'
					,GETDATE(),'UpdateOrderStatus','通过发货数量自动更新订单状态为：部分发货')
				END
				ELSE
				BEGIN
					UPDATE 	poh set poh.POH_OrderStatus='Completed'
					FROM PurchaseOrderHeader poh
					WHERE poh.POH_OrderNo=@OrderNO
					--AND NOT EXISTS(SELECT 1 FROM PurchaseOrderDetail WHERE ISNULL(POD_ReceiptQty,0)<ISNULL(POD_RequiredQty,0) AND POD_POH_ID=poh.POH_ID)
					--记录订单日志
					INSERT INTO PurchaseOrderLog(POL_ID,POL_POH_ID,POL_OperUser,POL_OperDate,POL_OperType,POL_OperNote)
					Values(NEWID(),ISNULL((SELECT POH_ID FROM PurchaseOrderHeader WHERE POH_OrderNo=@OrderNO AND POH_CreateType = 'Manual'),'00000000-0000-0000-0000-000000000000'),'00000000-0000-0000-0000-000000000000'
					,GETDATE(),'UpdateOrderStatus','通过发货数量自动更新订单状态为：已完成')
				END

				FETCH NEXT FROM cursor_Order INTO @OrderNO
