...
{
    public class OrderCfnDialogLPPickerService : ABaseQueryService
    {
        public string Query(OrderCfnDialogLPPickerVO model)
        {
            try
            {
                ...
                //参数(新增)：清指定批号订单
                if (!string.IsNullOrEmpty(model.hidOrderTypeId))
                {
                    if (model.hidOrderTypeId.Equals("ClearBorrow") || model.hidOrderTypeId.Equals("ClearBorrowManual"))
                    {
                ...
                {
                    param.Add("OrderType", model.hidOrderTypeId);            
                    ds = business.QueryCfnForPurchaseOrderByAuth(param, start, model.PageSize, out totalCount);
                }
