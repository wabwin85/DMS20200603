DROP view [interface].[V_I_QV_DealerInventory_WithQR]

GO



CREATE view [interface].[V_I_QV_DealerInventory_WithQR]
as
--是否需要过滤停用经销商的库存
       ...
       when IsForReceipt=0 and DMA_DealerType='T1' and i.WHM_Type='Frozen' then 34   --一级的冻结库
       when IsForReceipt=0 and DMA_DealerType='T2' and i.WHM_Type='Frozen' then 35   --二级的冻结库
      ...
      ,DATEDIFF(day,TransferDate,DATEADD(month,1,convert(datetime,convert(nvarchar(6),GETDATE(),112)+'01'))) AS TransferDays
			,ReceiptDate
			--,DATEDIFF(day,ReceiptDate,getdate()) AS ReceiptDays
      ,DATEDIFF(day,ReceiptDate,DATEADD(month,1,convert(datetime,convert(nvarchar(6),GETDATE(),112)+'01'))) AS ReceiptDays
  FROM interface.V_I_CR_Inventory i
