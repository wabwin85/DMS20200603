...
DealerInvAdjust = function () {
    var that = {};

    var business = 'Report.DealerInvAdjust';
    ...
    that.Init = function () {
        ...
        FrameUtil.SubmitAjax({
            ...
            callback: function (model) {
                ...
                $('#QryInvAdjustDate').FrameDatePickerRange({
                    value: model.QryInvAdjust
                });
                //产品型号
                $('#QryCFN').FrameTextBox({
                    value: model.QryCFN
                ...
                $('#BtnExportInvAdjust').FrameButton({
                    text: '导出',
                    icon: 'file-excel-o',
                    onClick: function () {
                        that.ExportInvAdjust();
                    }
                });                                
    ...
    var createResultList = function () {
        $("#RstResultList").kendoGrid({
            ...
            columns: [
                ...
                {
                    field: "IAH_Inv_Adj_Nbr", title: "库存调整单号", width: 150,
                    headerAttributes: { "class": "text-center text-bold", "title": "库存调整单号" },
                    attributes: { "class": "table-td-cell" }
                },
                {
                    field: "AdjustTypeName", title: "调整类型", width: 80,
                    headerAttributes: { "class": "text-center text-bold", "title": "调整类型" },
                    attributes: { "class": "table-td-cell" }
                },
                {
                ...
                {
                    field: "IAH_ApprovalDate", title: "调整日期", width: 100,
                ...
                {
                    field: "IAL_LotQty", title: "调整数量", width: 80,
                    headerAttributes: { "class": "text-center text-bold", "title": "调整数量" },
                    attributes: { "class": "table-td-cell" }
                },                
                {
                    field: "IAH_UserDescription", title: "调整原因", width: 150,
    ...
    that.ExportInvAdjust = function () {
        var data = that.GetModel();

        var urlExport = Common.ExportUrl;
        urlExport = Common.UpdateUrlParams(urlExport, 'Business', business);
        urlExport = Common.UpdateUrlParams(urlExport, 'DownloadCookie', 'DealerInvAdjustExportByDealer');
        urlExport = Common.UpdateUrlParams(urlExport, 'ExportType', 'ExportDealerInvAdjust');
        ...
        urlExport = Common.UpdateUrlParams(urlExport, 'QryInvAdjustDateStart', data.QryInvAdjustDate.StartDate);
        urlExport = Common.UpdateUrlParams(urlExport, 'QryInvAdjustDateEnd', data.QryInvAdjustDate.EndDate);
        ...
        startDownload(urlExport, 'DealerInvAdjustExportByDealer');
    }
