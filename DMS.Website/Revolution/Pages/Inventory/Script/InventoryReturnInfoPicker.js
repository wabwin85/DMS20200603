var InventoryReturnInfoPicker = {};

InventoryReturnInfoPicker = function () {
    var that = {};

    var business = 'Inventory.InventoryReturnInfoPicker';
    var pickedList = [];
    ...
    that.Init = function () {
        ...
        $('#ReturnTypeWin').val(Common.GetUrlParam('ReturnTypeWin'));
        $('#QryReturnType').val(Common.GetUrlParam('hiddenReturnType'));
        ...
        data.ReturnTypeWin = $('#ReturnTypeWin').val();
        data.QryReturnType = $('#QryReturnType').val();
            ...
            callback: function (model) {
                ...
                if (model.IsCFN) {//CFN状态
                    ...
                    $('#QryWorehourse').FrameDropdownList({
                        ...
                        value: wareValue,
                        onChange: function (e) {
                            var type = ""; var woreId = this.value;
                            $.each(RstWarehouseListArr, function (index, val) {
                                if (woreId === val.Id)
                                    type = val.Type;
                            })
                            that.ChangeWoreHourse(type);
                        }
                    });
                        ...
                        if (pickedList.length == 0) {
                            ...
                            FrameWindow.ShowConfirm({
                                ...
                                confirmCallback: function () {
                                    var QryWarehouse = ""; QryWarehouse2 = "";
                                    var model = FrameUtil.GetModel();
                                    if (StatusPicker)
    ...
    that.Query = function () {
        if (!StatusPicker) {
            if ($('#QryWorehourse').FrameDropdownList('getValue').Key == "") {
                FrameWindow.ShowAlert({
                    target: 'top',
                    alertType: 'warning',
                    message: '请选择仓库',
    ...
    that.CreateResultList = function () {
        $("#RstResultList").kendoGrid({
            ...
            columns: [
                ...
                {
                    field: "QRCode", title: "二维码", width: '180px',
                ...
                {
                    field: "LotInvQty", title: "库存数量", width: '100px',
