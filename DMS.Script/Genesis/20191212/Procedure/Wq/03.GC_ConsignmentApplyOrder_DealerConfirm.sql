...
/****** Object:  StoredProcedure [dbo].[GC_ConsignmentApplyOrder_DealerConfirm]    Script Date: 2019/12/5 9:40:03 ******/
SET ANSI_NULLS ON
GO
...
**/
ALTER Procedure [dbo].[GC_ConsignmentApplyOrder_DealerConfirm]
	@CAH_ID uniqueidentifier,
	@UserId uniqueidentifier,
    --@SubCompanyId UNIQUEIDENTIFIER,--20191105,增加分子公司和品牌分组
...
BEGIN TRY
	...
	(
		...
		[PRL_IsCalRebate] [bit] NULL,
		 [PRL_Lot]    NVARCHAR (50) COLLATE Chinese_PRC_CI_AS NULL,
		 [PRL_QrCode]    NVARCHAR (50) COLLATE Chinese_PRC_CI_AS NULL,
		 [PRL_DOM]    NVARCHAR (50) COLLATE Chinese_PRC_CI_AS NULL
	)
	...
	IF @PurchaseOrderNo IS NOT NULL
		BEGIN
			...
			,@SubCompanyId=vp.SubCompanyId,@BrandId=BrandId 
			FROM PurchaseOrderHeader poh
			LEFT JOIN View_ProductLine vp on poh.POH_ProductLine_BUM_ID=vp.Id
			...
			--得到DMS收货单号
			EXEC [GC_GetNextAutoNumber] @DealerId,'Next_POReceiptNbr',@BuName,@SubCompanyId,@BrandId,@POReceiptNo OUTPUT
			...
			SELECT PoLineId,NEWID(),LotNumber,Qty,ExpiredDate,WarehouseId,NULL,NULL,NULL,NULL,NULL FROM #TEMP

			UPDATE #POReceiptLot_TEMP 
			SET PRL_Lot = LTM_Lot,PRL_QrCode = LTM_QRCode,PRL_DOM = LTM_Type
			FROM LotMaster
			WHERE prl_lotnumber = LTM_LotNumber
		END
	ELSE 
		BEGIN
