...
/****** Object:  StoredProcedure [interface].[P_I_EW_Contract_Status_New]    Script Date: 2019/11/19 10:37:25 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

ALTER PROCEDURE [interface].[P_I_EW_Contract_Status_New]
@Contract_ID nvarchar(36), @Contract_Type nvarchar(50), @Contract_Status nvarchar(20)
AS BEGIN
	...
	DECLARE @CON_BEGIN_OLD DATETIME
	...
	/*调用原同步状态程序 */
	EXEC [interface].[P_I_EW_Contract_Approval_New] @Contract_ID,@Contract_Type,null,null
	...
	BEGIN
		...
		DECLARE @DealerSubCompany UNIQUEIDENTIFIER
		...
		SELECT @dealerAccount=DealerMaster.DMA_SAP_Code,@dealerTypeEM=DealerMaster.DMA_DealerType,@DealerSubCompany=DealerMaster.DMA_SubCompanyId
		...
		SET @CompanyAbbr=ISNULL((SELECT TOP 1 REV1 FROM dbo.Lafite_ATTRIBUTE WHERE ATTRIBUTE_TYPE='SUBCOMPANY' AND Id=@DealerSubCompany),'BP');
		SET @dealerAccount=@CompanyAbbr+'_'+ISNULL(@dealerAccount,'')+'_01';
		...
		BEGIN
			...
			BEGIN
				...
				SET @mailName=@Division+'-'+@CC_NameCN+'-'+isnull(@dealerName,'')+'-'+'Appointment'+'('+@dealerTypeEM+')-'+isnull(@mailName,'')
				INSERT INTO MailMessageQueue([MMQ_ID],[MMQ_QueueNo] ,[MMQ_From] ,[MMQ_To],[MMQ_Subject],[MMQ_Body],[MMQ_Status],[MMQ_CreateDate],[MMQ_SendDate])
		...
		BEGIN
			...
			BEGIN
				...
				SET @mailBodyLP=REPLACE(REPLACE(REPLACE(REPLACE(ISNULL(@mailBodyLP,''),'{#DealerNameLP}',ISNULL(@dealerNameLP,'')),'{#DealerName}',ISNULL(@dealerName,'')),'{#ApplicantUser}',ISNULL(@ApplicantUser,'')),'{#DealerAccount}',isnull(@dealerAccount,''));
				SET @mailNameLP=ISNULL(@Division,'')+'-'+ISNULL(@CC_NameCN,'')+'-'+ISNULL(@dealerName,'')+'-'+'Appointment'+'('+ISNULL(@dealerTypeEM,'')+')-'+@mailNameLP
				...
				DECLARE @MailTitalT2 NVARCHAR(200)
				DECLARE @MailBodyT2 NVARCHAR(max)
				DECLARE @MailAddressT2 NVARCHAR(200)
				SELECT @MailAddressT2= DMA_Email FROM DealerMaster A WHERE A.DMA_ID=@DMA_ID
				SELECT @MailTitalT2=MMT_Subject,@MailBodyT2=MMT_Body FROM MailMessageTemplate  WHERE MMT_Code='EMAL_DCMS_NewDealerContract_Dealer'
				SET @MailBodyT2=REPLACE(REPLACE(@MailBodyT2,'{#DealerAccount}',@DealerAccount),'{#DealerName}',ISNULL(@DealerName,''))
				IF ISNULL(@MailAddressT2,'')<>''
				BEGIN
					INSERT INTO MailMessageQueue (MMQ_ID,MMQ_QueueNo,MMQ_From,MMQ_To,MMQ_Subject,MMQ_Body,MMQ_Status,MMQ_CreateDate,MMQ_SendDate)
					VALUES(NEWID(),'email','',@MailAddressT2,@MailTitalT2,@MailBodyT2,'Waiting',GETDATE(),NULL)
				END
		...
		IF(@Contract_Status='Completed')
		BEGIN
			SELECT @DMA_ID=CAP_DMA_ID,@CON_BEGIN=CAP_EffectiveDate,@CON_END=CAP_ExpirationDate,@Division=CAP_Division,@CON_BEGIN_OLD=CAP_BeginDateOld FROM  ContractAppointment WHERE CAP_ID= @Contract_ID
			
			INSERT INTO [interface].[T_I_EW_ContractState] (ContractId,BeginDate,ContractState,ContractType,SynState)
			VALUES(@Contract_ID,isnull(@CON_BEGIN,getdate()),'Completed','Appointment','0')
		END
	...
	BEGIN
		...
		BEGIN
			...
			SELECT @DMA_ID=CAM_DMA_ID,@Amd_DelerName=DealerMaster.DMA_ChineseName,@CON_BEGIN=CAM_Amendment_EffectiveDate,@CON_END=CAM_Agreement_ExpirationDate,@CON_BEGIN_OLD=CAM_BeginDateOld,@Division=CAM_Division ,@Amd_DelerType=DealerMaster.DMA_DealerType
			...
			BEGIN	
				...
				SET @Amd_email_Body=REPLACE(REPLACE(REPLACE(@Amd_email_Body,'{#ProductLine}',ISNULL(@Amd_email_Pl,'')),'{#SubBU}',ISNULL(@CC_NameCN,'')),'{#ApplicantUser}',ISNULL(@ApplicantUser,''));
			...
			BEGIN
				...
				SET @Amd_email_Body=REPLACE(REPLACE(REPLACE(REPLACE(ISNULL(@Amd_email_Body,''),'{#ProductLine}',ISNULL(@Amd_email_Pl,'')),'{#DealerName}',ISNULL(@Amd_DelerName,'')),'{#SubBU}',ISNULL(@CC_NameCN,'')),'{#ApplicantUser}',ISNULL(@ApplicantUser,''));
				SET @Amd_email_Name=@Division+'-'+@CC_NameCN+'-'+@Amd_DelerName+'-'+'Amendment'+'('+@Amd_DelerType+')-'+@Amd_email_Name
			...
			SET @Amd_email_Body=REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(ISNULL(@Amd_email_Body,''),'{#ContractType}','Amendment'),'{#DealerName}',ISNULL(@Amd_DelerName,'')),'{#ProductLine}',ISNULL(@Amd_email_Pl,'')),'{#SubBU}',ISNULL(@CC_NameCN,'')),'{#ApplicantUser}',ISNULL(@ApplicantUser,''));
	...
	BEGIN
		...
		--获取参数
		SELECT @DMA_ID=CRE_DMA_ID,@CON_BEGIN=CRE_Agrmt_EffectiveDate_Renewal,@CON_END=CRE_Agrmt_ExpirationDate_Renewal,@CON_BEGIN_OLD=CRE_BeginDateOld,@Division=CRE_Division 
		...
		BEGIN
			...
			BEGIN
				...
				SET @emailBody_Ren=REPLACE(REPLACE(REPLACE(@emailBody_Ren,'{#ProductLine}',ISNULL(@ProductLine_Ren,'')),'{#SubBU}',ISNULL(@CC_NameCN,'')),'{#ApplicantUser}',ISNULL(@ApplicantUser,''));
			END
			IF (@Contract_Status='Completed')
			BEGIN
				--同Amdment
				...
				SET @emailBody_Ren=REPLACE(REPLACE(REPLACE(ISNULL(@emailBody_Ren,''),'{#ProductLine}',ISNULL(@ProductLine_Ren,'')),'{#SubBU}',ISNULL(@CC_NameCN,'')),'{#ApplicantUser}',ISNULL(@ApplicantUser,''));
			END
	...
	BEGIN
		...
		END
		--EXEC [dbo].[GC_UpdateProcessIndex] @Contract_ID,@Contract_Type,@CON_BEGIN,@CON_BEGIN_OLD,NULL,NULL
		EXEC [dbo].[GC_UpdateProcessIndex_New]@Contract_ID,@Contract_Type,@CON_BEGIN
	END
