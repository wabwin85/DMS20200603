...
/****** Object:  StoredProcedure [Contract].[Proc_ApproveContract]    Script Date: 2019/11/26 15:54:52 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

ALTER PROCEDURE [Contract].[Proc_ApproveContract](
	...
	BEGIN TRY
		...
		BEGIN
		    ...
		    SET    --ContractStatus  = @ContractStatus,
				   ContractStatus  = Case @ContractStatus WHEN 'InApproval' Then ContractStatus ELSE @ContractStatus END,
		...
		BEGIN
			...
			IF @ContractType='Appointment' OR @ContractType='Renewal'
			BEGIN
				EXEC [dbo].[GC_NoticeUploadDD_SendEmail] @ContractId,@ContractType
			END
		END
		...
		BEGIN
		    ...
		    DECLARE @DealerCode NVARCHAR(10);
		    SELECT TOP 1 @DealerCode=ISNULL(C.DMA_SAP_Code,'') FROM Contract.AppointmentMain A 
		    INNER JOIN Contract.AppointmentCandidate B ON A.ContractId=b.ContractId	
		    INNER JOIN DealerMaster C ON C.DMA_ChineseName=B.CompanyName AND a.DealerType=c.DMA_DealerType
		    INNER JOIN DealerMaster D ON D.DMA_ID=C.DMA_Parent_DMA_ID AND D.DMA_SAP_Code=B.LPSAPCode
		    WHERE A.ContractId=@ContractId AND C.DMA_ActiveFlag=1;
			...
			'您好，<br/>'+TempTB.EName+' 申请的经销商'+ContractType+'合同审批完成。<br/><br/>&nbsp;&nbsp;&nbsp;经销商名称：'+DealerName+'('+ISNULL(DealerCode,@DealerCode)+')<br/>&nbsp;&nbsp;&nbsp;合同编号：'+ContractNo+'<br/>&nbsp;&nbsp;&nbsp;产品线：'+D.DivisionName+'<br>&nbsp;&nbsp;&nbsp;合同分类:'+E.CC_NameCN+'<br/><br/>详情请访问<b>“经销商管理系统（DMS）- 合同管理”</b>模块查看。<be/>谢谢！'
			...
			--T2终止审批通过发送邮件给平台对应所有账号
			IF(@DealerType='T2')--@ContractType='Termination' AND 
			BEGIN
				...
				SELECT newid(),'email',
					  '',
					  lid.Email1,
					  '经销商合同'+Case @ContractType WHEN 'Appointment' THEN '申请'WHEN'Amendment'THEN'修改'WHEN'Renewal'THEN'续约'ELSE'终止'END+'审批完成通知',
						'您好，<br/>'+libc.IDENTITY_Name+' 申请的经销商'+Case @ContractType WHEN 'Appointment' THEN '申请'WHEN'Amendment'THEN'修改'WHEN'Renewal'THEN'续约'ELSE'终止'END+'合同审批完成。<br/><br/>&nbsp;&nbsp;&nbsp;经销商名称：'+dma.DMA_ChineseName+'('+dma.DMA_SAP_CODE+')<br/>&nbsp;&nbsp;&nbsp;合同编号：'+tm.ContractNo+'<br/><br/>详情请访问<b>“经销商管理系统（DMS）- 合同管理”</b>模块查看。<be/>谢谢！'
					  +'</table><br/>蓝威医疗<br/>――――――――――――――――――――――――<br/>此邮件为系统自动发送，请勿回复！<br/>谢谢！',
					  'Waiting',
					  getdate (),
					  NULL
				FROM Lafite_IDENTITY lid
				INNER JOIN DealerMaster dmap on lid.corp_ID=dmap.DMA_ID
				INNER JOIN DealerMaster dma on dma.DMA_Parent_DMA_ID = dmap.DMA_ID
				INNER JOIN [Contract].TerminationMain tm on dma.DMA_SAP_Code=tm.dealerName
				LEFT JOIN Lafite_IDENTITY libc on libc.id=tm.createuser
				where tm.ContractId=@ContractId
			END
		    /*End*/
