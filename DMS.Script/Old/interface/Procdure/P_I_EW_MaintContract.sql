DROP Procedure [interface].[P_I_EW_MaintContract]
GO

/*
维护合同主数据
*/
CREATE Procedure [interface].[P_I_EW_MaintContract]
...
BEGIN TRAN
	...
	BEGIN
		...
		INSERT INTO interface.T_I_EW_Contract(SubDepID,
						...
						Attachment,Reason,FormeRname,DealerMark
						)
						SELECT top 1 SubDepID,
						CONVERT(uniqueidentifier,ApplicationID),CONVERT(uniqueidentifier,@UserID),CONVERT(uniqueidentifier,ApplicationID),'Appointment',MARKETTYPE,
						...
						Compete,CompeteRemark,SubDealer,Interest,InterestRemark,Exclusive,ExclusiveRemark,
						Documents.BizLicense,Documents.MedicalLicense,Documents.Tax,
						FCPA.Officials,FCPA.CompanyB,FCPA.ShareholdersB,FCPA.Others,
						InterView.Interview,InterView.Venue,InterView.Interviewee,InterView.Findings,COCTraining,
						(SELECT DEP.DivisionName FROM (
						SELECT Distinct t2.ATTRIBUTE_NAME AS DivisionName,
								 ...
								 WHERE     t1.AttributeType = 'Product_Line'
									   ...
									   AND t1.RootID IN (SELECT AttributeID
														  ...
														  WHERE AttributeType = 'BU')) DEP WHERE DEP.DivisionCode=Convert(NVARCHAR(10),Main.DepId)) as Division
						,Main.EName,Main.ETitle,Main.DealerType,
						...
						--,Comment
						FROM interface.Biz_Dealer_New_Candidate Candidate
						inner join  interface.Biz_Dealer_New_Competency Competency on Candidate.WFINSTANCEID=Competency.WFINSTANCEID
						inner join  interface.Biz_Dealer_New_Conflict Conflict  on Conflict.WFINSTANCEID=Competency.WFINSTANCEID
						inner join  interface.Biz_Dealer_New_Documents Documents on Documents.WFINSTANCEID=Competency.WFINSTANCEID
						inner join interface.Biz_Dealer_New_FCPA FCPA on FCPA.WFINSTANCEID=Competency.WFINSTANCEID
						inner join interface.Biz_Dealer_New_InterView InterView on InterView.WFINSTANCEID=Competency.WFINSTANCEID
						inner join interface.Biz_Dealer_New_Main Main on Main.WFINSTANCEID=Competency.WFINSTANCEID
						inner join interface.Biz_Dealer_New_Proposals  Proposals  on Proposals.WFINSTANCEID=Competency.WFINSTANCEID
						WHERE  ApplicationID=@InstanceID
	...
	BEGIN
		...
		(SubDepID,InstanceID,UserID,ContractID,Contract_Type,DMA_ID,DealerLevel
		 ...
		 SELECT top 1 SubDepID,CONVERT(uniqueidentifier,ApplicationID),CONVERT(uniqueidentifier,@UserID),CONVERT(uniqueidentifier,ApplicationID),'Amendment',(SELECT DMA_ID FROM DealerMaster WHERE DMA_SAP_Code=mian.DealerName), dm.DMA_DealerType,
		 dm.DMA_ChineseName,mian.MarketType,(SELECT DEP.DivisionName FROM (
						SELECT Distinct t2.ATTRIBUTE_NAME AS DivisionName,
									   t2.REV1 AS DivisionCode
								  FROM Cache_OrganizationUnits t1, Lafite_ATTRIBUTE t2
								 WHERE     t1.AttributeType = 'Product_Line'
									   ...
									   AND t1.RootID IN (SELECT AttributeID
														   FROM Cache_OrganizationUnits
														  WHERE AttributeType = 'BU')) DEP WHERE DEP.DivisionCode=Convert(NVARCHAR(10),mian.DepId)) as Division
		,mian.Purpose
		...
		,ncm.Conflict,	ncm.ConflictRemark,ncm.HandoverRemark
		 FROM [interface].[Biz_Dealer_Amend_Main] mian 
		 INNER JOIN [interface].[Biz_Dealer_Amend_Current] tCurrent ON  mian.[WFINSTANCEID]=tCurrent.[WFINSTANCEID]
		 INNER JOIN [interface].[Biz_Dealer_Amend_Proposals] proposals ON tCurrent.[WFINSTANCEID]=proposals.[WFINSTANCEID]
		 INNER JOIN [interface].[Biz_Dealer_Amend_NCM] ncm ON proposals.[WFINSTANCEID]=ncm.[WFINSTANCEID]
		 ...
		 WHERE mian.ApplicationID=@InstanceID
	...
	BEGIN
		...
		SELECT top 1 SubDepID,CONVERT(uniqueidentifier,ApplicationID),CONVERT(uniqueidentifier,@UserID),CONVERT(uniqueidentifier,ApplicationID),'Termination',(SELECT DMA_ID FROM DealerMaster WHERE DMA_SAP_Code=Main.DealerName),(SELECT DMA_DealerType FROM DealerMaster WHERE DMA_SAP_Code=Main.DealerName),
		(SELECT DMA_ChineseName FROM DealerMaster WHERE DMA_SAP_Code=Main.DealerName),Main.MarketType,(SELECT DEP.DivisionName FROM (
						SELECT Distinct t2.ATTRIBUTE_NAME AS DivisionName,
									   t2.REV1 AS DivisionCode
								  FROM Cache_OrganizationUnits t1, Lafite_ATTRIBUTE t2
								 WHERE     t1.AttributeType = 'Product_Line'
		...
		FROM [interface].[Biz_Dealer_End_Main] Main
		INNER JOIN [interface].[Biz_Dealer_End_Status] Staus ON Main.WFINSTANCEID=Staus.WFINSTANCEID
		INNER JOIN [interface].[Biz_Dealer_End_NCM] Ncm ON Ncm.WFINSTANCEID=Main.WFINSTANCEID
		INNER JOIN [interface].[Biz_Dealer_End_Handover]  Handover ON Handover.WFINSTANCEID=Main.WFINSTANCEID
		WHERE Main.ApplicationID=@InstanceID
	END
	...
	BEGIN
		...
		SELECT top 1 SubDepID, CONVERT(uniqueidentifier,ApplicationID),CONVERT(uniqueidentifier,@UserID),CONVERT(uniqueidentifier,ApplicationID),'Renewal',(SELECT DMA_ID FROM DealerMaster WHERE DMA_SAP_Code=Main.DealerName), dm.DMA_DealerType,
		dm.DMA_ChineseName,Main.MarketType,(SELECT DEP.DivisionName FROM (
						SELECT Distinct t2.ATTRIBUTE_NAME AS DivisionName,
									   t2.REV1 AS DivisionCode
								  FROM Cache_OrganizationUnits t1, Lafite_ATTRIBUTE t2
								 WHERE     t1.AttributeType = 'Product_Line'
		...
		,dcm.DCM_Attachment,proposals.Attachment
		--,ncm.Handover
		FROM [interface].[Biz_Dealer_Renew_Main] main
		INNER JOIN [interface].[Biz_Dealer_Renew_Proposals] Proposals ON main.WFINSTANCEID=Proposals.WFINSTANCEID
		INNER JOIN [interface].[Biz_Dealer_Renew_NCM] ncm ON  Proposals.WFINSTANCEID=ncm.WFINSTANCEID
		...
		BEGIN
			...
			BEGIN
				...
				BEGIN
					...
					BEGIN
						...
						BEGIN
							...
							END
						END
						IF @Parent_DMA_ID='2F2D2BD9-6FAE-4A40-BC44-8E5FECC1C6DD'
						BEGIN
							--国药河南
							IF ISNULL(@SAPCode,'')=''
							BEGIN
				...
				SELECT DMA_ID, @SAPCode,@DealerName, @DealerEnName, null,1, @DealerLevel,'0',@EMail
					   ,Company_Type,Mobile_Phone,Contact_Person,Office_Address,Reason,FormeRname
				FROM interface.T_I_EW_Contract
				WHERE InstanceID=@InstanceID AND UserID=@UserID;
			...
			ELSE IF (@drParameters='OLD')
			 BEGIN
					IF(@DMA_ID_Midd IS NULL)
					BEGIN
						SELECT @DMA_ID= DMA_ID FROM DealerMaster dm WHERE dm.DMA_ChineseName=@DealerName and dm.DMA_DealerType=@DealerLevel and dm.DMA_ActiveFlag='1' and (@Parent_DMA_ID is null OR DMA_Parent_DMA_ID= @Parent_DMA_ID)
					END
					ELSE
					BEGIN
			...
			INSERT INTO ContractAppointment 
						(CAP_SubDepID,	CAP_ID,CAP_CM_ID,CAP_DMA_ID,CAP_Division,CAP_MarketType,CAP_Recommender,
							...
							CAP_Attachment,CAP_Reason,CAP_FormeRname )
	...
	IF (@ContractType='Amendment')
		BEGIN
			...
			CAM_Create_Date,CAM_Create_User)
	...
	IF (@ContractType='Renewal')
		BEGIN
			...
			CRE_Create_Date,CRE_Create_User)
	...
	IF (@ContractType='Termination')
		BEGIN
			...
			CTE_Create_Date,CTE_Create_User,CTE_TakeOverNew)
			...
			Create_Date,Create_User,TakeOverNew
			FROM interface.T_I_EW_Contract
			WHERE InstanceID=@InstanceID AND UserID=@UserID;
			...
			SELECT top 1 @InstanceID,@CM_ID,@DMA_ID,'1','1',
			...
			INNER JOIN [interface].[Biz_Dealer_End_Main] main ON main.ApplicationID=ContractTermination.CTE_ID
			INNER JOIN [interface].[Biz_Dealer_End_EndForm] endfrom ON main.WFINSTANCEID=endfrom.WFINSTANCEID
			INNER JOIN [interface].[Biz_Dealer_End_Status] endstatus on endstatus.WFINSTANCEID=main.WFINSTANCEID
