...
*/
ALTER Procedure [dbo].[GC_Interface_Sales]
...
BEGIN TRAN
	...
	if @ClientID is not null
		...
		SELECT @DealerId = Corp_ID FROM Lafite_IDENTITY(nolock) WHERE Id = @UserId
	...
	begin
		INSERT INTO SalesNote (SNL_ID,SNL_ArticleNumber,SNL_UnitPrice,SNL_LotNumber,SNL_LotQty,SNL_SaleDate,
		    ...
		    SNL_BatchNbr,SNL_DMA_ID,SNL_Authorized,SNL_WriteOff,SNL_SalesNo,SNL_ServiceAgent)
		SELECT NEWID(),ITS_ArticleNumber,ITS_UnitPrice,ITS_LotNumber + '@@' + isnull(ITS_QRCode,'NoQR'),ITS_LotQty,ITS_SaleDate,ITS_SaleType,
		       ...
		       ,ITS_SalesNo,ITS_ServiceAgent
		FROM   InterfaceSales(nolock)
		...
		FROM SalesNote A
		INNER JOIN 	CFN(nolock) ON CFN.CFN_CustomerFaceNbr = A.SNL_ArticleNumber
		INNER JOIN Product(nolock) ON Product.PMA_CFN_ID = CFN.CFN_ID
		inner join CfnClassification ccf(nolock) on ccf.CfnCustomerFaceNbr=cfn.CFN_CustomerFaceNbr and ccf.ClassificationId in (select ProducPctId from GC_FN_GetDealerAuthProductSub(a.SNL_DMA_ID))
		...
		--销售单号是否已使用过
		UPDATE SalesNote SET SNL_ProblemDescription = (CASE WHEN SNL_ProblemDescription IS NULL THEN '' ELSE SNL_ProblemDescription + ',' END) + N'销售单号已使用过，不能重复上传'
		WHERE SalesNote.SNL_BatchNbr = @BatchNbr
		AND EXISTS (SELECT 1 FROM ShipmentHeader HD WHERE HD.SPH_ShipmentNbr=SalesNote.SNL_SalesNo AND HD.SPH_Status='Complete')
		
		--同一销售单号包含不同BU产品
		IF (SELECT  COUNT (DISTINCT SNL_BUM_ID) FROM SalesNote WHERE SNL_BatchNbr = @BatchNbr GROUP BY SNL_SalesNo)>1
		BEGIN
			UPDATE SalesNote SET SNL_ProblemDescription = (CASE WHEN SNL_ProblemDescription IS NULL THEN '' ELSE SNL_ProblemDescription + ',' END) + N'同一销售单号包含多个产品线产品'
			WHERE SalesNote.SNL_BatchNbr = @BatchNbr
		END
	end

	--如果@UserId不为空，则表示从ShipmentInitProcessing中读取数据
	if @UserId is not null
		begin
			...
			SELECT NEWID(),I.SPI_HospitalCode,I.SPI_HospitalName, isnull(I.SPI_HospitalOffice,''),
		  	...
		  	FROM ShipmentInitProcessing I(nolock)
	...
	AND NOT EXISTS (SELECT 1 FROM Lot(nolock) INNER JOIN Inventory INV(nolock) ON INV.INV_ID = Lot.LOT_INV_ID
	...
	FROM SalesNote (nolock)
	INNER JOIN Lot(nolock) ON Lot.LOT_LTM_ID = SNL_LTM_ID 
	INNER JOIN Inventory INV(nolock) ON INV.INV_ID = Lot.LOT_INV_ID AND INV.INV_WHM_ID = SNL_WHM_ID
	AND INV.INV_PMA_ID = SNL_PMA_ID
	WHERE SNL_ProblemDescription IS NULL AND SalesNote.SNL_BatchNbr = @BatchNbr	
	...
	INSERT INTO Lot (LOT_ID, LOT_LTM_ID, LOT_OnHandQty, LOT_INV_ID,LOT_WHM_ID,LOT_CFN_ID,LOT_LTM_ExpiredDate,LOT_LTM_Lot,LOT_LTM_QRCode,LOT_LTM_DOM)
	SELECT LOT_ID, LOT_LTM_ID, Convert(decimal(18,2),LOT_OnHandQty), LOT_INV_ID ,tmp.LOT_WHM_ID,PD.PMA_CFN_ID,LM.LTM_ExpiredDate,LM.LTM_Lot,LM.LTM_QRCode,LM.LTM_Type
	FROM #tmp_lot AS TMP
	INNER JOIN LotMaster LM ON LM.LTM_ID=TMP.LOT_LTM_ID
	LEFT JOIN Product PD ON PD.PMA_ID=TMP.LOT_PMA_ID
	WHERE NOT EXISTS (SELECT 1 FROM Lot(nolock) WHERE Lot.LOT_LTM_ID = TMP.LOT_LTM_ID
	AND Lot.LOT_INV_ID = TMP.LOT_INV_ID)

	--生成销售单
	--取得表头数据
	select newid() as HeaderId
	--平台接口上传使用平台销售单号
	,CASE WHEN @ClientID is not null THEN SNL_SalesNo ELSE CONVERT(NVARCHAR(100),'')END as OrderNo
	,SNL_DMA_ID as DmaId,SNL_HOS_ID as HosId,
	...
	CONVERT(NVARCHAR(100),'') as BuName,SNL_ClientID,SNL_ServiceAgent AS ServiceAgent
	into #tmp_header
	...
	,isnull(SNL_SalesNo,'') AS SNL_SalesNo,isnull(SNL_ServiceAgent,'') AS SNL_ServiceAgent
	FROM SalesNote(nolock) 
	WHERE SNL_ProblemDescription IS NULL AND SalesNote.SNL_BatchNbr = @BatchNbr
	group by SNL_DMA_ID,SNL_HOS_ID,SNL_BUM_ID,SNL_Remark,SNL_SaleType,SNL_SaleDate,SNL_WriteOff,
	isnull(SNL_HospitalOffice,''), isnull(SNL_InvoiceNumber,''), isnull(SNL_InvoiceTitle,''), SNL_InvoiceDate,SNL_ClientID
	,isnull(SNL_SalesNo,''),isnull(SNL_ServiceAgent,'')
	) as t
	...
	INNER JOIN Lot(nolock) ON Lot.LOT_LTM_ID = a.SNL_LTM_ID
	INNER JOIN Inventory INV(nolock) ON INV.INV_ID = Lot.LOT_INV_ID AND INV.INV_WHM_ID = a.SNL_WHM_ID
	...
	BEGIN
		...
		BEGIN
			EXEC [GC_GetNextAutoNumber] @m_DmaId,'Next_ShipmentNbr',@m_BuName,@SubCompanyId,@BrandId, @m_OrderNo output
	...
	select LineId,ShipmentQty,LotId,DetailId,WhmId,UnitPrice,ShipmentDate,LotRemark,ExpiredDate,Lot,QRCode,DOM
	from #tmp_detail
	--left join (select SPI_WHM_ID,LOT_ID,SPI_LotShipmentDate,SPI_Remark from ShipmentInitProcessing si,Inventory inv,Lot l 
  ...
  --update BY hua Kaichun 寄售报销量不需要写入接口表让平台下载 20181012
  --写入接口表
	...
	--from ShipmentHeader sph(nolock),#tmp_header th where SPH_ID = th.HeaderId and SPH_Type='Consignment' and SPH_Status in ('Complete','Reversed') and SPH_ID not in (select SI_SPH_ID from SalesInterface  )
  ...
  DECLARE	curGenPurchaseOrder CURSOR 
	...
	*/
	...
	BEGIN
	 IF ( @P_DealerType = 'T2' AND @P_SaleType='2')
      BEGIN    
			 EXEC Consignment.Proc_ConsignmentInv_BuyOff 'Shipment_Consignment',@P_OrderNo,@SubCompanyId,@BrandId,@P_RtnVal output,@P_RtnMsg output
	 END
	
      IF (@P_DealerType = 'T1' AND @P_SaleType='2')
      BEGIN    
	  EXEC [GC_PurchaseOrder_AfterShipment] @P_OrderNo,'ConsignmentSales',@SubCompanyId,@BrandId,@P_RtnVal output, @P_RtnMsg output
	END
    IF (@P_DealerType in ('T1','LP','LS') AND @P_SaleType='3')
      BEGIN 
			--寄售销售后，生成寄售销售买断单据 
