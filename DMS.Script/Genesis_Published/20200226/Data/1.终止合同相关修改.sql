INSERT INTO  Contract.ExportTemplate VALUES
(newid(),'Termination','T2','二级经销商合同修改协议','~/Upload/ContractElectronicAttachmentTemplate/T2/Termination/T2修改协议.docx',1,1,2,NULL,NULL)
...
USE [GenesisDMS_PRD]
GO
/****** Object:  UserDefinedFunction [Contract].[GC_Fn_ContractAttach_list]    Script Date: 2020/1/15 13:32:26 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	IF EXISTS (SELECT 1 FROM Contract.ExportMain em INNER JOIN dbo.V_DivisionProductLineRelation dpr ON em.DeptId=dpr.DivisionCode WHERE DealerId=@DealerId AND ContractType='Appointment' AND DealerType=@DealerType AND em.DeptId=@ProductlineId)
	OR EXISTS (SELECT 1 FROM Contract.ExportT2Main etm INNER JOIN dbo.V_DivisionProductLineRelation dpr ON etm.DeptId=dpr.DivisionCode WHERE DealerId=@DealerId AND ContractType='Appointment' AND DealerType=@DealerType AND etm.DeptId=@ProductlineId )
	BEGIN 
