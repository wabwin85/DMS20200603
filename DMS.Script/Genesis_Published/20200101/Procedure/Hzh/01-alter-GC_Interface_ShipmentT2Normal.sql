...
--            AND WHM_Type = 'DefaultWH'
      ...
      --ShipmentType=Normal：默认仓库；ShipmentType=Consignment：寄售仓库
      declare @cnt int
	     select @cnt = COUNT(*) 
         from (
	...
	and c.PCT_ParentClassification_PCT_ID=b.CA_ID
      ...
      --根据单据号再更新一次产品线       
      update A set A.DNL_ProductLine_BUM_ID = POH.POH_ProductLine_BUM_ID
      from #DeliveryNote A , PurchaseOrderHeader POH
      where A.DNL_PONbr = POH.POH_OrderNo
...
--                             GROUP BY DN.DNL_Product_PMA_ID, DN.DNL_UPN) AS A
  ...
  --                           GROUP BY DN.DNL_Product_PMA_ID, DN.DNL_LTM_ID, DN.DNL_LotNumber) AS A
              ...
              BEGIN	
        				...
        				EXEC [Consignment].[Proc_ConsignmentInv_BuyOff] 'POReceipt_Consignment',@SubCompanyId,@BrandId,@PONum,@IsValid,@RtnMsg
              END
            END
          ELSE
            BEGIN
