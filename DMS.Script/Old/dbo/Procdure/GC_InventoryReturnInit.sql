DROP Procedure [dbo].[GC_InventoryReturnInit]
GO

/*
订单导入
*/
CREATE Procedure [dbo].[GC_InventoryReturnInit]
...
(
	...
	POReceiptPurchaseOrderNo nvarchar(100),
	Qty decimal(18,6),
...
UPDATE InventoryreturnInit SET IRI_PurchaseOrderNbr= Convert(nvarchar(20),isnull(dbo.fn_GetPriceForDealerRetrun( IRI_DMA_ID,IRI_CFN_ID, IRI_LotNumber + '@@' + IRI_QrCode,'4'),0))
...
ELSE
    ...
    /*上传按钮不写正式表，导入按钮才写*/
		...
		BEGIN
			INSERT INTO #TEMP 
				SELECT NULL,NULL,NEWID(),IRI_BUM_ID,IRI_DMA_ID,IRI_WHM_ID,NULL,NULL,IRI_CFN_ID,NULL,NULL,NULL,NULL,IRI_LotNumber+'@@'+IRI_QrCode,NULL,IRI_PRH_ID,IRI_PurchaseOrderNbr,IRI_ReturnQty,null
				FROM InventoryReturnInit 
				WHERE IRI_USER = @UserId
				...
				UPDATE #TEMP SET LtmId = LTM_ID,ExpiredDate = LTM_ExpiredDate FROM LotMaster WHERE LTM_LotNumber = LotNumber
				...
				--更新价格
				UPDATE #TEMP SET UnitPrice = dbo.fn_GetPriceForDealerRetrun(DealerId,CfnId,LotNumber,'4')

				--插入临时退货批次表
				INSERT INTO #mmbo_InventoryAdjustLot
				SELECT RDetailId,RLotId,Qty,LotId,WhmId,LotNumber,ExpiredDate,POReceiptId,UnitPrice ,NULL,NULL,NULL FROM #TEMP
