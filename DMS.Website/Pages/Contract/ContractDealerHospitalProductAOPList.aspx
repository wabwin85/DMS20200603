<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ContractDealerHospitalProductAOPList.aspx.cs"
    Inherits="DMS.Website.Pages.Contract.ContractDealerHospitalProductAOPList" %>
...
<head id="Head1" runat="server">
    ...
    <style type="text/css">
        ...
        .x-grid3-col-RefYear, .x-grid3-td-RefYear, .x-grid3-col-UnitY, .x-grid3-td-UnitY, .x-grid3-col-RefQ2, .x-grid3-td-RefQ2, .x-grid3-col-Q2, .x-grid3-td-Q2, .x-grid3-col-RefQ4, .x-grid3-td-RefQ4, .x-grid3-col-Q4, .x-grid3-td-Q4, .x-grid3-td-AOPD_Amount_Y, .x-grid3-col-AOPD_Amount_Y, .x-grid3-td-Unit4, .x-grid3-col-Unit4, .x-grid3-td-Unit5, .x-grid3-col-Unit5, .x-grid3-td-Unit6, .x-grid3-col-Unit6, .x-grid3-td-Unit10, .x-grid3-col-Unit10, .x-grid3-td-Unit11, .x-grid3-col-Unit11, .x-grid3-td-Unit12, .x-grid3-col-Unit12, .x-grid3-td-AOPD_Amount_4, .x-grid3-col-AOPD_Amount_4, .x-grid3-td-AOPD_Amount_5, .x-grid3-col-AOPD_Amount_5, .x-grid3-td-AOPD_Amount_6, .x-grid3-col-AOPD_Amount_6, .x-grid3-td-AOPD_Amount_10, .x-grid3-col-AOPD_Amount_10, .x-grid3-td-AOPD_Amount_11, .x-grid3-col-AOPD_Amount_11, .x-grid3-td-AOPD_Amount_12, .x-grid3-col-AOPD_Amount_12
        {
...
<body>
    ...
    <script type="text/javascript">
        ...
        var afterErrorDetails=function(){
            var aa= <%= txtAopErrorMag.ClientID %>.getValue()
            alert(aa);
            return true;
        }
        
         var cancelAuthorizationWindow =function()
        {
             <%= AuthorizationWindow.ClientID %>.hide(null);
        }
        ...
        function refreshLines(tree) {
           ...
           function refreshLinesPrice(tree) {
           if(<%= hidCheckHospitalProudct.ClientID %>.getValue()=="0")
           {
                Ext.Msg.alert('Error', '每一家医院都必须维护产品分类！');
         ...
         var SubmitClick = function(tree) {
            ...
            Coolite.AjaxMethods.SaveAuthorProduct(Nodesid.toString(), { success: function(result) {if(result==''){ Ext.getCmp('<%=this.txtGPHospitalUpdate.ClientID%>').reload();  Ext.getCmp('<%=this.GridAuthorization.ClientID%>').reload(); Ext.getCmp('<%=this.GridPanelAOPStore.ClientID%>').reload();Ext.getCmp('<%=this.GridDealerAop.ClientID%>').reload();Ext.getCmp('<%=this.AuthorizationWindow.ClientID%>').hide();}else{Ext.Msg.alert('Error', result); } }, failuer: function(err) { Ext.Msg.alert('Error', err); } });
        }
         ...
         function refreshLinesProduct(tree) {
            Coolite.AjaxMethods.RefreshLinesProduct({
                success: function(result) {
                    var nodes = eval(result);
                    if (tree.root != null)
                        tree.root.ui.remove();
        ...
        function refreshLinesProduct2(tree) {
            Coolite.AjaxMethods.RefreshLinesProduct2({
                success: function(result) {
                    var nodes = eval(result);
                    if (tree.root != null)
                        tree.root.ui.remove();
        ...
        var changeColumn1 = function(value) {
         ...
         function prepareCommand1(grid, toolbar, rowIndex, record) {
            var firstButton = toolbar.items.get(0);
            var type = record.data.AOPType;
    ...
    <div>
        ...
        <ext:JsonStore ID="AuthorizationStore" runat="server" UseIdConfirmation="false" OnRefreshData="AuthorizationStore_RefershData"
            AutoLoad="true">
            <AutoLoadParams>
            ...
            <Reader>
                <ext:JsonReader ReaderID="HOS_ID">
                    <Fields>
                        <ext:RecordField Name="HOS_ID" />
                        <ext:RecordField Name="HOS_HospitalName" />
                        <ext:RecordField Name="Product" />
                    </Fields>
                </ext:JsonReader>
            </Reader>
            <SortInfo Field="HOS_HospitalName" Direction="ASC" />
        </ext:JsonStore>
            ...
            <Body>
                <ext:FitLayout ID="FitLayout1" runat="server">
                    <ext:TabPanel ID="TabPanelDeatil" runat="server" ActiveTabIndex="0" Border="false">
                        <Tabs>
                            <ext:Tab ID="HospitalMapping" runat="server" Title="经销商授权医院" Icon="ChartOrganisation"
                                AutoShow="true">
                                <Body>
                                    <ext:BorderLayout ID="BorderLayout1" runat="server">
                                        ...
                                        <Center MarginsSummary="0 5 5 5">
                                            <ext:Panel runat="server" ID="plAuthorization" Border="false" Frame="true" Title="第一步：维护授权医院与产品分类关系(如不维护将无对应医院指标)"
                                                Icon="HouseKey" Height="200" ButtonAlign="Left">
                                                <Body>
                                                    <ext:FitLayout ID="FitLayout2" runat="server">
                                                        <ext:GridPanel ID="GridAuthorization" runat="server" Header="false" StoreID="AuthorizationStore"
                                                            Border="false" Icon="Lorry" StripeRows="true">
                                                            <ColumnModel ID="ColumnModel3" runat="server">
                                                                <Columns>
                                                                    <ext:CommandColumn ColumnID="Details" Header="编辑" Width="60" Align="Center">
                                                                        <Commands>
                                                                            <ext:GridCommand Icon="VcardEdit" CommandName="Modify" ToolTip-Text="编辑">
                                                                            </ext:GridCommand>
                                                                        </Commands>
                                                                    </ext:CommandColumn>
                                                                    <ext:Column ColumnID="HOS_HospitalName" DataIndex="HOS_HospitalName" Header="授权医院"
                                                                        Width="200">
                                                                    </ext:Column>
                                                                    <ext:Column ColumnID="Product" DataIndex="Product" Header="产品分类" Width="700">
                                                                    </ext:Column>
                                                                </Columns>
                                                            </ColumnModel>
                                                            <SelectionModel>
                                                        ...
                                                        Coolite.AjaxMethods.EditAuthProduct(record.data.HOS_ID,record.data.HOS_HospitalName,{
                                                            success: function() {
                                                                #{TreeProduct}.root.ui.remove();
                                                                #{TreeProduct}.root.render();
                                                                refreshLinesProduct(#{TreeProduct});
                                                ...
                                                <Buttons>
                                                    <ext:Button ID="Button5" runat="server" Text="提交，进入下一步" Icon="Disk" CommandArgument=""
                                                        CommandName="" IDMode="Legacy">
                                                        <Listeners>
                                                            <%--  <Click Fn="nextPage" />--%>
                                                            <Click Handler="refreshLinesPrice(#{menuTree});" />
                                                        </Listeners>
                                                    </ext:Button>
                                                </Buttons>
                                            </ext:Panel>
                                            ...
                                            <ext:Panel runat="server" ID="Panel11" Border="false" Frame="true" Title="第二步：选择产品价格"
                                                ...
                                                <Body>
                                                    <ext:FitLayout ID="FitLayout6" runat="server">
                                                        <ext:TreePanel ID="menuTree" runat="server" Title="产品分类价格" Header="true" RootVisible="true">
                                                            <Tools>
                                                ...
                                                <Buttons>
                                                    <ext:Button ID="Button8" runat="server" Text="提交，进入下一步" Icon="Disk" CommandArgument=""
                                                        ...
                                                        <Listeners>
                                                            <%--   <Click Fn="nextPage2" />--%>
                                                            <Click Handler="SubmitPriceClick(#{menuTree});" />
                                                        </Listeners>
                                                    </ext:Button>
                                                </Buttons>
                                            </ext:Panel>
                            ...
                            <ext:Tab ID="DealerAop" runat="server" Title="指标维护" Icon="ChartOrganisation" AutoShow="true"
                                Enabled="false">
                                <Body>
                                    <ext:BorderLayout ID="BorderLayout2" runat="server">
                                        ...
                                        <Center Collapsible="True" MarginsSummary="0 5 5 5">
                                            <ext:Panel runat="server" ID="PanelHospitalProduct" Border="false" Frame="true" Title="第三步：维护经销商医院指标: <img src='/resources/images/icons/flag_ch.png' > </img> 代表本次新授权医院"
                                                Icon="HouseKey">
                                                <Body>
                                        ...
                                        <South Collapsible="True" MarginsSummary="0 5 0 5">
                                            <ext:Panel runat="server" ID="PanelDealerAop" Border="false" Frame="true" Title="第四步：维护经销商医院指标: <img src='/resources/images/icons/exclamation.png' > </img> 经销商指标小于医院指标或者大于医院指标的20% "
                                                Icon="HouseKey" Height="200" ButtonAlign="Left">
                                                <Body>
                                                        ...
                                                        <ext:GridPanel ID="GridDealerAop" runat="server" Header="false" StoreID="AOPDealerStore"
                                                            ...
                                                            <ColumnModel ID="ColumnModel2" runat="server">
                                                                <Columns>
                                                                    ...
                                                                    <ext:Column ColumnID="AOPType" DataIndex="AOPType" Header="指标类型" Width="80" Align="Center">
                                                                    </ext:Column>
        ...
        <ext:Window ID="AOPEditorWindow" runat="server" Icon="Group" Title="经销商医院指标设置" Closable="false"
            AutoShow="false" ShowOnLoad="false" Resizable="false" Height="580" Draggable="false"
            Width="900" Modal="true" BodyStyle="padding:5px;">
            <Body>
        ...
        <ext:Window ID="AuthorizationWindow" runat="server" Icon="Group" Title="选择产品分类" Closable="false"
            AutoShow="false" ShowOnLoad="false" Resizable="false" Height="550" Draggable="false"
            Width="600" Modal="true" BodyStyle="padding:5px;">
            <Body>
                <ext:ColumnLayout ID="ColumnLayout1" runat="server" FitHeight="true">
                    <ext:LayoutColumn ColumnWidth="1">
                        <ext:Panel ID="Panel1" runat="server" Frame="true" AutoScroll="true" Header="true"
                            Title="基础信息">
                            <Body>
                                <ext:FormLayout ID="FormLayout1" runat="server" LabelWidth="80">
                                    <ext:Anchor Horizontal="100%">
                                        <ext:Label ID="blAuthor_HospitalName" runat="server" FieldLabel="医院名称">
                                        </ext:Label>
                                    </ext:Anchor>
                                    <ext:Anchor Horizontal="100%">
                                        <ext:Hidden runat="server" ID="hidAuthor_HospitalId">
                                        </ext:Hidden>
                                    </ext:Anchor>
                                    <ext:Anchor Horizontal="100%">
                                        <ext:TreePanel ID="TreeProduct" runat="server" Title="产品分类" Header="true" RootVisible="true">
                                            <Tools>
                                                <ext:Tool Type="Refresh" Handler="refreshLinesProduct(#{TreeProduct}); " />
                                            </Tools>
                                        </ext:TreePanel>
                                    </ext:Anchor>
                                </ext:FormLayout>
            ...
            <Buttons>
                <ext:Button ID="Button3" runat="server" Text="全选耗材" Icon="Disk">
                    <Listeners>
                        <Click Handler="refreshLinesProduct2(#{TreeProduct});" />
                    </Listeners>
                </ext:Button>
                <ext:Button ID="Button1" runat="server" Text="确认" Icon="Disk">
                    <Listeners>
                        <Click Handler="SubmitClick(#{TreeProduct});" />
                    </Listeners>
                </ext:Button>
                <ext:Button ID="Button2" runat="server" Text="关闭" Icon="Cancel">
                    <Listeners>
                        <Click Handler="cancelAuthorizationWindow();" />
                    </Listeners>
                </ext:Button>
            </Buttons>
        </ext:Window>
