...
InventoryInit = function () {
    ...
    var business = 'Inventory.InventoryInit';
    ...
    that.Init = function () {
        ...
        $('#BtnDownTemplate').FrameButton({
            ...
            icon: 'file',
            onClick: function () {
                window.open('/Upload/ExcelTemplate/Template_InventoryImport.xls');
            }
        });
        ...
        $('#InvInitUpload').kendoUpload({
            async: {
                saveUrl: "../Handler/UploadFile.ashx?Type=InventoryInit&SheetName=Sheet1&Dest=Inv",
                autoUpload: true
            },
            localization: {
    ...
    var fields = {
        ExpiredDate: { type: "date", validation: { format: "{0: yyyy-MM-dd}" } },
        Qty: { type: "number", validation: { min: 0.0000001, max: 999999999.0 } }
    }
    ...
    var createInitResultList = function () {
        $("#RstInitImportResult").kendoGrid({
            ...
            columns: [
                ...
                {
                    field: "DealerSapCode", title: "经销商编号", width: 120, editable: function () {
                        return false;
                    },
                    template: "<a style='text-decoration:none;color:black;' title='#=DealerSapCodeErrMsg==null?'':DealerSapCoderrMsg#'>#=DealerSapCode==null?'':DealerSapCode#</a>",
                    headerAttributes: { "class": "text-center text-bold", "title": "经销商编号" }
                },
                {
                    field: "DealerName", title: "经销商名称", width: 150, editable: function () {
                        return false;
                    },
                ...
                {
                    field: "WarehouseType", title: "仓库类型", width: 120, editable: function () {
                        return false;
                    },
                    headerAttributes: { "class": "text-center text-bold", "title": "仓库类型" }
                },
                {
                    field: "ArticleNumber", title: "产品型号(UPN)", width: 120,
                ...
                {
                    field: "ExpiredDate", title: "产品有效期", format: "{0: yyyy-MM-dd}", width: 120, editable: function () {
                        return false;
                    },
                ...
                {
                    field: "Period", title: "库存期间", format: "{0: yyyy-MM-dd}", width: 120,
                    template: "<a style='text-decoration:none;color:black;' title='#=PeriodErrMsg==null?'':PeriodErrMsg#'>#=Period==null?'':Period#</a>",
                    headerAttributes: { "class": "text-center text-bold", "title": "库存期间" }
                },
                {
                    title: "删除", width: 50,
                    headerAttributes: {
    ...
    that.ImportCorrectData = function () {
        ...
        for (var i = 0; i < initData.length ; i++) {
            ...
            if (record.Warehouse == null || record.Warehouse == '' ||
                record.ArticleNumber == null || record.ArticleNumber == '' || record.LotNumber == null || record.LotNumber == '' ||
                record.Period == null || record.Period == '' || record.Qty == null || record.Qty == '') {
                FrameWindow.ShowAlert({
                    target: 'top',
                    alertType: 'info',
                    message: '必填项为空！',
