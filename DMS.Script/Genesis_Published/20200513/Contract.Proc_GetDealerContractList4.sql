...
ALTER PROCEDURE [Contract].[Proc_GetDealerContractList]
    (
      ...
      @InitDealerName NVARCHAR(50),    --此处用于传当前用户ID
...
AS
    BEGIN  
	DECLARE @IsAdmin INT;
	IF (@InitDealerName='00000000-0000-0000-0000-000000000000' OR EXISTS (
	       SELECT 1
	       FROM   Lafite_IDENTITY_MAP T1,
	              Lafite_ATTRIBUTE T2
	       WHERE  T1.MAP_ID = T2.Id
	              AND T1.IDENTITY_ID = @InitDealerName
   ...
   DealerID uniqueidentifier
    ...
    ProductLineName,DivisionCode,ContractStatus,DMA_Phone,DealerID)
     SELECT
	  ...
	  AND (@IsAdmin=1 OR T.CreateUser=@InitDealerName)
      ...
      OR (T.DealerType='T1' OR T.DealerType='T2'))
		AND (
				(@SubCompanyId IS NULL OR PR.SubCompanyId = @SubCompanyId) AND (@BrandId IS NULL OR PR.BrandId = @BrandId)
			)
      IF @DealerType='T2'
        BEGIN
      ...
      IF(@DealerType='T2')
       BEGIN
        ...
        ProductLineName,DivisionCode,CASE  ContractStatus WHEN 'NewContract' THEN '已生成' ELSE '未生成' END AS ContractStatus,DMA_Phone,DealerID,dbo.FN_Contract_IsPassTraining(ContractId,DealerID,DealerType) AS IsPassTraining
