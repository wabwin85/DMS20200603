DROP VIEW [dbo].[V_DealerContractMaster]
GO
...
CREATE VIEW [dbo].[V_DealerContractMaster]
AS
SELECT DCM_ID ID,
       DCM_DMA_ID DMA_ID,
	  ...
	  --获取临时授权延展时间
	    CASE WHEN EXISTS (SELECT 1 FROM DealerContract 
				 inner join DealerAuthorizationTable on DCL_ID=DAT_DCL_ID  AND DAT_Type='Temp'
				 where DCL_DMA_ID=DCM_DMA_ID and DCL_CC_ID=DCM_CC_ID)
				 AND (SELECT  CONVERT(NVARCHAR(10),ISNULL(MAX(DAT_EndDate),'1900-01-01'),120) FROM DealerContract inner join DealerAuthorizationTable on DCL_ID=DAT_DCL_ID AND DAT_Type='Temp'
				 where DCL_DMA_ID=DCM_DMA_ID and DCL_CC_ID=DCM_CC_ID) >=CONVERT(NVARCHAR(10),MinDate,120)
			 THEN  (SELECT MAX(DAT_EndDate) FROM DealerContract inner join DealerAuthorizationTable on DCL_ID=DAT_DCL_ID  AND DAT_Type='Temp'
				 where DCL_DMA_ID=DCM_DMA_ID and DCL_CC_ID=DCM_CC_ID)
				ELSE MinDate END
        AS  MinDate,
       CASE
          ...
          THEN
             ...
             --判断当前是否包含有效授权
             CASE WHEN EXISTS (SELECT 1 FROM DealerContract 
					 inner join DealerAuthorizationTable on DCL_ID=DAT_DCL_ID  AND GETDATE() BETWEEN  DAT_StartDate AND DAT_EndDate AND DAT_Type='Temp'
					 where DCL_DMA_ID=DCM_DMA_ID and DCL_CC_ID=DCM_CC_ID
             )
             THEN 1 ELSE 0 END 
