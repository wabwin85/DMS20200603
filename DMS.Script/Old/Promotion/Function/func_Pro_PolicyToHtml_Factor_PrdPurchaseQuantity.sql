DROP FUNCTION [Promotion].[func_Pro_PolicyToHtml_Factor_PrdPurchaseQuantity]
GO

CREATE FUNCTION [Promotion].[func_Pro_PolicyToHtml_Factor_PrdPurchaseQuantity](
	@PolicyFactorId INT
	)
...
BEGIN
	...
	SET @SQL = CASE WHEN @ProductPolicyFactorId IS NULL THEN '指定产品' 
		ELSE '产品['+CONVERT(NVARCHAR,@ProductPolicyFactorId)+']' END +'的商业采购量'
