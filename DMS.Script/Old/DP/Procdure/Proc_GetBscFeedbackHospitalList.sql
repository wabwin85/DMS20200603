DROP PROCEDURE [DP].[Proc_GetBscFeedbackHospitalList]
GO


CREATE PROCEDURE [DP].[Proc_GetBscFeedbackHospitalList]
(
...
BEGIN
	...
	FROM   interface.RV_HospitalKPI_WarningLevel Warning
	       ...
	       INNER JOIN DP.BscFeedbackProposal FeedbackProposal
	            ON  Feedback.FeedbackCode = FeedbackProposal.FeedbackCode
				    AND FeedbackProposal.HospitalCode = Warning.DMSCode
	       ...
	       LEFT JOIN dp.DealerFeedback DealerFeedback
	            ON  Warning.DivisionID = DealerFeedback.Bu
	                AND Warning.SubBUCode = DealerFeedback.SubBu
	                AND Warning.[Year] = DealerFeedback.[Year]
	                AND Warning.[Quarter] = DealerFeedback.[Quarter]
	                AND Warning.SAPID = DealerFeedback.SapCode
