...
/**********************************************
	...
	1.创建 2017-03-14
**********************************************/
ALTER PROCEDURE [dbo].[GC_OrderDiscountRuleInit] 
	@UserId NVARCHAR(36),
...
BEGIN
	...
	UPDATE ConsignmentDiscountRuleInit SET ErrMassage='当前分子公司及品牌下不存在该产品线' 
	WHERE ISNULL(ErrMassage,'')='' AND CreateUser=@UserId
	AND NOT EXISTS (SELECT 1 FROM V_DivisionProductLineRelation R WHERE R.ProductLineName=BU AND R.SubCompanyId=@SubCompanyId AND r.BrandId=@BrandId);
	...
	UPDATE A  SET ErrMassage='UPN不存在' 
	FROM dbo.ConsignmentDiscountRuleInit A
	WHERE ISNULL(ErrMassage,'')=''  AND CreateUser=@UserId
	AND NOT EXISTS(SELECT 1 FROM dbo.CFN WHERE CFN_CustomerFaceNbr=A.UPN)
	...
	AND B.SubCompanyId=@SubCompanyId AND B.BrandId=@BrandId
	...
	AND(((ISNULL(CONVERT(NVARCHAR,LeftValue),'')<>'' OR ISNULL(CONVERT(NVARCHAR,RightValue),'')<>'') AND ISNULL(DiscountValue,'0')='0' )
	OR (ISNULL(CONVERT(NVARCHAR,LeftValue),'')='' AND ISNULL(CONVERT(NVARCHAR,RightValue),'')='' and ISNULL(DiscountValue,'0')<>'0'))	  
	AND CreateUser=@UserId;
	
	UPDATE ConsignmentDiscountRuleInit SET ErrMassage='折扣规则适用时间没有限定' 
	WHERE ISNULL(ErrMassage,'')=''  
	and (ISNULL(BeginDate,'')='' OR ISNULL(EndDate,'')='')
	AND ISNULL(DiscountValue ,'0')<>'0' 
	AND CreateUser=@UserId;
	...
	SET @PRODUCT_CUR=cursor for 
		...
		WHERE EXISTS (SELECT 1 FROM ConsignmentDiscountRuleInit B WHERE B.CreateUser=@UserId and CONVERT(NVARCHAR(10),C.ProductLineName)=CONVERT(NVARCHAR(10),B.BU) AND ISNULL(B.ErrMassage,'')='' and ISNULL(B.DiscountValue,0)<>0.00)
	...
	BEGIN
		...
		FROM ConsignmentDiscountRuleInit A 
		INNER JOIN V_DivisionProductLineRelation C ON C.ProductLineName=A.BU AND C.IsEmerging='0' AND C.SubCompanyId=@SubCompanyId AND C.BrandId=@BrandId
		...
		WHERE A.CreateUser=@UserId AND ISNULL(A.DiscountValue,'0')='0' 
		...
		AND ISNULL(A.DiscountValue,'0')<>'0'
	...
	BEGIN
		...
		FROM ConsignmentDiscountRuleInit A
		INNER JOIN V_DivisionProductLineRelation B ON A.BU=B.ProductLineName AND B.IsEmerging='0' AND B.SubCompanyId=@SubCompanyId AND B.BrandId=@BrandId 
