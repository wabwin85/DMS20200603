...
namespace DMS.Signature
{
    ...
    public class BaseSqlMapDao : IBaseSqlDao
    {
        ...
        public IList<T> ExecuteQueryForList<T>(string statementName, object parameterObject, int start, int limit, out int totalRowCount)
        {
            ...
            try
            {
                ...
                using (IDbCommand cmd = request.IDbCommand)
                {
                    ...
                    cmdText = string.Format("select Extent1.* from ({1})  AS [Extent1] WHERE [Extent1].[row_number] > {2} AND [Extent1].[row_number] <= ({2}+{0}) order by [Extent1].[row_number]", limit, sqlText, start);
        ...
        /// <param name="totalRowCount">The total row count.</param>
        /// <returns></returns>
        public DataSet ExecuteQueryForDataSet(string statementName, object paramObject, int start, int limit, out int totalRowCount)
        {
            ISqlMapper mapper = this.SqlMap;
