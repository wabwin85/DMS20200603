...
/****** Object:  StoredProcedure [Contract].[Proc_GetDealerContractList]    Script Date: 2019/9/27 22:22:47 ******/
SET ANSI_NULLS ON
GO
     ...
     SELECT
      ...
      OR (T.DealerType='T1' OR T.DealerType='T2'))
		AND ((PR.SubCompanyId = @SubCompanyId AND PR.BrandId = @BrandId) 
					OR (ContractStatus='Draft' AND ISNULL(T.DepId,0)=0)
				)
      IF @DealerType='T2'
        BEGIN
      ...
      IF(@DealerType='T2')
       BEGIN
        ...
        ProductLineName,DivisionCode,CASE  ContractStatus WHEN 'NewContract' THEN '已生成' ELSE '未生成' END AS ContractStatus,DMA_Phone,DealerID  
