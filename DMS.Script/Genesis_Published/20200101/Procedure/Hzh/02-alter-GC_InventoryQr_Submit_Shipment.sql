...
ALTER PROCEDURE [dbo].[GC_InventoryQr_Submit_Shipment]
...
SET NOCOUNT ON
	...
	CREATE TABLE #ShipmentLot(
		...
		[SLT_Lot]   [nvarchar](50) NULL,	
		[SLT_QRCode]  [nvarchar](50) NULL,	
		[SLT_ExpiredDate] [datetime] NULL,
		[SLT_DOM]  [nvarchar](30) NULL
	)
	/*附件临时表*/
	...
	SELECT distinct IO_Id AS ID,
		...
		CAST(null AS nvarchar(max)) AS Error_Messinge
	...
	AND EXISTS (SELECT  1
			  ...
			  INNER JOIN DealerContract AS DC(nolock) ON DA.DAT_DCL_ID = DC.DCL_ID
	...
	AND EXISTS (SELECT 1 FROM interface.T_I_WC_DealerBarcodeQRcodeScan(nolock) 
	...
	SELECT NEWID() AS Id,WarehouseTypeCode INTO #ShipmentHeadId_TEMP
	...
	SELECT SplId,Qty,LotId,SLT_ID,WhmId,Price,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,LotNumber,BarCode,ExpiredDate,DOM FROM #TEMP
...
BEGIN TRAN
	...
	IF((SELECT COUNT(*) FROM #TEMP WHERE Error_Messinge IS NOT NULL)>0)
     ...
     --附件信息
     IF ((SELECT COUNT(*) FROM Attachment(nolock) WHERE AT_Main_ID = @DealerId AND AT_Type = 'Dealer_Shipment_Qr') > 0)
		BEGIN
			...
			FROM #ShipmentHeader INNER JOIN Attachment(nolock) ON #ShipmentHeader.SPH_Dealer_DMA_ID = Attachment.AT_Main_ID
   ...
   ,LOT_LTM_ExpiredDate datetime
   ,LOT_LTM_Lot	 nvarchar(50)
   ,LOT_LTM_QRCode nvarchar(50)
   ,LOT_LTM_DOM nvarchar(50)
   ,LOT_CreateDate datetime
    ...
    BEGIN 
		...
		WHERE Id IN (
						SELECT RootID FROM Cache_OrganizationUnits(nolock) 
						WHERE AttributeID = Convert(varchar(36),@ProductLineId)
					) 
		AND ATTRIBUTE_TYPE = 'BU'		
		
		EXEC [GC_GetNextAutoNumber] @DealerId,'Next_ShipmentNbr',@m_BuName,@SubCompanyId,@BrandId, @m_OrderNo OUTPUT
       ...
       --修改单据状态,由于要某些情况下要生成清指定批号订单所已把修改单据状态放到这
		...
		SELECT @DMA_TYPE = DMA_DealerType FROM DealerMaster(nolock) WHERE DMA_ID=@DealerId
		...
		BEGIN
			...
			EXEC Consignment.Proc_ConsignmentInv_BuyOff 'Shipment_Consignment',@m_OrderNo,@SubCompanyId,@BrandId,@RtnVal OUTPUT,@RtnMsg OUTPUT
 ...
 ,LOT_CFN_ID , LOT_LTM_ExpiredDate, LOT_LTM_Lot , LOT_LTM_QRCode , LOT_LTM_DOM , LOT_CreateDate )
SELECT A.LotId,
		A.LTM_ID,
		A.WhmId,
		A.PmaId,
...
FROM 
    (SELECT #TEMP.LotId,
		...
		#TEMP.PmaId,
		InvId,
		#TEMP.LotNumber+'@@'+#TEMP.BarCode AS LotNumber,SUM(#TEMP.Qty) AS QTY
		,#TEMP.CfnId AS CfnId
		,#TEMP.ExpiredDate AS  ExpiredDate
		,#TEMP.LotNumber AS LOT
		,#TEMP.BarCode as QRCode
		,#TEMP.DOM as DOM
		, GETDATE() AS CreateDate
    FROM #TEMP
    GROUP BY  #TEMP.LotId,#TEMP.LTM_ID,#TEMP.WhmId,#TEMP.PmaId,InvId,#TEMP.LotNumber,#TEMP.BarCode,#TEMP.CfnId ,#TEMP.ExpiredDate,#TEMP.DOM)A 
	--更新批次库存
	UPDATE Lot SET LOT.LOT_OnHandQty=Convert(decimal(18,2),LOT.LOT_OnHandQty)+Convert(decimal(18,2),-#tmp_lot.LOT_OnHandQty),LOT.LOT_CreateDate=GETDATE()
