...
/****** Object:  StoredProcedure [dbo].[GC_InventoryInit]    Script Date: 2019/12/11 18:30:19 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
ALTER Procedure [dbo].[GC_InventoryInit]
...
WHERE PMA_ID = II_PMA_ID AND II_QTY_ErrMsg IS NULL AND II_CFN_ErrMsg IS NULL AND II_USER = @UserId AND 
...
/* 检查导入的CFN产品线是否正确 */
UPDATE InventoryInit SET II_CFN_ErrMsg = '产品对应的产品线不正确'
WHERE II_CFN_ErrMsg IS NULL AND II_USER = @UserId
AND not exists (select 1 from CFN INNER JOIN View_ProductLine(nolock) VP ON CFN_ProductLine_BUM_ID=VP.Id WHERE CFN_ID = II_CFN_ID AND VP.SubCompanyId=@SubCompanyId AND VP.BrandId=@BrandId )
...
/* 检查同一产品同一批次号是否有效期相同 */
UPDATE InventoryInit SET II_LTM_ExpiredDate_ErrMsg = '同批号的有效期必须相同'
...
ELSE
	BEGIN
		...
		*/
		...
		SELECT INV_OnHandQuantity, NEWID(), '00000000-0000-0000-0000-000000000000', '期初库存', INV_WHM_ID, INV_PMA_ID, 0, '期初库存导入'
		FROM #tmp_inventory
		...
		SELECT LOT_ID, LOT_LTM_ID, Convert(decimal(18,2),LOT_OnHandQty), LOT_INV_ID FROM #tmp_lot
