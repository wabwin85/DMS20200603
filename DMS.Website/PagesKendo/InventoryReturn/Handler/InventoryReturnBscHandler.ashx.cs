...
{
    /// <summary>
    /// InventoryReturnBscHandler 的摘要说明
    /// </summary>
    public class InventoryReturnBscHandler : IHttpHandler
    {
        private EkpWorkflowBLL ekpBll = new EkpWorkflowBLL();
        public void ProcessRequest(HttpContext context)
        {
            InventoryReturnBscVO model = new InventoryReturnBscVO();
            try
            {
                ...
                model = JsonConvert.DeserializeObject<InventoryReturnBscVO>(data);
                DMS.Business.InventoryReturnBscService service = new DMS.Business.InventoryReturnBscService();

                if (model.Method == "InitPage")
                {
                    model.IsSuccess = ekpBll.ValidateUserCanEditEkpProcess(model.AdjId, RoleModelContext.Current.User.LoginId);
